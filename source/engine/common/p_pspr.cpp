...
//-----------------------------------------------------------------------------
//
// $Id: b87b8ed06bf1f614f29cbb5ab4fed948f6436750 $
//
...
void P_SwitchWeapon(player_t *player)
{
    ...
    {
        if (player->weaponowned[i] && P_EnoughAmmo(player, static_cast<weapontype_t>(i), true) &&
...
bool P_CheckSwitchWeapon(player_t *player, weapontype_t weapon)
{
    ...
    if ((multiplayer && !sv_allowpwo) || player->userinfo.switchweapon == WPSW_ALWAYS || demoplayback)
    {
        return true;
    }
    ...
    if (player->userinfo.switchweapon == WPSW_NEVER && player->readyweapon != NUMWEAPONS &&
        player->pendingweapon != NUMWEAPONS)
        return false;

    weapontype_t currentweapon = (player->pendingweapon == wp_nochange) ? player->readyweapon : player->pendingweapon;
...
void A_ReFire(AActor *mo)
{
    ...
    if ((player->cmd.buttons & BT_ATTACK && G_CanFireWeapon()) && player->pendingweapon == wp_nochange &&
        player->health)
    {
        player->refire++;
        P_FireWeapon(player);
...
void A_Punch(AActor *mo)
{
    ...
    M_LogWDLEvent(WDL_EVENT_SSACCURACY, player, NULL, player->mo->angle / 4, MOD_FIST, 0, GetMaxShotsForMod(MOD_FIST));
...
void A_Saw(AActor *mo)
{
    ...
    M_LogWDLEvent(WDL_EVENT_SSACCURACY, player, NULL, player->mo->angle / 4, MOD_CHAINSAW, 0,
                  GetMaxShotsForMod(MOD_CHAINSAW));
    ...
    // turn to face target
    angle = P_PointToAngle(player->mo->x, player->mo->y, linetarget->x, linetarget->y);
...
void A_RefireTo(AActor *mo)
{
    ...
    if ((psp->state->args[1] || P_CheckAmmo(player)) && (player->cmd.buttons & BT_ATTACK) &&
...
void A_WeaponMeleeAttack(AActor *mo)
{
    ...
    // un-missed!
    UV_SoundAvoidPlayer(player->mo, CHAN_WEAPON, SoundMap[hitsound], ATTN_NORM);
...
void A_WeaponSound(AActor *mo)
{
    ...
    UV_SoundAvoidPlayer(player->mo, CHAN_WEAPON, SoundMap[sndmap], (psp->state->args[1] ? ATTN_NONE : ATTN_NORM));
}
...
void A_FireRailgun(AActor *mo)
{
    ...
    M_LogWDLEvent(WDL_EVENT_SSACCURACY, player, NULL, player->mo->angle / 4, MOD_RAILGUN, 0,
                  GetMaxShotsForMod(MOD_RAILGUN));
...
void P_FireHitscan(player_t *player, size_t quantity, spreadtype_t spread)
{
    ...
    bool predict_puffs = clientside && !serverside && consoleplayer().userinfo.predict_weapons;
...
void A_FirePistol(AActor *mo)
{
    ...
    M_LogWDLEvent(WDL_EVENT_SSACCURACY, player, NULL, player->mo->angle / 4, MOD_PISTOL, 0,
                  GetMaxShotsForMod(MOD_PISTOL));
}
...
void A_FireShotgun(AActor *mo)
{
    ...
    M_LogWDLEvent(WDL_EVENT_SPREADACCURACY, player, NULL, player->mo->angle / 4, MOD_SHOTGUN, 0,
                  GetMaxShotsForMod(MOD_SHOTGUN));
}
...
void A_FireShotgun2(AActor *mo)
{
    ...
    M_LogWDLEvent(WDL_EVENT_SPREADACCURACY, player, NULL, player->mo->angle / 4, MOD_SSHOTGUN, 0,
                  GetMaxShotsForMod(MOD_SSHOTGUN));
}
...
void A_FireCGun(AActor *mo)
{
    ...
    P_SetPsprite(player, ps_flash,
                 (statenum_t)(weaponinfo[player->readyweapon].flashstate + psp->state - &states[S_CHAIN1]));

    spreadtype_t accuracy = player->refire ? SPREAD_NORMAL : SPREAD_NONE;
    P_FireHitscan(player, 1, accuracy);

    M_LogWDLEvent(WDL_EVENT_SSACCURACY, player, NULL, player->mo->angle / 4, MOD_CHAINGUN, 0,
                  GetMaxShotsForMod(MOD_CHAINGUN));
}
...
void A_BFGSpray(AActor *mo)
{
    ...
    for (i = 0; i < 40; i++)
    {
        ...
        new AActor(linetarget->x, linetarget->y, linetarget->z + (linetarget->height >> 2), MT_EXTRABFG);
...
VERSION_CONTROL(p_pspr_cpp, "$Id: b87b8ed06bf1f614f29cbb5ab4fed948f6436750 $")
