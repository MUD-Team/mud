...
//-----------------------------------------------------------------------------
//
// $Id: 22b0575c5ba83e9d49686ec129c66351daf303fe $
//
...
static ItemEquipVal P_GiveAmmoAutoSwitch(player_t *player, ammotype_t ammo, int oldammo)
{
    ...
    if (player->userinfo.switchweapon != WPSW_NEVER)
    {
            ...
            player->ammo[weaponinfo[player->readyweapon].ammotype] != ammo)
        {
            ...
            {
                if (player->weaponowned[i] && !(weaponinfo[i].flags & WPF_NOAUTOSWITCHTO) &&
                    weaponinfo[i].ammotype == ammo && weaponinfo[i].ammouse > oldammo &&
...
bool P_SpecialIsWeapon(AActor *special)
{
    ...
    return (special->type == MT_CHAINGUN || special->type == MT_SHOTGUN || special->type == MT_SUPERSHOTGUN ||
            special->type == MT_MISC25 || special->type == MT_MISC26 || special->type == MT_MISC27 ||
...
void P_GiveSpecial(player_t *player, AActor *special)
{
    ...
    case SPR_BFUG:
        ...
        {
            M_LogWDLPickupEvent(player, special, WDL_PICKUP_BFG, special->flags & MF_DROPPED);
        }
        break;
    ...
    case SPR_MGUN:
        ...
        if (val == IEV_EquipStay || val == IEV_EquipRemove)
        {
            M_LogWDLPickupEvent(player, special, WDL_PICKUP_CHAINGUN, special->flags & MF_DROPPED);
        }
        break;
    ...
    case SPR_CSAW:
        ...
        if (val == IEV_EquipStay || val == IEV_EquipRemove)
        {
            M_LogWDLPickupEvent(player, special, WDL_PICKUP_CHAINSAW, special->flags & MF_DROPPED);
        }
        break;
    ...
    case SPR_LAUN:
        ...
        if (val == IEV_EquipStay || val == IEV_EquipRemove)
        {
            M_LogWDLPickupEvent(player, special, WDL_PICKUP_ROCKETLAUNCHER, special->flags & MF_DROPPED);
        }
        break;
    ...
    case SPR_PLAS:
        ...
        if (val == IEV_EquipStay || val == IEV_EquipRemove)
        {
            M_LogWDLPickupEvent(player, special, WDL_PICKUP_PLASMAGUN, special->flags & MF_DROPPED);
        }
        break;
    ...
    case SPR_SHOT:
        ...
        if (val == IEV_EquipStay || val == IEV_EquipRemove)
        {
            M_LogWDLPickupEvent(player, special, WDL_PICKUP_SHOTGUN, special->flags & MF_DROPPED);
        }
        break;
    ...
    case SPR_SGN2:
        ...
        if (val == IEV_EquipStay || val == IEV_EquipRemove)
        {
            M_LogWDLPickupEvent(player, special, WDL_PICKUP_SUPERSHOTGUN, special->flags & MF_DROPPED);
        }
        break;
...
void P_TouchSpecialThing(AActor *special, AActor *toucher)
{
    ...
    if (special->special)
    {
        LineSpecials[special->special](NULL, toucher, special->args[0], special->args[1], special->args[2],
                                       special->args[3], special->args[4]);
        special->special = 0;
    }
...
static void ClientObituary(AActor *self, AActor *inflictor, AActor *attacker)
{
    ...
    SexMessage(GStrings(OB_DEFAULT), gendermessage, gender, self->player->userinfo.netname.c_str(),
               self->player->userinfo.netname.c_str());
    SV_BroadcastPrintf(PRINT_OBITUARY, "%s\n", gendermessage);
...
void P_KillMobj(AActor *source, AActor *target, AActor *inflictor, bool joinkill)
{
    ...
    {
        LineSpecials[target->special](NULL, source, target->args[0], target->args[1], target->args[2], target->args[3],
                                      target->args[4]);
        target->special = 0;
    }
    ...
    if (source && source->player)
    {
        ...
        if (target->player && level.time)
        {
            if (!joinkill)
            {
                ...
                else if (G_IsTeamGame() && (splayer->userinfo.team == tplayer->userinfo.team))
                {
        ...
        if (G_IsCoopGame() && ((target->flags & MF_COUNTKILL) || (target->type == MT_SKULL)))
        {
...
void P_DamageMobj(AActor *target, AActor *inflictor, AActor *source, int damage, int mod, int flags)
{
    ...
    if (player)
    {
        ...
        {
            if ((target->subsector->sector->special & 255) == special && damage >= target->health)
            {
        ...
        {
            if (G_IsCoopGame() || (G_IsTeamGame() && player->userinfo.team == source->player->userinfo.team))
            {
                damage = 0;
            }
        }
        ...
        {
            M_LogActorWDLEvent(WDL_EVENT_DAMAGE, source, target, actualdamage, armorDamage, mod, 0);
        }
        ...
        case MOD_CHAINGUN:
        case MOD_RAILGUN:
            M_LogWDLEvent(WDL_EVENT_SSACCURACY, splayer, player, sangle, mod, 1, GetMaxShotsForMod(mod));
            break;
        case MOD_SHOTGUN:
        case MOD_SSHOTGUN:
            M_LogWDLEvent(WDL_EVENT_SPREADACCURACY, splayer, player, sangle, mod, 1, GetMaxShotsForMod(mod));
            break;
        case MOD_ROCKET:
        case MOD_R_SPLASH:
        case MOD_BFG_BOOM:
        case MOD_PLASMARIFLE:
            M_LogWDLEvent(WDL_EVENT_PROJACCURACY, splayer, player, sangle, mod, 1, GetMaxShotsForMod(mod));
            break;
        case MOD_BFG_SPLASH:
            M_LogWDLEvent(WDL_EVENT_TRACERACCURACY, splayer, player, sangle, mod, 1, GetMaxShotsForMod(mod));
            break;
        }
    ...
    if (!(target->flags2 & MF2_DORMANT))
    {
        ...
        if (pain < target->info->painchance && !(target->flags & MF_SKULLFLY) && !(player && !damage))
        {
        ...
        if (source && source != target && !(source->flags3 & MF3_DMGIGNORED) && !(source->oflags & MFO_INFIGHTINVUL) &&
            (!target->threshold || target->flags3 & MF3_NOTHRESHOLD) && !P_InfightingImmune(target, source))
        {
...
VERSION_CONTROL(p_interaction_cpp, "$Id: 22b0575c5ba83e9d49686ec129c66351daf303fe $")
