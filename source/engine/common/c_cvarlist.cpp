...
//-----------------------------------------------------------------------------
//
// $Id: 312c042dc08f640450626f832ec9829fb0172bd8 $
//
...
CVAR_RANGE(sv_scorelimit, "5", "Game ends when team score is reached in Teamplay/CTF", CVARTYPE_BYTE,
           CVAR_SERVERARCHIVE | CVAR_SERVERINFO | CVAR_NOENABLEDISABLE, 0.0f, 255.0f)

CVAR(sv_teamspawns, "1",
     "When disabled, treat team spawns like normal deathmatch "
     "spawns in Teamplay/CTF",
     CVARTYPE_BOOL, CVAR_SERVERARCHIVE | CVAR_LATCH | CVAR_SERVERINFO)

CVAR(sv_playerbeacons, "0",
     "When enabled, print player beacons in WDL events."
     "This will take effect on map change.",
     CVARTYPE_BOOL, CVAR_SERVERARCHIVE | CVAR_LATCH | CVAR_SERVERINFO)

CVAR(sv_allowcheats, "0", "Allow usage of cheats in all game modes", CVARTYPE_BOOL,
...
CVAR_RANGE(sv_weapondamage, "1.0", "Amount to multiply player weapon damage by", CVARTYPE_FLOAT,
           CVAR_SERVERARCHIVE | CVAR_SERVERINFO | CVAR_LATCH | CVAR_NOENABLEDISABLE, 0.0f, 100.0f)
...
CVAR_RANGE(sv_fraglimit, "0", "Sets the amount of frags a player can accumulate before the game ends", CVARTYPE_WORD,
           CVAR_SERVERARCHIVE | CVAR_SERVERINFO | CVAR_NOENABLEDISABLE, 0.0f, 65536.0f)

CVAR(sv_fastmonsters, "0", "Monsters are at nightmare speed", CVARTYPE_BOOL, CVAR_SERVERARCHIVE | CVAR_SERVERINFO)

CVAR_RANGE(sv_monsterdamage, "1.0", "Amount to multiply monster weapon damage by", CVARTYPE_FLOAT,
           CVAR_SERVERARCHIVE | CVAR_SERVERINFO | CVAR_LATCH | CVAR_NOENABLEDISABLE, 0.0f, 100.0f)
...
CVAR(sv_fragexitswitch, "0", "When enabled, exit switch will kill the player who flips it", CVARTYPE_BOOL,
     CVAR_SERVERARCHIVE | CVAR_SERVERINFO)

CVAR(sv_infiniteammo, "0", "Infinite ammo for all players", CVARTYPE_BOOL, CVAR_SERVERARCHIVE | CVAR_SERVERINFO)

CVAR(sv_itemsrespawn, "0", "Items will respawn after a fixed period, see sv_itemrespawntime", CVARTYPE_BOOL,
     CVAR_SERVERARCHIVE | CVAR_SERVERINFO | CVAR_LATCH)

CVAR(sv_respawnsuper, "0",
     "Allows Invisibility/Invulnerability spheres from respawning (need sv_itemsrespawn set to 1)", CVARTYPE_BOOL,
     CVAR_SERVERARCHIVE | CVAR_SERVERINFO | CVAR_LATCH)

CVAR_RANGE(sv_itemrespawntime, "30",
           "If sv_itemsrespawn is set, items will respawn after this "
           "time (in seconds)",
           CVARTYPE_WORD, CVAR_SERVERARCHIVE | CVAR_SERVERINFO | CVAR_NOENABLEDISABLE, 0.0f, 500.0f)

CVAR(sv_monstersrespawn, "0", "Monsters will respawn after a period of time", CVARTYPE_BOOL,
     CVAR_SERVERARCHIVE | CVAR_SERVERINFO)

CVAR(sv_nomonsters, "0", "No monsters will be present", CVARTYPE_BOOL,
     CVAR_SERVERARCHIVE | CVAR_SERVERINFO | CVAR_LATCH)

CVAR_RANGE(sv_monstershealth, "1.0", "Amount to multiply monster health by", CVARTYPE_FLOAT,
           CVAR_SERVERARCHIVE | CVAR_SERVERINFO | CVAR_LATCH | CVAR_NOENABLEDISABLE, 0.0f, 100.0f)

CVAR_RANGE(sv_skill, "3",
...
CVAR_RANGE(sv_timelimit, "0", "Sets the time limit for the game to end (in minutes)", CVARTYPE_FLOAT,
           CVAR_SERVERARCHIVE | CVAR_SERVERINFO | CVAR_NOENABLEDISABLE, 0.0f, 1440.0f)
...
CVAR(sv_weaponstay, "1", "Weapons stay after pickup", CVARTYPE_BOOL, CVAR_SERVERARCHIVE | CVAR_SERVERINFO | CVAR_LATCH)

CVAR(sv_keepkeys, "0", "Keep keys on death", CVARTYPE_BOOL, CVAR_SERVERARCHIVE | CVAR_SERVERINFO)

CVAR_FUNC_DECL(sv_sharekeys, "0", "Share keys found to every player.", CVARTYPE_BOOL,
               CVAR_SERVERARCHIVE | CVAR_SERVERINFO)
...
CVAR(sv_allowmovebob, "1", "Allow weapon & view bob changing", CVARTYPE_BOOL, CVAR_SERVERARCHIVE | CVAR_SERVERINFO)

CVAR(sv_allowredscreen, "1", "Allow clients to adjust amount of red pain screen intensity", CVARTYPE_BOOL,
     CVAR_SERVERINFO | CVAR_SERVERARCHIVE)

CVAR(sv_allowpwo, "0", "Allow clients to set their preferences for automatic weapon switching", CVARTYPE_BOOL,
     CVAR_SERVERINFO | CVAR_SERVERARCHIVE)

CVAR_FUNC_DECL(sv_allowwidescreen, "1", "Allow clients to use true widescreen", CVARTYPE_BOOL,
               CVAR_SERVERINFO | CVAR_SERVERARCHIVE | CVAR_LATCH)

CVAR(sv_allowshowspawns, "1", "Allow clients to see spawn points as particle fountains", CVARTYPE_BOOL,
     CVAR_SERVERINFO | CVAR_SERVERARCHIVE | CVAR_LATCH)

CVAR(sv_forcerespawn, "0", "Force a player to respawn", CVARTYPE_BOOL, CVAR_SERVERARCHIVE | CVAR_SERVERINFO)
...
CVAR_RANGE(sv_spawndelaytime, "0.0", "Force a player to wait a period (in seconds) before respawning", CVARTYPE_FLOAT,
           CVAR_SERVERARCHIVE | CVAR_SERVERINFO | CVAR_NOENABLEDISABLE, 0.0f, 32768.0f)

CVAR(sv_unblockplayers, "0",
...
CVAR(g_ctf_notouchreturn, "0", "Prevents touch-return of the flag, forcing the player to wait for it to timeout",
...
CVAR(g_roundlimit, "0", "Number of total rounds the game can go on for before the game ends", CVARTYPE_INT,
     CVAR_SERVERARCHIVE | CVAR_SERVERINFO | CVAR_NOENABLEDISABLE)

CVAR(g_lives, "0",
     "Number of lives a player has before they can no longer respawn - this can result "
     "in a game loss depending on the gametype",
     CVARTYPE_INT, CVAR_SERVERARCHIVE | CVAR_SERVERINFO | CVAR_LATCH | CVAR_NOENABLEDISABLE)
...
CVAR(g_winlimit, "0", "Number of times a round must be won before a player or team wins the game", CVARTYPE_INT,
     CVAR_SERVERARCHIVE | CVAR_SERVERINFO | CVAR_NOENABLEDISABLE)

CVAR(g_winnerstays, "0", "After a match winners stay in the game, losers get spectated.", CVARTYPE_BOOL,
     CVAR_SERVERARCHIVE | CVAR_SERVERINFO | CVAR_LATCH)

CVAR(g_preroundtime, "5", "Amount of time before a round where you can't shoot", CVARTYPE_INT,
     CVAR_SERVERARCHIVE | CVAR_NOENABLEDISABLE)

CVAR(g_preroundreset, "0", "After preround is over, reset the map one last time.", CVARTYPE_INT, CVAR_SERVERARCHIVE)

CVAR(g_postroundtime, "3", "Amount of time after a round before the next round/endgame", CVARTYPE_INT,
     CVAR_SERVERARCHIVE | CVAR_NOENABLEDISABLE)

CVAR_RANGE(g_thingfilter, "0",
           "Removes some things from the map. Values are:\n"
           ...
           CVARTYPE_BYTE, CVAR_SERVERARCHIVE | CVAR_SERVERINFO | CVAR_NOENABLEDISABLE | CVAR_LATCH, 0.0f, 3.0f)
...
CVAR(cl_deathcam, "1", "Dead player's view follows the actor who killed them", CVARTYPE_BOOL, CVAR_CLIENTARCHIVE)

CVAR_RANGE(cl_predictsectors, "1",
...
CVAR_RANGE_FUNC_DECL(sv_gravity, "800", "Gravity of the environment", CVARTYPE_WORD,
                     CVAR_ARCHIVE | CVAR_SERVERINFO | CVAR_NOENABLEDISABLE, 0.0f, 32768.0f)

CVAR_RANGE_FUNC_DECL(sv_aircontrol, "0.00390625", "How much control the player has in the air", CVARTYPE_FLOAT,
                     CVAR_ARCHIVE | CVAR_SERVERINFO | CVAR_NOENABLEDISABLE, 0.0f, 32768.0f)

CVAR_RANGE_FUNC_DECL(sv_splashfactor, "1.0", "Rocket explosion thrust effect?", CVARTYPE_FLOAT,
                     CVAR_ARCHIVE | CVAR_SERVERINFO | CVAR_NOENABLEDISABLE, 0.01f, 100.0f)

CVAR(cl_waddownloaddir, "", "Set custom WAD download directory", CVARTYPE_STRING,
     CVAR_CLIENTARCHIVE | CVAR_NOENABLEDISABLE)

CVAR(r_softinvulneffect, "1", "Change invuln to enable light googles and invert the pallete on the weapon sprite only.",
     CVARTYPE_BOOL, CVAR_CLIENTARCHIVE)

// Misc stuff
// ----------

CVAR(developer, "0", "Debugging mode", CVARTYPE_BOOL, CVAR_NULL)

CVAR(debug_disconnect, "0", "Show source file:line where a disconnect happens", CVARTYPE_BOOL, CVAR_CLIENTARCHIVE)

CVAR_FUNC_DECL(language, "auto", "Language to use for ingame strings", CVARTYPE_STRING,
               CVAR_ARCHIVE | CVAR_NOENABLEDISABLE)

CVAR(port, "0", "Display currently used network port number", CVARTYPE_INT, CVAR_NOSET | CVAR_NOENABLEDISABLE)

CVAR_RANGE(chase_height, "-8", "Height of chase camera", CVARTYPE_WORD, CVAR_CLIENTARCHIVE | CVAR_NOENABLEDISABLE,
           -32768.0f, 32768.0f)

CVAR_RANGE(chase_dist, "90", "Chase camera distance", CVARTYPE_WORD, CVAR_CLIENTARCHIVE | CVAR_NOENABLEDISABLE,
           -32768.0f, 32768.0f)

CVAR(lookspring, "1", "Generate centerview when mlook encountered", CVARTYPE_BOOL, CVAR_CLIENTARCHIVE)

CVAR(waddirs, "", "Allow custom WAD directories to be specified", CVARTYPE_STRING, CVAR_ARCHIVE | CVAR_NOENABLEDISABLE)

CVAR_RANGE_FUNC_DECL(net_rcvbuf, "131072", "Net receive buffer size in bytes", CVARTYPE_INT,
                     CVAR_ARCHIVE | CVAR_NOENABLEDISABLE, 1500.0f, 256.0f * 1024.0f * 1024.0f)

CVAR_RANGE_FUNC_DECL(net_sndbuf, "131072", "Net send buffer size in bytes", CVARTYPE_INT,
                     CVAR_ARCHIVE | CVAR_NOENABLEDISABLE, 1500.0f, 256.0f * 1024.0f * 1024.0f)

// Experimental settings (all categories)
// =======================================

CVAR(sv_weapondrop, "0", "Enable/disable weapon drop.", CVARTYPE_BOOL, CVAR_SERVERARCHIVE | CVAR_SERVERINFO)

VERSION_CONTROL(c_cvarlist_cpp, "$Id: 312c042dc08f640450626f832ec9829fb0172bd8 $")
