...
template <> void MustGet<float>(OScanner &os)
{
    os.mustScanFloat();
}

// ensure token is bool
template <> void MustGet<bool>(OScanner &os)
{
    os.mustScanBool();
}

// ensure token is std::string
template <> void MustGet<std::string>(OScanner &os)
{
    os.mustScan();
}

// ensure token is OLumpName
template <> void MustGet<OLumpName>(OScanner &os)
{
    os.mustScan();

    if (os.getToken().length() > 8)
    {
        os.error("Lump name \"%s\" too long. Maximum size is 8 characters.", os.getToken().c_str());
    }
}
...
{
    return os.compareTokenNoCase("map") || os.compareTokenNoCase("defaultmap") || os.compareTokenNoCase("cluster") ||
...
void ParseUMapInfoFile(const char *filename)
{
    ...
    while (os.scan())
    {
        ...
        // Find the level.
        level_pwad_info_t &info = (levels.findByName(mapname).exists()) ? levels.findByName(mapname) : levels.create();
...
template <typename T> void ParseMapInfoHelper(OScanner &os, bool doEquals)
{
    if (doEquals)
        MustGetStringName(os, "=");
...
template <> void ParseMapInfoHelper<void>(OScanner &os, bool doEquals)
{
    // do nothing
}
...
void MIType_BaronSpecial(OScanner &os, bool doEquals, void *data, unsigned int flags, unsigned int flags2)
{
    ...
    for (std::vector<bossaction_t>::iterator it = bossactionvector.begin(); it != bossactionvector.end(); ++it)
    {
        it->type = MT_BRUISER;
    }
}
...
// blocks)
template <typename T> struct MapInfoDataSetter
{
...
// level_pwad_info_t
template <> struct MapInfoDataSetter<level_pwad_info_t>
{
    MapInfoDataContainer mapInfoDataContainer;
...
// cluster_info_t
template <> struct MapInfoDataSetter<cluster_info_t>
{
    MapInfoDataContainer mapInfoDataContainer;
...
// gameinfo_t
template <> struct MapInfoDataSetter<gameinfo_t>
{
    MapInfoDataContainer mapInfoDataContainer;
...
//
template <typename T> void ParseMapInfoLower(OScanner &os, MapInfoDataSetter<T> &mapInfoDataSetter)
{
...
// SkillInfo
template <> struct MapInfoDataSetter<SkillInfo>
{
    MapInfoDataContainer mapInfoDataContainer;
    ...
    MapInfoDataSetter(SkillInfo &ref)
    {
        ...
        // ENTRY3("playerclassname", &???, &ref.menu_names_for_player_class) // todo - requires special MIType to work
        // properly
    ...
    while (os.scan())
    {
        ...
        else if (os.compareTokenNoCase("map"))
        {
            ...
            level_pwad_info_t &info =
                (levels.findByName(map_name).exists()) ? levels.findByName(map_name) : levels.create();
