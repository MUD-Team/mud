...
//-----------------------------------------------------------------------------
//
// $Id: 96cdfad7f2e4da98d7d50996a95f6421c88a17f2 $
//
...
{
    return ::levelstate.getState() == LevelState::INGAME || ::levelstate.getState() == LevelState::WARMUP;
}
...
{
    return ::g_lives > 0 && ::levelstate.getState() == LevelState::INGAME && ::levelstate.getJoinTimeLeft() > 0;
}
...
void G_AssertValidPlayerCount()
{
    ...
    // check is useless.
    if (::levelstate.getState() == LevelState::WARMUP || ::levelstate.getState() == LevelState::ENDGAME_COUNTDOWN)
        return;
...
void G_TimeCheckEndGame()
{
    ...
    if (sv_gametype == GM_DM)
    {
        if (G_IsLivesGame() && ::sv_fraglimit.asInt() == 0)
        {
            ...
            {
                SV_BroadcastPrintf("Time limit hit. Game is a draw on tied lives left!\n");
    ...
    else if (G_IsTeamGame())
    {
        if (G_IsSidesGame())
        {
            ...
            SV_BroadcastPrintf("Time limit hit. %s team wins on successful defense!\n", ti.ColorizedTeamName().c_str());
        ...
        else
        {
            ...
            if (G_IsLivesGame() && ((!G_UsesScorelimit() && ::sv_fraglimit.asInt() == 0) ||
...
bool G_RoundsShouldEndGame()
{
    ...
    if (G_IsCoopGame())
    {
        ...
        {
            SV_BroadcastPrintf("Round limit hit. Players were unable to finish the level.\n");
            ::levelstate.setWinner(WinInfo::WIN_NOBODY, 0);
            return true;
        }
    }
    ...
    else if (G_IsTeamGame())
    {
        ...
        else if (tv.size() == 1 && ::g_roundlimit && ::levelstate.getRound() >= ::g_roundlimit)
        {
