...
//-----------------------------------------------------------------------------
//
// $Id: 70e1dea4672b1651065b061bab769a885ded5ec3 $
//
...
class PixelFormat
{
  public:
    inline PixelFormat()
        : mBitsPerPixel(0), mAShift(0), mRShift(0), mGShift(0), mBShift(0), mALoss(0), mRLoss(0), mGLoss(0), mBLoss(0)
    {
    }

    inline PixelFormat(uint8_t bitsperpixel, uint8_t abits, uint8_t rbits, uint8_t gbits, uint8_t bbits, uint8_t ashift,
                       uint8_t rshift, uint8_t gshift, uint8_t bshift)
        : mBitsPerPixel(bitsperpixel), mAShift(ashift), mRShift(rshift), mGShift(gshift), mBShift(bshift),
    ...
    {
        return ((0xFF >> mALoss) << mAShift) | ((r >> mRLoss) << mRShift) | ((g >> mGLoss) << mGShift) |
               ((b >> mBLoss) << mBShift);
    }

    inline uint32_t convert(const uint8_t a, const uint8_t r, const uint8_t g, const uint8_t b) const
    {
        return ((a >> mALoss) << mAShift) | ((r >> mRLoss) << mRShift) | ((g >> mGLoss) << mGShift) |
               ((b >> mBLoss) << mBShift);
    }
...
static inline void V_Convert(void *dest, const PixelFormat *destformat, uint16_t destpitch, void *source,
                             const PixelFormat *source_format, uint16_t sourcepitch, uint16_t width, uint16_t height)
{
}
