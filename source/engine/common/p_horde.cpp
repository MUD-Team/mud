...
//-----------------------------------------------------------------------------
//
// $Id: 38487e57887666eca58a072bd0b17adebbf7b785 $
//
...
class HordeState
{
  ...
  public:
    HordeState()
        : m_state(HS_STARTING), m_wave(0), m_waveTime(0), m_bossTime(0), m_defineID(0), m_spawnedHealth(0),
          m_killedHealth(0), m_bossHealth(0), m_bossDamage(0), m_waveStartHealth(0), m_bossRecipe(hordeRecipe_t()),
          m_nextSpawn(0), m_nextPowerup(0)
    {
    }
    ...
    void reset()
    {
        ...
        SV_BroadcastPrintf("Wave %d: \"%s\"\n", m_wave, G_HordeDefine(m_defineID).name.c_str());
    }
        ...
        if (G_IsLivesGame())
        {
            ...
            for (PlayersView::iterator it = ingame.begin(); it != ingame.end(); ++it)
            {
                ...
                if ((*it)->lives <= 0)
                {
                    ...
                    S_PlayerSound(*it, NULL, CHAN_INTERFACE, "misc/plraise", ATTN_NONE);
                }
                ...
                if ((*it)->lives < g_lives)
                {
                    ...
                    MSG_BroadcastSVC(CLBUF_RELIABLE, SVC_PlayerMembers(**it, SVC_PM_LIVES), (*it)->id);
                }
            }
...
void HordeState::getNextSpawnTime(int32_t &min, int32_t &max)
{
    ...
    double minf = Remap(alive, define.minTotalHealth(), define.maxTotalHealth(), EMPTY_MIN_SPAWN, EMPTY_MAX_SPAWN);
...
void P_RunHordeTics()
{
    ...
    if (!P_HordeHasSpawns())
    {
        ...
        if (!P_HordeHasSpawns())
        {
            if (::level.time == 0)
            {
                Printf(PRINT_WARNING, "WARNING: This map is missing Horde Monster, Horde Supply Cache, "
...
BEGIN_COMMAND(hordeinfo)
{
    ...
    Printf("Max Group Health: %d (Difficulty: %s)\n", define.maxGroupHealth, define.difficulty(false));
    Printf("Min Total Health: %d = waveMaxGroup:%d * g_horde_mintotalhp:%s * "
           "skillLevel:%0.2f\n",
           define.minTotalHealth(), define.maxGroupHealth, ::g_horde_mintotalhp.cstring(), skillScaler);
    Printf("Max Total Health: %d = waveMaxGroup:%d * g_horde_maxtotalhp:%s * "
           "skillLevel:%0.2f\n",
           define.maxTotalHealth(), define.maxGroupHealth, ::g_horde_maxtotalhp.cstring(), skillScaler);
    ...
    Printf("Empty/Full Spawn Rate: %d-%dsec, %d-%dsec\n", g_horde_spawnempty_min.asInt(),
           g_horde_spawnempty_max.asInt(), g_horde_spawnfull_min.asInt(), g_horde_spawnfull_max.asInt());
