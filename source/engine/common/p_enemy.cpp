...
//-----------------------------------------------------------------------------
//
// $Id: ce563b10d0d25c86baf3b61ed6a4ddac07f8188e $
//
...
dirtype_t opposite[9] = {DI_WEST,      DI_SOUTHWEST, DI_SOUTH,     DI_SOUTHEAST, DI_EAST,
                         DI_NORTHEAST, DI_NORTH,     DI_NORTHWEST, DI_NODIR};

dirtype_t diags[4] = {DI_NORTHWEST, DI_NORTHEAST, DI_SOUTHWEST, DI_SOUTHEAST};
...
void P_RecursiveSound(sector_t *sec, int soundblocks, AActor *soundtarget)
{
    ...
    for (i = 0; i < sec->linecount; i++)
    {
        ...
        // demo desyncs.
        P_LineOpening(check, (check->v1->x >> 1) + (check->v2->x >> 1), (check->v1->y >> 1) + (check->v2->y >> 1));
...
static bool P_CheckRange(AActor *actor, fixed_t range)
{
    ...
    return // killough 7/18/98: friendly monsters don't attack other friends
        pl && !(actor->flags & pl->flags & MF_FRIEND) && P_AproxDistance(pl->x - actor->x, pl->y - actor->y) < range &&
        P_CheckSight(actor, actor->target) && pl->z <= actor->z + actor->height && actor->z <= pl->z + pl->height;
}
...
BOOL P_Move(AActor *actor)
{
    ...
    if (!try_ok)
    {
        ...
        while (!spechit.empty())
        {
            ...
            // that can be opened,
            // return false
            if (P_UseSpecialLine(actor, ld, 0, false) || P_PushSpecialLine(actor, ld, 0))
                good = true;
        }
        return good;
    }
...
void P_NewChaseDir(AActor *actor)
{
    ...
    if (monster_backing)
    {
        ...
        if (actor->flags & actor->target->flags & MF_FRIEND && distfriend << FRACBITS > dist && !P_IsOnLift(actor) &&
            !P_IsUnderDamage(actor))
        {
            deltax = -deltax, deltay = -deltay;
        }
        else if (actor->target->health > 0 && (actor->flags ^ actor->target->flags) & MF_FRIEND)
        { // Live enemy target
            if (actor->info->missilestate && actor->type != MT_SKULL &&
                ((!actor->target->info->missilestate && dist < actor->target->info->meleerange * 2) ||
                 (actor->target->player && dist < actor->target->player->mo->info->meleerange * 3 &&
...
bool P_LookForPlayers(AActor *actor, bool allaround)
{
    ...
    for (;; actor->lastlook = (actor->lastlook + 1) % maxid)
    {
        ...
        if (!allaround)
        {
            angle_t an = P_PointToAngle(actor->x, actor->y, player->mo->x, player->mo->y) - actor->angle;
            if (an > ANG90 && an < ANG270)
            {
                fixed_t dist = P_AproxDistance(player->mo->x - actor->x, player->mo->y - actor->y);
    ...
    if (!actor->target || !(actor->target->flags & MF_SHOOTABLE))
    {
        ...
        if (!actor->target)
        {
            P_SetMobjState(actor, actor->info->spawnstate,
                           true); // denis - todo - this sometimes leads to a stack overflow due to infinite recursion:
...
void A_Tracer(AActor *actor)
{
    ...
    if (serverside)
    {
        ...
        AActor *th = new AActor(actor->x - actor->momx, actor->y - actor->momy, actor->z, MT_SMOKE);
    ...
    // change angle
    angle_t exact = P_PointToAngle(actor->x, actor->y, dest->x, dest->y);
...
void A_Fire(AActor *actor)
{
    ...
    actor->SetOrigin(dest->x + FixedMul(24 * FRACUNIT, finecosine[an]), dest->y + FixedMul(24 * FRACUNIT, finesine[an]),
                     dest->z);
}
...
void A_VileTarget(AActor *actor)
{
    ...
    fog = new AActor(actor->target->x, actor->target->x, actor->target->z, MT_FIRE);
...
void A_Scream(AActor *actor)
{
    ...
    if (stricmp(sound, "grunt/death1") == 0 || stricmp(sound, "shotguy/death1") == 0 ||
        stricmp(sound, "chainguy/death1") == 0)
    {
        sound[strlen(sound) - 1] = P_Random(actor) % 3 + '1';
    }

    if (stricmp(sound, "imp/death1") == 0 || stricmp(sound, "imp/death2") == 0)
    {
...
void P_SpawnBrainTargets(void) // killough 3/26/98: renamed old function
{
    ...
    while ((other = iterator.Next()))
    {
        ...
        { // killough 2/7/98: remove limit on icon landings:
            ...
            {
                braintargets = (AActor **)Realloc(
                    braintargets, (numbraintargets_alloc = numbraintargets_alloc ? numbraintargets_alloc * 2 : 32) *
                                      sizeof *braintargets);
            }
...
void A_PlaySound(AActor *mo)
{
    ...
    S_Sound(mo, CHAN_BODY, SoundMap[sndmap], 1, (mo->state->misc2 ? ATTN_NONE : ATTN_NORM));
}
...
VERSION_CONTROL(p_enemy_cpp, "$Id: ce563b10d0d25c86baf3b61ed6a4ddac07f8188e $")
