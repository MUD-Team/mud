...
//-----------------------------------------------------------------------------
//
// $Id: 303184ed22df0669b2b5970a2ce76e1ec9cbea15 $
//
...
DLighting::DLighting(sector_t *sector) : DSectorEffect(sector)
{
}
...
// P_SpawnFireFlicker
//
DFireFlicker::DFireFlicker(sector_t *sector) : DLighting(sector)
{
...
// P_SpawnLightFlash
//
DLightFlash::DLightFlash(sector_t *sector) : DLighting(sector)
{
...
DGlow::DGlow(sector_t *sector) : DLighting(sector)
{
    m_MinLight  = P_FindMinSurroundingLight(sector, sector->lightlevel);
    m_MaxLight  = sector->lightlevel;
    m_Direction = -1;
...
void DPhased::RunThink()
{
    ...
    if (m_Phase < steps)
        ...
        m_Sector->lightlevel = ((255 - m_BaseLevel) * (2 * steps - m_Phase - 1) / steps + m_BaseLevel);
    else
...
DPhased::DPhased(sector_t *sector) : DLighting(sector)
{
...
VERSION_CONTROL(p_lights_cpp, "$Id: 303184ed22df0669b2b5970a2ce76e1ec9cbea15 $")
