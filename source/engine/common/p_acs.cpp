...
static DoomEntity DoomMonsterNames[NUMMONSTERS] = {
    {"ZombieMan", MT_POSSESSED},     {"ShotgunGuy", MT_SHOTGUY},  {"ChaingunGuy", MT_CHAINGUY},
    {"DoomImp", MT_TROOP},           {"Demon", MT_SERGEANT},      {"Spectre", MT_SHADOWS},
    {"LostSoul", MT_SKULL},          {"Cacodemon", MT_HEAD},      {"HellKnight", MT_KNIGHT},
    {"BaronOfHell", MT_BRUISER},     {"Arachnotron", MT_BABY},    {"PainElemental", MT_PAIN},
    {"Revenant", MT_UNDEAD},         {"Fatso", MT_FATSO},         {"Archvile", MT_VILE},
    {"SpiderMastermind", MT_SPIDER}, {"Cyberdemon", MT_CYBORG},   {"CommanderKeen", MT_KEEN},
    {"WolfensteinSS", MT_WOLFSS},    {"BossBrain", MT_BOSSBRAIN}, {"BossTarget", MT_BOSSTARGET},
    {"BossEye", MT_BOSSSPIT}};

static DoomEntity DoomAmmoNames[8] = {{"Clip", MT_CLIP},         {"Shell", MT_MISC22},   {"Cell", MT_MISC20},
                                      {"RocketAmmo", MT_MISC18}, {"ClipBox", MT_MISC17}, {"ShellBox", MT_MISC23},
                                      {"RocketBox", MT_MISC19},  {"CellPack", MT_MISC21}};

static DoomEntity DoomWeaponNames[9] = {{"Fist", (mobjtype_t)NULL}, // Default weapon, no entity
...
static DoomEntity DoomKeyNames[6] = {{"BlueCard", MT_MISC4},  {"YellowCard", MT_MISC6},  {"RedCard", MT_MISC5},
                                     {"BlueSkull", MT_MISC9}, {"YellowSkull", MT_MISC7}, {"RedSkull", MT_MISC8}};
...
class DFlashFader : public DThinker
{
  ...
  public:
    DFlashFader(float r1, float g1, float b1, float a1, float r2, float g2, float b2, float a2, float time,
                AActor *who);
    ~DFlashFader();
...
void DFlashFader::SetBlend(float time)
{
    ...
    player->blend_color = fargb_t(Blends[0][3] * iT + Blends[1][3] * time, Blends[0][0] * iT + Blends[1][0] * time,
                                  Blends[0][1] * iT + Blends[1][1] * time, Blends[0][2] * iT + Blends[1][2] * time);
}
...
void DPlaneWatcher::Serialize(FArchive &arc)
{
    ...
    if (arc.IsStoring())
    {
        arc << Special << Arg0 << Arg1 << Arg2 << Arg3 << Arg4 << Sector << bCeiling << WatchD << LastD << Activator
...
void DPlaneWatcher::RunThink()
{
    ...
    if ((LastD < WatchD && newd >= WatchD) || (LastD > WatchD && newd <= WatchD))
    {
        ...
        if (serverside)
        {
            SERVER_ONLY(SV_SendExecuteLineSpecial(Special, Line, Activator, Arg0, Arg1, Arg2, Arg3, Arg4));
        }
        Destroy();
    }
}
...
void DLevelScript::Serialize(FArchive &arc)
{
    ...
    {
        arc >> next >> prev >> script >> sp >> state >> statedata >> activator >> activationline >> lineSide;
...
static void DoActualFadeRange(player_s *viewer, float ftime, bool fadingFrom, float fr1, float fg1, float fb1,
                              float fa1, float fr2, float fg2, float fb2, float fa2)
{
    if (ftime <= 0.f)
    {
...
void DLevelScript::RunScript()
{
    ...
    while (state == SCRIPT_Running)
    {
        ...
        case PCD_LSPEC1:
            ActivateLineSpecial(NEXTBYTE, activationline, activator, STACK(1), 0, 0, 0, 0);
            sp -= 1;
            break;

        case PCD_LSPEC2:
            ActivateLineSpecial(NEXTBYTE, activationline, activator, STACK(2), STACK(1), 0, 0, 0);
            sp -= 2;
            break;

        case PCD_LSPEC3:
            ActivateLineSpecial(NEXTBYTE, activationline, activator, STACK(3), STACK(2), STACK(1), 0, 0);
            sp -= 3;
            break;

        case PCD_LSPEC4:
            ActivateLineSpecial(NEXTBYTE, activationline, activator, STACK(4), STACK(3), STACK(2), STACK(1), 0);
            sp -= 4;
            break;

        case PCD_LSPEC5:
            ActivateLineSpecial(NEXTBYTE, activationline, activator, STACK(5), STACK(4), STACK(3), STACK(2), STACK(1));
            sp -= 5;
            break;

        case PCD_LSPEC1DIRECT:
            temp = NEXTBYTE;
            ActivateLineSpecial(temp, activationline, activator, pc[0], 0, 0, 0, 0);
            pc += 1;
            break;

        case PCD_LSPEC2DIRECT:
            temp = NEXTBYTE;
            ActivateLineSpecial(temp, activationline, activator, pc[0], pc[1], 0, 0, 0);
            pc += 2;
            break;

        case PCD_LSPEC3DIRECT:
            temp = NEXTBYTE;
            ActivateLineSpecial(temp, activationline, activator, pc[0], pc[1], pc[2], 0, 0);
            pc += 3;
            break;

        case PCD_LSPEC4DIRECT:
            temp = NEXTBYTE;
            ActivateLineSpecial(temp, activationline, activator, pc[0], pc[1], pc[2], pc[3], 0);
            pc += 4;
            break;

        case PCD_LSPEC5DIRECT:
            temp = NEXTBYTE;
            ActivateLineSpecial(temp, activationline, activator, pc[0], pc[1], pc[2], pc[3], pc[4]);
            pc += 5;
            break;
        ...
        case PCD_PRINTLOCALIZED:
            lookup = (pcd == PCD_PRINTSTRING ? level.behavior->LookupString(STACK(1))
        ...
        case PCD_PRINTNAME: {
            ...
            if (player)
            {
                workwhere += sprintf(workwhere, "%s", activator->player->userinfo.netname.c_str());
            }
            else if (activator)
            {
        ...
        */
        ...
        case PCD_FADERANGE:
            DoFadeRange(activator, STACK(9), STACK(8), STACK(7), STACK(6), STACK(5), STACK(4), STACK(3), STACK(2),
                        STACK(1));
            sp -= 9;
            break;
        ...
        */
        ...
        case PCD_SETFLOORTRIGGER:
            new DPlaneWatcher(activator, activationline, lineSide, false, STACK(8), STACK(7), STACK(6), STACK(5),
                              STACK(4), STACK(3), STACK(2), STACK(1));
            sp -= 8;
            break;

        case PCD_SETCEILINGTRIGGER:
            new DPlaneWatcher(activator, activationline, lineSide, true, STACK(8), STACK(7), STACK(6), STACK(5),
                              STACK(4), STACK(3), STACK(2), STACK(1));
            sp -= 8;
            break;
                   ...
                   int always)
{
    if (level.mapname == map)
    {
        ...
        {
            return P_GetScriptGoing(who, where, script, scriptdata, lineSide, arg0, arg1, arg2, always, false);
        }
        else
        {
    ...
    else
    {
        ...
        addDefered(levels.findByName(map), always ? acsdefered_t::defexealways : acsdefered_t::defexecute, script, arg0,
                   arg1, arg2, who);
    }
    return false;
}
...
void P_SuspendScript(int script, const char *map)
{
    ...
    {
        LevelInfos &levels = getLevelInfos();
        addDefered(levels.findByName(map), acsdefered_t::defsuspend, script, 0, 0, 0, NULL);
    }
    else
    {
...
void P_TerminateScript(int script, const char *map)
{
    ...
    {
        LevelInfos &levels = getLevelInfos();
        addDefered(levels.findByName(map), acsdefered_t::defterminate, script, 0, 0, 0, NULL);
    }
    else
    {
...
FArchive &operator>>(FArchive &arc, acsdefered_s *&defertop)
{
    ...
    while (inbyte)
    {
        *defer = new acsdefered_s;
        ...
        arc >> (*defer)->script >> (*defer)->arg0 >> (*defer)->arg1 >> (*defer)->arg2;
        defer = &((*defer)->next);
        arc >> inbyte;
    }
    *defer = NULL;
...
{
    static const char *stateNames[] = {"Running",  "Suspended",     "Delayed",    "TagWait",
                                       "PolyWait", "ScriptWaitPre", "ScriptWait", "PleaseRemove"};
    DLevelScript      *script       = Scripts;
...
VERSION_CONTROL(p_acs_cpp, "$Id: 672d3042faa472fce4edea1254c6518a7607cb76 $")
