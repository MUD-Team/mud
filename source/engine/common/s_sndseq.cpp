...
class DSeqActorNode : public DSeqNode
{
  ...
  public:
    ...
    void MakeSound()
    {
        S_SoundID(m_Actor, CHAN_BODY, m_CurrentSoundID, m_Volume, m_Atten);
    }
    void MakeLoopedSound()
    {
        S_LoopedSoundID(m_Actor, CHAN_BODY, m_CurrentSoundID, m_Volume, m_Atten);
    }
    bool IsPlaying()
    {
        return S_GetSoundPlayingInfo(m_Actor, m_CurrentSoundID);
    }
    void *Source()
    {
        return m_Actor;
...
class DSeqPolyNode : public DSeqNode
{
  ...
  public:
    ...
    void MakeSound()
    {
        S_SoundID(&m_Poly->startSpot[0], CHAN_BODY, m_CurrentSoundID, m_Volume, m_Atten);
    }
    void MakeLoopedSound()
    {
        S_LoopedSoundID(&m_Poly->startSpot[0], CHAN_BODY, m_CurrentSoundID, m_Volume, m_Atten);
    }
    bool IsPlaying()
    {
...
class DSeqSectorNode : public DSeqNode
{
  ...
  public:
    ...
    void MakeSound()
    {
        S_SoundID(&m_Sector->soundorg[0], CHAN_BODY, m_CurrentSoundID, m_Volume, m_Atten);
    }
    void MakeLoopedSound()
    {
        S_LoopedSoundID(&m_Sector->soundorg[0], CHAN_BODY, m_CurrentSoundID, m_Volume, m_Atten);
    }
    bool IsPlaying()
    {
...
static const char *SSStrings[]    = {"play",      "playuntildone", "playtime",     "playrepeat", "playloop",    "delay",
                                     "delayrand", "volume",        "end",          "stopsound",  "attenuation", "door",
                                     "platform",  "environment",   "nostopcutoff", NULL};
static const char *Attenuations[] = {"none", "normal", "idle", "static", "surround", NULL};
...
void DSeqNode::RunThink()
{
    ...
    case SS_CMD_DELAYRAND:
        m_DelayTics = GetData(*m_SequencePtr) + M_Random() % (*(m_SequencePtr + 1) - GetData(*m_SequencePtr));
        m_SequencePtr += 2;
        m_CurrentSoundID = -1;
        break;
...
VERSION_CONTROL(s_sndseq_cpp, "$Id: 06bde90b3f7fc7f2bf3f8f4d154b989331b80ac1 $")
