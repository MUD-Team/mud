...
{
    static std::unique_ptr<source_files_t> source_files(new source_files_t);
...
const char *NiceVersionDetails()
{
    ...
    else
    {
        ...
        StrFormat(version, "%s, g%s-%s%s", GitBranch(), GitShortHash(), GitRevCount(), debug);
    }
...
VERSION_CONTROL(version_cpp, "$Id: 62250214a4d5acf352604fb339b4519eff86bbe4 $")
