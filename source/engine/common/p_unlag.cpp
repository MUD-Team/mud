...
//-----------------------------------------------------------------------------
//
// $Id: 01242e704cb38d4e29a68e3c624facde0bdc8b3f $
//
...
Unlag::SectorHistoryRecord::SectorHistoryRecord()
    : sector(NULL), history_size(0), history_ceilingheight(), history_floorheight(), backup_ceilingheight(0),
      backup_floorheight(0)
{
}

Unlag::SectorHistoryRecord::SectorHistoryRecord(sector_t *sec)
    : sector(sec), history_size(Unlag::MAX_HISTORY_TICS), history_ceilingheight(), history_floorheight(),
...
void Unlag::moveSector(sector_t *sector, fixed_t ceilingheight, fixed_t floorheight)
{
...
void Unlag::reconcileSectorPositions(size_t ticsago)
{
    for (size_t i = 0; i < sector_history.size(); i++)
    {
        ...
        if (!reconciled)
        {
            ...
            size_t cur         = (sector_history[i].history_size - 1 - ticsago) % Unlag::MAX_HISTORY_TICS;
...
void Unlag::recordPlayerPositions()
{
    ...
    for (size_t i = 0; i < player_history.size(); i++)
    {
        ...
        if (player->playerstate == PST_LIVE && !player->spectator && player->mo)
        {
...
void Unlag::recordSectorPositions()
{
    ...
    for (size_t i = 0; i < sector_history.size(); i++)
    {
        ...
        size_t cur                                   = sector_history[i].history_size++ % Unlag::MAX_HISTORY_TICS;
...
void Unlag::debugReconciliation(uint8_t shooter_id)
{
    ...
    for (size_t i = 0; i < player_history.size(); i++)
    {
        ...
        for (size_t n = 0; n < MAX_HISTORY_TICS; n++)
        {
            ...
            angle_t deltaangle =
                angle - shooter->mo->angle < ANG180 ? angle - shooter->mo->angle : shooter->mo->angle - angle;

            if (deltaangle < 3 * FRACUNIT)
            {
                DPrintf("Unlag (%03d): would have hit player %d at gametic %d (%zu tics ago)\n", gametic & 0xFF,
                        player_history[i].player->id, (gametic - n) & 0xFF, n);
            }
        }
    }
}
