...
//-----------------------------------------------------------------------------
//
// $Id: 8667e3981752d80232c930fae5c51dfa91b251aa $
//
...
DDoor::DDoor() : m_Status(init), m_Line(NULL)
{
}
...
void DDoor::Serialize(FArchive &arc)
{
    ...
    if (arc.IsStoring())
    {
        arc << m_Type << m_Status << m_TopHeight << m_Speed << m_TopWait << m_TopCountdown << m_LightTag;
    }
    else
    {
        arc >> m_Type >> m_Status >> m_TopHeight >> m_Speed >> m_TopWait >> m_TopCountdown >> m_LightTag;
...
void DDoor::RunThink()
{
    ...
    case closing:
        ...
        if (m_LightTag)
        {
            EV_LightTurnOnPartway(m_LightTag, FixedDiv(ceilingheight - floorheight, m_TopHeight - floorheight));
        }
        if (res == pastdest)
        {
...
DDoor::DDoor(sector_t *sector) : DMovingCeiling(sector), m_Status(init), m_Line(NULL)
{
}
...
VERSION_CONTROL(p_doors_cpp, "$Id: 8667e3981752d80232c930fae5c51dfa91b251aa $")
