...
void R_ConvertPatch(patch_t *newpatch, patch_t *rawpatch, const unsigned int lump)
{
    ...
    for (int i = 0; i < rawpatch->width(); i++)
    {
        ...
        while (!rawpost->end())
        {
            ...
            if (length < 0)
            {
                I_Error("%s: Patch %s appears to be corrupted.", __FUNCTION__, W_LumpName(lump).c_str());
            }
...
void R_DrawColumnInCache(const tallpost_t *post, byte *cache, int originy, int cacheheight, byte *marks)
{
    while (!post->end())
    {
...
{
    byte *block = (byte *)Z_Malloc(texturecompositesize[texnum], PU_STATIC, (void **)&texturecomposite[texnum]);
...
void R_InitTextures(void)
{
    ...
    for (i = 0; i < numtextures; i++, directory++)
    {
        ...
        texture = textures[i] = (texture_t *)Z_Malloc(
            sizeof(texture_t) + sizeof(texpatch_t) * (SAFESHORT(mtexture->patchcount) - 1), PU_STATIC, 0);
...
void R_PrecacheLevel(void)
{
    ...
    {
        hitlist[sides[i].toptexture] = hitlist[sides[i].midtexture] = hitlist[sides[i].bottomtexture] = 1;
    }
