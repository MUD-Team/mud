...
//-----------------------------------------------------------------------------
//
// $Id: 43fdc348d290450b21b65a283058ff3707234f92 $
//
...
void DFloor::Serialize(FArchive &arc)
{
    ...
    if (arc.IsStoring())
    {
        arc << m_Type << m_Status << m_Crush << m_Direction << m_HexenCrush << m_NewSpecial << m_NewDamageRate
            << m_NewDmgInterval << m_NewFlags << m_NewLeakRate << m_Texture << m_FloorDestHeight << m_Speed
            << m_ResetCount << m_OrgHeight << m_Delay << m_PauseTime << m_StepTime << m_PerStepTime;
    }
    else
    {
        arc >> m_Type >> m_Status >> m_Crush >> m_Direction >> m_HexenCrush >> m_NewSpecial >> m_NewDamageRate >>
...
DFloor::DFloor(sector_t *sec) : DMovingFloor(sec), m_Status(init)
{
}
    ...
    : DMovingFloor(sec), m_Status(init)
{
    ...
    if (change & 3)
    {
        ...
        if (change & 4)
        {
            ...
            sector_t *found = NULL;
            if (floortype == DFloor::floorRaiseToLowestCeiling || floortype == DFloor::floorLowerToLowestCeiling ||
                floortype == DFloor::floorRaiseToCeiling || floortype == DFloor::floorLowerToCeiling)
            {
    ...
    : DMovingFloor(sec), m_Status(init)
{
    ...
    case FtoLnF:
        ...
        break;
    case FtoNnF:
        m_FloorDestHeight = direction ? P_FindNextHighestFloor(sec) : P_FindNextLowestFloor(sec);
        break;
    case FtoLnC:
        m_FloorDestHeight = P_FindLowestCeilingSurrounding(sec);
        break;
    ...
    case FbyST:
        m_FloorDestHeight =
            (sec->floorheight >> FRACBITS) + m_Direction * (P_FindShortestTextureAround(sec) >> FRACBITS);
        ...
        break;
    case Fby24:
        m_FloorDestHeight = sec->floorheight + m_Direction * 24 * FRACUNIT;
        break;
    case Fby32:
    ...
    if (change)    // if a texture change is indicated
    {
        if (model) // if a numeric model change
        {
            ...
            // is a ceiling type
            chgsec = (target == FtoLnC || target == FtoC) ? P_FindModelCeilingSector(m_FloorDestHeight, sec)
...
bool EV_DoGenFloor(line_t *line)
{
    ...
    sector_t *sec;
    DFloor   *floor;
    uint32_t  value = (uint32_t)line->special - GenFloorBase;
...
bool EV_DoGenStairs(line_t *line)
{
    ...
    uint32_t value = (uint32_t)line->special - GenStairsBase;
                    ...
                    int32_t reset, int32_t igntxt, int32_t usespecials)
{
    ...
    manual_stair:
        ...
        floor->m_Crush = (!usespecials && speed == 4 * FRACUNIT)
                             ? DOOM_CRUSH
                             : NO_CRUSH; // jff 2/27/98 fix uninitialized crush field
        ...
        do
        {
            ...
            if (usespecials)
            {
                ...
                tsec = P_NextSpecialSector(
                    sec, (sec->special & 0xff) == Stairs_Special1 ? Stairs_Special2 : Stairs_Special1, prev);
            ...
            if (ok)
            {
                ...
                {
                    // [RH]
                    fixed_t rise   = (floor->m_FloorDestHeight - floorheight) * floor->m_Direction;
                ...
                floor->m_Type = DFloor::buildStair; // jff 3/31/98 do not leave uninited
                // jff 2/27/98 fix uninitialized crush field
                floor->m_Crush      = (!usespecials && speed == 4 * FRACUNIT) ? DOOM_CRUSH : NO_CRUSH;
                floor->m_ResetCount = reset;       // [RH] Tics until reset (0 if never)
                floor->m_OrgHeight  = floorheight; // [RH] Height to reset to
            }
        } while (ok);
...
DElevator::DElevator() : m_Status(init)
{
}
...
void DElevator::Serialize(FArchive &arc)
{
    ...
    if (arc.IsStoring())
    {
        arc << m_Type << m_Status << m_Direction << m_FloorDestHeight << m_CeilingDestHeight << m_Speed;
    }
    else
    {
        arc >> m_Type >> m_Status >> m_Direction >> m_FloorDestHeight >> m_CeilingDestHeight >> m_Speed;
...
DElevator::DElevator(sector_t *sec) : Super(sec)
{
}
...
bool SpawnCommonElevator(line_t *line, DElevator::EElevator type, fixed_t speed, fixed_t height, int32_t tag)
{
    ...
    while ((secnum = P_FindSectorFromTag(tag, secnum)) >= 0)
    {
        ...
        case DElevator::elevateDown:
            ...
            elevator->m_CeilingDestHeight = elevator->m_FloorDestHeight + ceilingheight - floorheight;
            break;
...
static const fixed_t FloatBobOffsets[64] = {
    0,       51389,   102283,  152192,  200636,  247147,  291278,  332604,  370727,  405280,  435929,  462380,  484378,
    501712,  514213,  521763,  524287,  521763,  514213,  501712,  484378,  462380,  435929,  405280,  370727,  332604,
    291278,  247147,  200636,  152192,  102283,  51389,   -1,      -51390,  -102284, -152193, -200637, -247148, -291279,
...
*/
VERSION_CONTROL(p_floor_cpp, "$Id: 43fdc348d290450b21b65a283058ff3707234f92 $")
