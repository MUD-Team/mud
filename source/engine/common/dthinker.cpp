...
//-----------------------------------------------------------------------------
//
// $Id: b83e8a4ec1e0915e859c5ff9c93ad844da0d1202 $
//
...
void DThinker::DestroyMostThinkers()
{
    ...
    while (thinker)
    {
        ...
        if (!thinker->IsKindOf(RUNTIME_CLASS(AActor)) || static_cast<AActor *>(thinker)->player == NULL ||
...
bool IndependentThinker(DThinker *thinker)
{
    ...
    if (thinker->IsA(RUNTIME_CLASS(DPillar)) || thinker->IsA(RUNTIME_CLASS(DElevator)) ||
        thinker->IsA(RUNTIME_CLASS(DFloor)) || thinker->IsA(RUNTIME_CLASS(DCeiling)) ||
        thinker->IsA(RUNTIME_CLASS(DPlat)) || thinker->IsA(RUNTIME_CLASS(DDoor)))
    {
...
bool P_ThinkerIsPlayerType(DThinker *thinker)
{
    ...
    return thinker->IsKindOf(RUNTIME_CLASS(AActor)) && static_cast<AActor *>(thinker)->type == MT_PLAYER;
}

VERSION_CONTROL(dthinker_cpp, "$Id: b83e8a4ec1e0915e859c5ff9c93ad844da0d1202 $")
