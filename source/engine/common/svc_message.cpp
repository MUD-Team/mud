...
//-----------------------------------------------------------------------------
//
// $Id: 079fcf420bf14a4b83fc95f26c5eacc7337c8f30 $
//
...
odaproto::svc::KillMobj SVC_KillMobj(AActor *source, AActor *target, AActor *inflictor, int32_t mod, bool joinkill)
{
    ...
    if (target->player)
    {
        Unlag::getInstance().getReconciliationOffset(target->player->id, xoffs, yoffs, zoffs);
    }
...
odaproto::svc::MaplistUpdate SVC_MaplistUpdate(const maplist_status_t status, const maplist_qrows_t *maplist)
{
    ...
    if (status == MAPLIST_OUTDATED)
    {
        ...
        for (maplist_qrows_t::const_iterator it = maplist->begin(); it != maplist->end(); ++it)
        {
            ...
            for (std::vector<std::string>::iterator itr = it->second->wads.begin(); itr != it->second->wads.end();
                 ++itr)
            {
        ...
        for (OStringIndexer::Indexes::const_iterator it = indexer.indexes.begin(); it != indexer.indexes.end(); ++it)
        {
