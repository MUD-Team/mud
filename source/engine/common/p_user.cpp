...
//-----------------------------------------------------------------------------
//
// $Id: d9a4bb9651fd0b7463a8a766dbe54abb16229e14 $
//
...
PlayerResults PlayerQuery::execute()
{
    ...
    for (PlayersView::iterator it = results.players.begin(); it != results.players.end(); ++it)
    {
        results.count += 1;
...
void P_ForwardThrust(player_t *player, angle_t angle, fixed_t move)
{
    ...
    if ((player->mo->waterlevel || (player->mo->flags2 & MF2_FLY)) && player->mo->pitch != 0)
    {
...
void P_FallingDamage(AActor *ent)
{
    ...
    if ((ent->player->oldvelocity[2] < 0) && (ent->momz > ent->player->oldvelocity[2]) &&
        (!(ent->flags2 & MF2_ONMOBJ) || !(ent->z <= ent->floorz)))
    {
...
void P_DeathThink(player_t *player)
{
    ...
    if (cl_deathcam && clientside && player->attacker && player->attacker != player->mo)
    {
        angle_t angle = P_PointToAngle(player->mo->x, player->mo->y, player->attacker->x, player->attacker->y);
...
bool P_CanSpy(player_t &viewer, player_t &other)
{
    ...
    else if (G_IsTeamGame())
    {
        ...
        {
            PlayerResults pr = PlayerQuery().hasLives().onTeam(viewer.userinfo.team).execute();
            if (pr.count == 0)
            {
...
void P_PlayerThink(player_t *player)
{
    ...
    if (player->mo->subsector &&
        (player->mo->subsector->sector->special || player->mo->subsector->sector->damageamount ||
    ...
    {
        if (displayplayer().powers[pw_invulnerability] > 4 * 32 || (displayplayer().powers[pw_invulnerability] & 8))
    ...
    if (serverside && player->ingame() && !player->spectator && player->mo->health > 0)
    {
        if (P_AtInterval(5))
        {
            M_LogWDLEvent(WDL_EVENT_PLAYERBEACON, player, NULL, player->mo->angle / 4, 0, 0, 0);
        }
    }
}
...
BEGIN_COMMAND(cheat_players)
{
    ...
    while ((mo = iterator.Next()))
    {
        ...
        if (mo->type == MT_PLAYER)
        {
            ...
            if (mo->player)
            {
                Printf("%.3u: %s\n", mo->player->id, mo->player->userinfo.netname.c_str());
            }
            else
            {
                Printf("???: ???\n");
...
void player_s::Serialize(FArchive &arc)
{
    ...
    { // saving to archive
        arc << id << playerstate
            << spectator
            //			<< deadspectator
            << cmd << userinfo << viewz << viewheight << deltaviewheight << bob << health << armorpoints << armortype
            << backpack << lives << roundwins << fragcount << readyweapon << pendingweapon << attackdown << usedown
            << cheats << refire << killcount << itemcount << secretcount << damagecount << bonuscount
            << points
            /*<< attacker->netid*/
            << extralight << fixedcolormap << xviewshift << jumpTics << death_time << air_finished;
    ...
    { // Restoring from archive
        ...
        arc >> id >> playerstate >> spectator
            //			>> deadspectator
            >> cmd >> userinfo // Q: Would it be better to restore the userinfo from the archive?
            >> viewz >> viewheight >> deltaviewheight >> bob >> health >> armorpoints >> armortype >> backpack >>
            lives >> roundwins >> fragcount >> readyweapon >> pendingweapon >> attackdown >> usedown >> cheats >>
            refire >> killcount >> itemcount >> secretcount >> damagecount >> bonuscount >> points
            /*>> attacker->netid*/
            >> extralight >> fixedcolormap >> xviewshift >> jumpTics >> death_time >> air_finished;
        for (i = 0; i < NUMPOWERS; i++)
            arc >> powers[i];
...
player_s::player_s()
    : id(0), playerstate(PST_LIVE), mo(AActor::AActorPtr()), cmd(ticcmd_t()), cmdqueue(std::queue<NetCommand>()),
      userinfo(UserInfo()), fov(90.0), viewz(0 << FRACBITS), prevviewz(0 << FRACBITS), viewheight(0 << FRACBITS),
      deltaviewheight(0 << FRACBITS), bob(0 << FRACBITS), health(0), armorpoints(0), armortype(0), backpack(false),
      lives(0), roundwins(0), points(0), fragcount(0), deathcount(0), monsterdmgcount(0), killcount(0), itemcount(0),
      secretcount(0), pendingweapon(wp_fist), readyweapon(wp_fist), attackdown(0), usedown(0), cheats(0), refire(0),
      damagecount(0), bonuscount(0), extralight(0), fixedcolormap(0), xviewshift(0), psprnum(0), jumpTics(0),
      death_time(0), suicidedelay(0), camera(AActor::AActorPtr()), air_finished(0), GameTime(0), JoinTime(time_t()),
      ping(0), last_received(0), tic(0), snapshots(PlayerSnapshotManager()), spying(0), spectator(false), joindelay(0),
      timeout_callvote(0), timeout_vote(0), ready(false), timeout_ready(0), blend_color(argb_t(0, 0, 0, 0)),
      doreborn(false), QueuePosition(0), hazardcount(0), hazardinterval(0), LastMessage(LastMessage_s()),
      to_spawn(std::queue<AActor::AActorPtr>()), client(player_s::client_t())
{
    cmd.clear();
    ArrayInit(powers, 0);
...
VERSION_CONTROL(p_user_cpp, "$Id: d9a4bb9651fd0b7463a8a766dbe54abb16229e14 $")
