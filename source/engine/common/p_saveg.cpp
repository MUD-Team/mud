...
//-----------------------------------------------------------------------------
//
// $Id: 2fba2d8da5e9a84c037ab2de7200433aec056848 $
//
...
void P_SerializeWorld(FArchive &arc)
{
    ...
    { // saving to archive
        ...
        {
            arc << sec->floorheight << sec->ceilingheight << sec->floorplane.a << sec->floorplane.b << sec->floorplane.c
                << sec->floorplane.d << sec->ceilingplane.a << sec->ceilingplane.b << sec->ceilingplane.c
                << sec->ceilingplane.d << sec->floorpic << sec->ceilingpic << sec->lightlevel << sec->special
                << sec->flags << sec->tag << sec->secretsector
                << sec->soundtraversed
                /*<< sec->soundtarget*/
                << sec->friction << sec->movefactor << sec->floordata << sec->ceilingdata << sec->lightingdata
                << sec->stairlock << sec->prevsec << sec->nextsec << sec->floor_xoffs << sec->floor_yoffs
                << sec->ceiling_xoffs << sec->ceiling_xoffs << sec->floor_xscale << sec->floor_yscale
                << sec->ceiling_xscale << sec->ceiling_yscale << sec->floor_angle << sec->ceiling_angle
                << sec->base_ceiling_angle << sec->base_ceiling_yoffs << sec->base_floor_angle << sec->base_floor_yoffs
                ...
                << sec->colormap->color.geta() << sec->colormap->color.getr() << sec->colormap->color.getg()
                << sec->colormap->color.getb() << sec->colormap->fade.geta() << sec->colormap->fade.getr()
                << sec->colormap->fade.getg()
                << sec->colormap->fade.getb()
                ...
                << sec->alwaysfake << sec->waterzone << sec->SecActTarget << sec->MoreFlags;
        }
        ...
        for (i = 0, li = lines; i < numlines; i++, li++)
        {
            ...
            for (j = 0; j < 2; j++)
            {
                ...
                arc << si->textureoffset << si->rowoffset << si->toptexture << si->bottomtexture << si->midtexture;
            }
        }
    }
    else
    { // loading from archive
        ...
        for (i = 0, sec = sectors; i < numsectors; i++, sec++)
        {
            ...
            arc >> sec->floorheight >> sec->ceilingheight >> sec->floorplane.a >> sec->floorplane.b >>
                sec->floorplane.c >> sec->floorplane.d >> sec->ceilingplane.a >> sec->ceilingplane.b >>
                sec->ceilingplane.c >> sec->ceilingplane.d >> sec->floorpic >> sec->ceilingpic >> sec->lightlevel >>
                sec->special >> sec->flags >> sec->tag >> sec->secretsector >> sec->soundtraversed
            ...
            sec->colormap = GetSpecialLights(lightcolor.getr(), lightcolor.getg(), lightcolor.getb(), fadecolor.getr(),
                                             fadecolor.getg(), fadecolor.getb());
            ...
            arc >> sec->alwaysfake >> sec->waterzone >> SecActTarget >> sec->MoreFlags;
...
VERSION_CONTROL(p_saveg_cpp, "$Id: 2fba2d8da5e9a84c037ab2de7200433aec056848 $")
