...
//-----------------------------------------------------------------------------
//
// $Id: 504379aada7df1bff708ffe6f866f79d72c7e87c $
//
...
{
  public:
    ActionKeyTracker() : mTable(NUM_ACTIONS)
    {
    }
...
DConsoleAlias::DConsoleAlias(const char *name, const char *command)
    : DConsoleCommand(StdStringToLower(name).c_str()), state_lock(false), m_Command(command)
{
}
...
BEGIN_COMMAND(logfile)
{
    ...
    if (::LOG.is_open())
    {
        if ((argc == 1 && ::LOG_FILE == default_logname) || (argc > 1 && ::LOG_FILE == argv[1]))
        {
...
VERSION_CONTROL(c_dispatch_cpp, "$Id: 504379aada7df1bff708ffe6f866f79d72c7e87c $")
