...
//-----------------------------------------------------------------------------
//
// $Id: 6434658bb40723f002b3929d1821f2304a13688e $
//
...
struct spawnInventory_t
{
    ...
    spawnInventory_t(const spawnInventory_t &other)
        : isdefault(other.isdefault), health(other.health), armorpoints(other.armorpoints), armortype(other.armortype),
          readyweapon(other.readyweapon), berserk(other.berserk), backpack(other.backpack), invul(other.invul)
    {
...
static int WeaponTypeFromChar(const char ch)
{
    ...
    return INT32_MIN; // best chance of loud and obvious crash
}
 ...
 */
static void InvSetAmmo(spawnInventory_t &inv, const ammotype_t type, const std::string &value)
{
    if (type < am_clip || type >= NUMAMMO)
    {
        return;
...
static void SpawninvCommandHelp(const char *cmd)
{
    ...
    else if (!stricmp(cmd, "weapons"))
    {
        ...
        Printf("  Valid values are 1-7, A(Saw), and C(SSG).\n");
        Printf("  Multiple weapons are passed as a single block of letters and numbers.\n");
