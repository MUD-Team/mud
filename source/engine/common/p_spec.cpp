...
//-----------------------------------------------------------------------------
//
// $Id: df1a42a6afc53b23a51a28e38fc4f1251928ba6f $
//
...
void DScroller::Serialize(FArchive &arc)
{
    ...
    if (arc.IsStoring())
    {
        arc << m_Type << m_dx << m_dy << m_Affectee << m_Control << m_LastHeight << m_vdx << m_vdy << m_Accel;
    }
    else
    {
        arc >> m_Type >> m_dx >> m_dy >> m_Affectee >> m_Control >> m_LastHeight >> m_vdx >> m_vdy >> m_Accel;
...
DPusher::DPusher() : m_Type(p_push), m_Xmag(0), m_Ymag(0), m_Magnitude(0), m_Radius(0), m_X(0), m_Y(0), m_Affectee(0)
{
}
...
void DPusher::Serialize(FArchive &arc)
{
    ...
    if (arc.IsStoring())
    {
        arc << m_Type << m_Source << m_Xmag << m_Ymag << m_Magnitude << m_Radius << m_X << m_Y << m_Affectee;
    }
    else
    {
        arc >> m_Type;
...
fixed_t P_FindLowestFloorSurrounding(sector_t *sec)
{
    ...
    for (i = 0; i < sec->linecount; i++)
    {
        ...
        fixed_t v1height = P_FloorHeight(sec->lines[i]->v1->x, sec->lines[i]->v1->y, other);
        fixed_t v2height = P_FloorHeight(sec->lines[i]->v1->x, sec->lines[i]->v1->y, other);
...
fixed_t P_FindLowestCeilingSurrounding(sector_t *sec)
{
    ...
    for (i = 0; i < sec->linecount; i++)
    {
        ...
        fixed_t v1height = P_CeilingHeight(sec->lines[i]->v1->x, sec->lines[i]->v1->y, other);
        fixed_t v2height = P_CeilingHeight(sec->lines[i]->v1->x, sec->lines[i]->v1->y, other);
...
bool P_CanUnlockGenDoor(line_t *line, player_t *player)
{
    ...
    case 7: // AllKeys
        if (!skulliscard &&
            (!player->cards[it_redcard] || !player->cards[it_redskull] || !player->cards[it_bluecard] ||
             !player->cards[it_blueskull] || !player->cards[it_yellowcard] || !player->cards[it_yellowskull]))
        {
            msg = &PD_ALL6; // Ty 03/27/98 - externalized
        }
        else if (skulliscard && ((!player->cards[it_redcard] && !player->cards[it_redskull]) ||
                                 (!player->cards[it_bluecard] && !player->cards[it_blueskull]) ||
                                 // e6y
...
bool P_HandleSpecialRepeat(line_t *line)
{
    ...
    if (P_IsExitLine(line->special) && (!sv_allowexit && sv_fragexitswitch))
        return false;
    else
        return true;
}
...
void P_CrossSpecialLine(line_t *line, int side, AActor *thing, bool bossaction)
{
    ...
    if (thing && thing->player && (thing->player->spectator || thing->player->playerstate != PST_LIVE) &&
    ...
    if (result)
    {
        ...
        {
            if (!(thing->player && (thing->player->spectator || thing->player->playerstate != PST_LIVE)))
            {
...
void P_ShootSpecialLine(AActor *thing, line_t *line)
{
    ...
    {
        lineresult = LineSpecials[line->special](line, thing, line->args[0], line->args[1], line->args[2],
...
bool P_UseSpecialLine(AActor *thing, line_t *line, int side, bool bossaction)
{
    ...
    if (!bossaction && thing)
    {
        ...
        else
        {
            ...
            if (thing->player->spectator || thing->player->playerstate != PST_LIVE)
                return false;
        }
    }
...
bool P_PushSpecialLine(AActor *thing, line_t *line, int side)
{
    ...
    if (LineSpecials[line->special](line, thing, line->args[0], line->args[1], line->args[2], line->args[3],
                                    line->args[4]))
    {
        SV_OnActivatedLine(line, thing, side, LinePush, false);

        if (serverside && !(thing->player && (thing->player->spectator || thing->player->playerstate != PST_LIVE)))
        {
            bool repeat;
...
CVAR_FUNC_IMPL(sv_forcewater)
{
    if (gamestate == GS_LEVEL)
    {
        ...
        {
            if (sectors[i].heightsec && !(sectors[i].heightsec->MoreFlags & SECF_IGNOREHEIGHTSEC) &&
...
void DPusher::RunThink()
{
    ...
    for (; node; node = node->m_snext)
    {
        ...
        if (m_Type == p_wind)
        {
            if (sec->heightsec == NULL || sec->heightsec->MoreFlags & SECF_IGNOREHEIGHTSEC) // NOT special water sector
            {
...
VERSION_CONTROL(p_spec_cpp, "$Id: df1a42a6afc53b23a51a28e38fc4f1251928ba6f $")
