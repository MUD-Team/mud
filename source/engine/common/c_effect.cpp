...
//-----------------------------------------------------------------------------
//
// $Id: c30e0daa5a5d8598092d0135b282c74ec5992df4 $
//
...
static const struct ColorList
{
    int *color, r, g, b;
              ...
              {&black, 0, 0, 0},        {&red1, 255, 127, 127},    {&green1, 127, 255, 127},
              {&blue1, 127, 127, 255},  {&yellow1, 255, 255, 180}, {&purple, 120, 0, 160},
              {&purple1, 200, 30, 255}, {&white, 255, 255, 255},   {&rblue1, 81, 81, 255},
              {&rblue2, 0, 0, 227},     {&rblue3, 0, 0, 130},      {&rblue4, 0, 0, 80},
              {&orange, 255, 120, 0},   {&yorange, 255, 170, 0},   {&dred, 80, 0, 0},
              {&maroon1, 154, 49, 49},  {&maroon2, 125, 24, 24},   {NULL}};
...
void P_DrawRailTrail(v3double_t &start, v3double_t &end)
{
    ...
    if (abs(mo->x - FLOAT2FIXED(start.x)) < 20 * FRACUNIT && abs(mo->y - FLOAT2FIXED(start.y)) < 20 * FRACUNIT)
    {
    ...
    else
    {
        ...
        double r =
            ((start.y - FIXED2FLOAT(mo->y)) * (-dir.y) - (start.x - FIXED2FLOAT(mo->x)) * (dir.x)) * ilength * ilength;

        M_ScaleVec3(&point, &dir, r);
        M_AddVec3(&point, &start, &point);
        point.z = start.z;

        S_Sound(FLOAT2FIXED(point.x), FLOAT2FIXED(point.y), CHAN_WEAPON, "weapons/railgf", 1, ATTN_NORM);
    }
...
VERSION_CONTROL(p_effect_cpp, "$Id: c30e0daa5a5d8598092d0135b282c74ec5992df4 $")
