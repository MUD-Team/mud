...
//-----------------------------------------------------------------------------
//
// $Id: 48570c90b9e3f6c2918e4ad0603087d95ac5f290 $
//
...
bool CHEAT_AreCheatsEnabled()
{
    ...
    if (!multiplayer && G_GetCurrentSkill().disable_cheats)
    {
        if (!sv_allowcheats)
        {
            Printf(PRINT_WARNING, "You must 'set sv_allowcheats 1' in the console to enable "
...
void CHEAT_DoCheat(player_t *player, int32_t cheat, bool silentmsg)
{
    ...
    case CHT_MDK: {
        ...
        if (serverside)
        {
            P_LineAttack(player->mo, player->mo->angle, 8192 * FRACUNIT,
                         P_AimLineAttack(player->mo, player->mo->angle, 8192 * FRACUNIT), 10000);
    ...
    case CHT_BUDDHA: {
        ...
        msg = (player->cheats & CF_BUDDHA) ? GStrings(TXT_BUDDHAON) : GStrings(TXT_BUDDHAOFF);
    }
    }
...
VERSION_CONTROL(m_cheat_cpp, "$Id: 48570c90b9e3f6c2918e4ad0603087d95ac5f290 $")
