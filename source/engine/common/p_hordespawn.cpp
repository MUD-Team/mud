...
//-----------------------------------------------------------------------------
//
// $Id: 3536161c73c5222e4ac11b3e530fa65f67d8fead $
//
 ...
 */
static AActor::AActorPtr SpawnMonster(hordeSpawn_t &spawn, const hordeRecipe_t &recipe, const v2fixed_t offset)
{
    AActor *mo = new AActor(spawn.mo->x + offset.x, spawn.mo->y + offset.y, spawn.mo->z, recipe.type);
    if (mo)
    {
        if (P_TestMobjLocation(mo))
        {
            if (recipe.isBoss)
            {
                ...
                // Set flags as a boss.
                mo->oflags = MFO_INFIGHTINVUL | MFO_UNFLINCHING | MFO_ARMOR | MFO_QUICK | MFO_NORAISE | MFO_BOSSPOOL |
                             MFO_FULLBRIGHT;
...
static AActors SpawnMonsterGroup(hordeSpawn_t &spawn, const hordeRecipe_t &recipe, const int count)
{
    ...
    if (ret.size() < count)
    {
        DPrintf("Partial spawn %zu/%d of type %s at a %s spawn (%f, %f).\n", ret.size(), count, name,
                HordeThingStr(spawn.type), FIXED2FLOAT(spawn.mo->x), FIXED2FLOAT(spawn.mo->y));
    }
...
hordeSpawn_t *P_HordeSpawnPoint(const hordeRecipe_t &recipe)
{
    ...
    for (hordeSpawns_t::iterator sit = monsterSpawns.begin(); sit != monsterSpawns.end(); ++sit)
    {
        mobjinfo_t &info     = ::mobjinfo[recipe.type];
        const bool  isFlying = info.flags & (MF_NOGRAVITY | MF_FLOAT);

        if (recipe.isBoss && sit->type != TTYPE_HORDE_BOSS && sit->type != TTYPE_HORDE_SMALLBOSS)
        {
...
void P_HordeSpawnItem()
{
    ...
    for (hordeSpawns_t::iterator it = ::itemSpawns.begin(); it != ::itemSpawns.end(); ++it)
    {
...
void P_HordeSpawnPowerup(const mobjtype_t pw)
{
    // Find all empty points.
    hordeSpawns_t emptys;
    for (hordeSpawns_t::iterator it = ::powerupSpawns.begin(); it != ::powerupSpawns.end(); ++it)
    {
        if (it->mo->target == NULL)
        {
            emptys.push_back(*it);
