...
//-----------------------------------------------------------------------------
//
// $Id: a6b6ed282f1196374408c034929d0bbd345a8698 $
//
...
bool P_ActivateZDoomLine(line_t *line, AActor *mo, int side, unsigned int activationType)
{
    ...
    if (!P_TestActivateZDoomLine(line, mo, side, activationType) || !P_CanActivateSpecials(mo, line))
    {
        return false;
    }
...
bool P_TestActivateZDoomLine(line_t *line, AActor *mo, int side, unsigned int activationType)
{
    ...
    if (line->special == Teleport && (lineActivation & ML_SPAC_CROSS | ML_SPAC_CROSSTHROUGH) &&
...
void P_PlayerInZDoomSector(player_t *player)
{
    ...
    case Scroll_Carry_North35:
        P_ThrustMobj(player->mo, ANG90, heretic_carry[sector->special - Scroll_Carry_North5]);
        break;
    ...
    case Scroll_Carry_South35:
        P_ThrustMobj(player->mo, ANG270, heretic_carry[sector->special - Scroll_Carry_South5]);
        break;
    ...
    case Scroll_Carry_West35:
        P_ThrustMobj(player->mo, ANG180, heretic_carry[sector->special - Scroll_Carry_West5]);
        break;
    ...
    case Scroll_South_Fast:
        P_ThrustMobj(player->mo, ANG270, hexen_carry[sector->special - Scroll_South_Slow]);
        break;
    ...
    case Scroll_NorthWest_Fast:
        P_ThrustMobj(player->mo, ANG135, hexen_carry[sector->special - Scroll_NorthWest_Slow]);
        break;
    case Scroll_NorthEast_Slow:
    case Scroll_NorthEast_Medium:
    case Scroll_NorthEast_Fast:
        P_ThrustMobj(player->mo, ANG45, hexen_carry[sector->special - Scroll_NorthEast_Slow]);
        break;
    case Scroll_SouthEast_Slow:
    case Scroll_SouthEast_Medium:
    case Scroll_SouthEast_Fast:
        P_ThrustMobj(player->mo, ANG315, hexen_carry[sector->special - Scroll_SouthEast_Slow]);
        break;
    case Scroll_SouthWest_Slow:
    case Scroll_SouthWest_Medium:
    case Scroll_SouthWest_Fast:
        P_ThrustMobj(player->mo, ANG225, hexen_carry[sector->special - Scroll_SouthWest_Slow]);
        break;
    default:
        break;
    }
...
void P_SpawnZDoomSectorSpecial(sector_t *sector)
{
    ...
    default:
        ...
        else if (sector->special >= Scroll_Carry_East5 && sector->special <= Scroll_Carry_East35)
        { // Heretic scroll special
            // Only east scrollers also scroll the texture
            fixed_t dx = FixedDiv((1 << (sector->special - Scroll_Carry_East5)) << FRACBITS, 2);
...
void P_SpawnZDoomScroller(line_t *l, int i)
{
    ...
    if (special == Scroll_Ceiling || special == Scroll_Floor || special == Scroll_Texture_Model)
    {
        if (l->args[1] & 3)
        {
    ...
    case Scroll_Texture_Offsets:
        ...
        if (l->args[0] == 0) // [Blair] Hack in generalized scrollers.
        {
            ...
            s = lines[i].sidenum[0];
            new DScroller(DScroller::sc_side, -sides[s].textureoffset, sides[s].rowoffset, -1, s, accel);
        }
        else
        {
            if (l->id == 0)
    ...
    case Scroll_Texture_Left:
        ...
        new DScroller(DScroller::sc_side, l->args[0] * (FRACUNIT / 64), 0, -1, lines[i].sidenum[0], accel);
        break;
    ...
    case Scroll_Texture_Up:
        ...
        new DScroller(DScroller::sc_side, 0, l->args[0] * (FRACUNIT / 64), -1, lines[i].sidenum[0], accel);
        break;
            ...
            for (s = -1; (s = P_FindSectorFromTag(l->args[0], s)) >= 0;)
            {
                ...
                { // No MT_P* means no effect
                    ...
                    if (!l->args[1] || l->args[1] == thing->tid)
                        new DPusher(DPusher::p_push, l->args[3] ? l : NULL, l->args[2], 0, thing, s);
                }
            }
        }
        else
        { // [RH] Find thing by tid
            ...
            while ((thing = AActor::FindByTID(thing, l->args[1])))
            {
                if (thing->type == MT_PUSH || thing->type == MT_PULL)
                    new DPusher(DPusher::p_push, l->args[3] ? l : NULL, l->args[2], 0, thing,
