...
//-----------------------------------------------------------------------------
//
// $Id: ca3cda97907843aac66e3bc10372164214e6bdd7 $
//
...
bool P_CheckSightZDoom(const AActor *t1, const AActor *t2)
{
    ...
    if ((s1->heightsec && !(s1->heightsec->MoreFlags & SECF_IGNOREHEIGHTSEC) &&
         ((t1->z + t1->height <= s1_floorheight_t1 && t2->z >= s1_floorheight_t2) ||
          (t1->z >= s1_ceilingheight_t1 && t2->z + t1->height <= s1_ceilingheight_t2))) ||
        (s2->heightsec && !(s2->heightsec->MoreFlags & SECF_IGNOREHEIGHTSEC) &&
         ((t2->z + t2->height <= s2_floorheight_t2 && t1->z >= s2_floorheight_t1) ||
          (t2->z >= s2_ceilingheight_t2 && t1->z + t2->height <= s2_ceilingheight_t1))))
        return false;
...
// Uses REJECT.
//
bool P_CheckSightEdgesDoom(const AActor *t1, const AActor *t2, float radius_boost)
{
    ...
    contact |= P_CheckSightDoom(t1->x, t1->y, t1->z, t1->height, t2->x, t2->y, t2->z, t2->height);

    contact |= P_CheckSightDoom(t1->x, t1->y, t1->z, t1->height, t2->x - FLOAT2FIXED(w.x), t2->y - FLOAT2FIXED(w.y),
                                t2->z, t2->height);
...
VERSION_CONTROL(p_sight_cpp, "$Id: ca3cda97907843aac66e3bc10372164214e6bdd7 $")
