...
//-----------------------------------------------------------------------------
//
// $Id: a02478e9b436d65f55dedc72583245f613234ea4 $
//
...
bool EV_Teleport(int32_t tid, int32_t tag, int32_t arg0, int32_t side, AActor *thing, int32_t nostop)
{
    ...
    if (serverside && !(player && player->spectator))
    {
        ...
        if (nostop == 1)
            S_Sound(new AActor(m->x + 5 * finecosine[an], m->y + 5 * finesine[an], thing->z, MT_TFOG), CHAN_VOICE,
                    "misc/teleport", 1, ATTN_NORM);
        else
            S_Sound(new AActor(m->x + 20 * finecosine[an], m->y + 20 * finesine[an], thing->z, MT_TFOG), CHAN_VOICE,
                    "misc/teleport", 1, ATTN_NORM);
    }
...
bool EV_SilentTeleport(int32_t tid, int32_t useangle, int32_t tag, int32_t keepheight, line_t *line, int32_t side, AActor *thing)
{
    ...
    angle_t angle = P_PointToAngle(0, 0, line->dx, line->dy) - m->angle + ANG90;
...
bool EV_SilentLineTeleport(line_t *line, int32_t side, AActor *thing, int32_t id, bool reverse)
{
    ...
    for (i = -1; (i = P_FindLineFromID(id, i)) >= 0;)
        if ((l = lines + i) != line && l->backsector)
        {
            ...
            fixed_t z = thing->z - P_FloorHeight(thing->x, thing->y, sides[line->sidenum[1]].sector) +
...
VERSION_CONTROL(p_teleport_cpp, "$Id: a02478e9b436d65f55dedc72583245f613234ea4 $")
