...
//-----------------------------------------------------------------------------
//
// $Id: be779e9f8f7a7c71fb82734764266c2a904d9dbc $
//
...
void hordeDefine_t::addMonster(const waveMonsterType_e monster, const mobjtype_t mobj, const monConfig_t &config)
{
...
bool P_HordeSpawnRecipe(hordeRecipe_t &out, const hordeDefine_t &define, const bool wantBoss)
{
    ...
    const hordeDefine_t::monster_t *const monster = P_RandomFloatWeighted(monsters, MonsterChance);
            ...
            if (argc >= 3)
            {
                ...
                if (current && current <= g_horde_waves)
                {
                    ...
                    if (total > 0)
                    {
                        ...
                        const size_t end            = MIN<size_t>(section_offset + section_choice, section_limit);
                        Printf("[Wave %d/%d - Start:%zu End:%zu]\n", current, total, start, end);
                        PrintDefines(::WAVE_DEFINES.begin() + start, ::WAVE_DEFINES.begin() + end + 1);
                        return;
                    }
                    else
                    {
