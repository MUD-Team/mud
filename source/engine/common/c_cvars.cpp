...
//-----------------------------------------------------------------------------
//
// $Id: 51ec62a3c8e7d1c552f036f36802fd6782f8a19b $
//
...
void cvar_t::ForceSet(const char *valstr)
{
    ...
    else
    {
        ...
        bool integral_type =
            m_Type == CVARTYPE_BOOL || m_Type == CVARTYPE_BYTE || m_Type == CVARTYPE_WORD || m_Type == CVARTYPE_INT;
...
void cvar_t::C_WriteCVars(byte **demo_p, DWORD filter, bool compact)
{
    ...
    else
    {
        ...
        while (cvar)
        {
            if (cvar->m_Flags & filter)
            {
                ptr += sprintf((char *)ptr, "\\%s\\%s", cvar->name(), cvar->cstring());
            }
            cvar = cvar->m_Next;
        }
    }
    ...
    while (var)
    {
        ...
        Printf(PRINT_HIGH, "%c%c%c%c %s \"%s\"\n",
               ...
               : flags & CVAR_SERVERARCHIVE ? 'S'
                                            : ' ',
               flags & CVAR_USERINFO ? 'U' : ' ', flags & CVAR_SERVERINFO ? 'S' : ' ',
               flags & CVAR_NOSET        ? '-'
               : flags & CVAR_LATCH      ? 'L'
...
VERSION_CONTROL(c_cvars_cpp, "$Id: 51ec62a3c8e7d1c552f036f36802fd6782f8a19b $")
