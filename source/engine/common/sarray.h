...
//-----------------------------------------------------------------------------
//
// $Id: f48b96a9ab449d70892299807fbaecb7ab2e7ae7 $
//
  ...
  public:
    ...
    template <typename IVT, typename ISAT> class generic_iterator
    {
      ...
      public:
        ...
        generic_iterator(ISAT &sarray, SArrayId id) : mSArray(sarray), mSlot(NOT_FOUND)
        {
            if (id != NOT_FOUND)
                mSlot = mSArray.getSlot(id);
        }

        generic_iterator(ISAT &sarray, const VT &item) : mSArray(sarray)
        {
    ...
    //
    SArray(const SArrayType &other) : mItemRecords(NULL), mSize(0)
    {
        clear();
        resize(other.mSize);
        copyFrom(other);
