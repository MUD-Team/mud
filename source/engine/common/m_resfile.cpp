 ...
 * @return True if the OWantFile was populated successfully.
 */
bool OWantFile::makeWithHash(OWantFile &out, const std::string &file, const ofile_t type, const OMD5Hash &hash)
{
    std::string basename = M_ExtractFileName(file);
    if (basename.empty())
    {
...
bool M_ResolveWantedFile(OResFile &out, const OWantFile &wanted)
{
    ...
    {
        const std::string result = M_BaseFileSearchDir(*it, basename, exts, wanted.getWantedMD5());
        if (!result.empty())
        {
            // Found a file.
