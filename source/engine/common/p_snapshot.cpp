...
//-----------------------------------------------------------------------------
//
// $Id: e805038da1dd312097b633b24aef8fc71b4e1339 $
//
...
{
    return mTime == other.mTime && mValid == other.mValid && mAuthoritative == other.mAuthoritative &&
...
ActorSnapshot::ActorSnapshot(int32_t time, const AActor *mo)
    : Snapshot(time), mFields(0xFFFFFFFF), mX(mo->x), mY(mo->y), mZ(mo->z), mMomX(mo->momx), mMomY(mo->momy),
      ...
      mFlags2(mo->flags2), mFlags3(mo->flags3), mFrame(mo->frame)
{
}

bool ActorSnapshot::operator==(const ActorSnapshot &other) const
{
    return Snapshot::operator==(other) && mFields == other.mFields && mX == other.mX && mY == other.mY &&
           mZ == other.mZ && mMomX == other.mMomX && mMomY == other.mMomY && mMomZ == other.mMomZ &&
           mAngle == other.mAngle && mPitch == other.mPitch && mOnGround == other.mOnGround &&
           mCeilingZ == other.mCeilingZ && mFloorZ == other.mFloorZ && mReactionTime == other.mReactionTime &&
           mWaterLevel == other.mWaterLevel && mFlags == other.mFlags && mFlags2 == other.mFlags2 &&
           mFlags3 == other.mFlags3 && mFrame == other.mFrame;
}
...
{
    return Snapshot::operator==(other) && mFields == other.mFields && mActorSnap == other.mActorSnap &&
...
PlayerSnapshotManager::PlayerSnapshotManager() : mMostRecent(0)
{
    clearSnapshots();
}
...
SectorSnapshot::SectorSnapshot(int32_t time)
    : Snapshot(time), mCeilingMoverType(SEC_INVALID), mFloorMoverType(SEC_INVALID), mSector(NULL), mCeilingType(0),
      ...
      mCeilingDirection(0), mFloorDirection(0), mCeilingOldDirection(0), mFloorOldDirection(0), mCeilingTexture(0),
      mFloorTexture(0), mNewCeilingSpecial(0), mNewFloorSpecial(0), mCeilingLow(0), mCeilingHigh(0), mFloorLow(0),
      mFloorHigh(0), mCeilingCrush(false), mFloorCrush(false), mSilent(false), mCeilingWait(0), mFloorWait(0),
      mCeilingCounter(0), mFloorCounter(0), mResetCounter(0), mCeilingStatus(0), mFloorStatus(0), mOldFloorStatus(0),
      mCrusherSpeed1(0), mCrusherSpeed2(0), mStepTime(0), mPerStepTime(0), mPauseTime(0), mOrgHeight(0), mDelay(0),
      mFloorLip(0), mFloorOffset(0), mCeilingChange(0), mFloorChange(0)
{
}
...
void SectorSnapshot::toSector(sector_t *sector) const
{
    ...
    if (mCeilingMoverType == SEC_DOOR && mCeilingStatus != DDoor::destroy)
    {
        ...
        {
            sector->ceilingdata =
                new DDoor(sector, mCeilingLine, static_cast<DDoor::EVlDoor>(mCeilingType), mCeilingSpeed, mCeilingWait);
        }
    ...
    if (mFloorMoverType == SEC_FLOOR && mFloorStatus != DFloor::destroy)
    {
        ...
        if (!sector->floordata)
        {
            sector->floordata = new DFloor(sector, static_cast<DFloor::EFloor>(mFloorType), mFloorLine, mFloorSpeed,
                                           mFloorOffset, mFloorCrush, mFloorChange);
        }
    ...
    if (mFloorMoverType == SEC_PLAT && mFloorStatus != DPlat::destroy)
    {
        ...
        if (!sector->floordata)
        {
            sector->floordata = new DPlat(sector, static_cast<DPlat::EPlatType>(mFloorType), mFloorOffset, mFloorSpeed,
                                          mFloorWait, mFloorLip);
        }
...
SectorSnapshotManager::SectorSnapshotManager() : mMostRecent(0)
{
    clearSnapshots();
}
    ...
    while (--prevsnaptime > mMostRecent - NUM_SNAPSHOTS)
    {
        if (mValidSnapshot(prevsnaptime))
        {
            ...
            for (int32_t i = 0; i < time - prevsnaptime; i++)
            {
                ...
                if (tempsector.floordata && tempsector.floordata != tempsector.ceilingdata)
...
bool P_CeilingSnapshotDone(SectorSnapshot *snap)
{
    ...
    if ((snap->getCeilingMoverType() == SEC_CEILING && snap->getCeilingStatus() == DCeiling::destroy) ||
        (snap->getCeilingMoverType() == SEC_DOOR && snap->getCeilingStatus() == DDoor::destroy) ||
        (snap->getCeilingMoverType() == SEC_PILLAR && snap->getCeilingStatus() == DPillar::destroy) ||
        (snap->getCeilingMoverType() == SEC_ELEVATOR && snap->getCeilingStatus() == DElevator::destroy))
        return true;
...
bool P_FloorSnapshotDone(SectorSnapshot *snap)
{
    ...
    if ((snap->getFloorMoverType() == SEC_FLOOR && snap->getFloorStatus() == DFloor::destroy) ||
        (snap->getFloorMoverType() == SEC_PLAT && snap->getFloorStatus() == DPlat::destroy) ||
        (snap->getFloorMoverType() == SEC_PILLAR && snap->getFloorStatus() == DPillar::destroy) ||
        (snap->getFloorMoverType() == SEC_ELEVATOR && snap->getFloorStatus() == DElevator::destroy))
        return true;

    return false;
}

VERSION_CONTROL(p_snapshot_cpp, "$Id: e805038da1dd312097b633b24aef8fc71b4e1339 $")
