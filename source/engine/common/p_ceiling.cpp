...
//-----------------------------------------------------------------------------
//
// $Id: fa77326e2df3fedfda84512d8d14b91207ec8af7 $
//
...
void DCeiling::Serialize(FArchive &arc)
{
    ...
    if (arc.IsStoring())
    {
        arc << m_Type << m_Status << m_BottomHeight << m_TopHeight << m_Speed << m_Speed1 << m_Speed2 << m_Crush
            << m_Silent << m_Direction << m_Texture << m_NewSpecial << m_Tag << m_OldDirection;
    }
    else
    {
        arc >> m_Type >> m_Status >> m_BottomHeight >> m_TopHeight >> m_Speed >> m_Speed1 >> m_Speed2 >> m_Crush >>
...
void DCeiling::RunThink()
{
    ...
    case -1:
        // DOWN
        res = MoveCeiling(m_Speed, m_BottomHeight, m_Crush, m_Direction, m_CrushMode == crushHexen);
...
DCeiling::DCeiling(sector_t *sec) : DMovingCeiling(sec), m_Status(init)
{
}
    ...
    : DMovingCeiling(sec), m_Status(init)
{
    ...
    case CtoLnC:
        ...
        break;
    case CtoNnC:
        targheight = direction ? P_FindNextHighestCeiling(sec) : P_FindNextLowestCeiling(sec);
        break;
    case CtoHnF:
    ...
    case CtoF:
        ...
        break;
    case CbyST:
        targheight = (sec->ceilingheight >> FRACBITS) + m_Direction * (P_FindShortestUpperAround(sec) >> FRACBITS);
        ...
        break;
    case Cby24:
        targheight = sec->ceilingheight + m_Direction * 24 * FRACUNIT;
        break;
    case Cby32:
        targheight = sec->ceilingheight + m_Direction * 32 * FRACUNIT;
        break;
    default:
        break;
    }
    ...
    if (change)    // if a texture change is indicated
    {
        if (model) // if a numeric model change
        {
            ...
            // is a floor type
            chgsec = (target == CtoHnF || target == CtoF) ? P_FindModelFloorSector(targheight, sec)
                         ...
                         int crush, int silent, int change, crushmode_e crushmode)
{
    ...
    manual_ceiling:
        ...
        case DCeiling::ceilLowerByTexture:
            targheight = ceiling->m_BottomHeight = ceilingheight - P_FindShortestUpperAround(sec);
            ceiling->m_Direction                 = -1;
            break;

        case DCeiling::ceilRaiseByTexture:
            targheight = ceiling->m_TopHeight = ceilingheight + P_FindShortestUpperAround(sec);
            ceiling->m_Direction              = 1;
            break;
        }
        ...
        if (change & 3)     // if a texture change is indicated
        {
            if (change & 4) // if a numeric model change
            {
                ...
                // is a floor type
                sector_t *found = NULL;
                if (type == DCeiling::ceilRaiseToFloor || type == DCeiling::ceilLowerToFloor)
                {
...
VERSION_CONTROL(p_ceiling_cpp, "$Id: fa77326e2df3fedfda84512d8d14b91207ec8af7 $")
