...
//-----------------------------------------------------------------------------
//
// $Id: 658c6d6620494450a08c7b2d4bfec1a825d21833 $
//
...
static const char *wdlevstrings[] = {
    "DAMAGE",        "CARRIERDAMAGE",     "KILL",       "CARRIERKILL",    "ENVIRODAMAGE", "ENVIROCARRIERDAMAGE",
    "ENVIROKILL",    "ENVIROCARRIERKILL", "TOUCH",      "PICKUPTOUCH",    "CAPTURE",      "PICKUPCAPTURE",
    "ASSIST",        "RETURNFLAG",        "PICKUPITEM", "SPREADACCURACY", "SSACCURACY",   "TRACERACCURACY",
    "PROJACCURACY",  "SPAWNPLAYER",       "SPAWNITEM",  "JOINGAME",       "DISCONNECT",   "PLAYERBEACON",
...
{
    return lhs.id == rhs.id && lhs.team == rhs.team && lhs.x == rhs.x && lhs.y == rhs.y && lhs.z == rhs.z;
}
...
static void AddWDLPlayer(player_t *player)
{
    ...
    {
        if ((*it).netname == player->userinfo.netname && (*it).team == player->userinfo.team && (*it).pid == player->id)
            return;
    }

    WDLPlayer wdlplayer = {
        (int32_t)::wdlplayers.size() + 1,
        player->id,
...
static void AddWDLPlayerSpawn(const mapthing2_t *mthing)
{
    ...
    WDLPlayerSpawn wdlplayerspawn = {(int32_t)::wdlplayerspawns.size() + 1, mthing->x, mthing->y, mthing->z, team};
...
int32_t GetItemSpawn(int32_t x, int32_t y, int32_t z, WDLPowerups item)
{
...
void M_LogWDLItemSpawn(AActor *target, WDLPowerups type)
{
    ...
    for (; it != ::wdlitemspawns.end(); ++it)
    {
        if ((*it).x == target->x && (*it).y == target->y && (*it).z == target->z && (*it).item == type)
            return;
    }

    WDLItemSpawn wdlitemspawn = {(int32_t)::wdlitemspawns.size() + 1, target->x, target->y, target->z, type};
...
{
    Printf(PRINT_HIGH, "wdlstats - Starts logging WDL statistics to the given directory.  Unless "
...
BEGIN_COMMAND(wdlstats)
{
    ...
    Printf(PRINT_HIGH, "wdlstats: Enabled, will log to directory \"%s\" on next map change.\n",
 ...
 */
static bool LogDamageEvent(WDLEvents event, player_t *activator, player_t *target, int32_t arg0, int32_t arg1,
                           int32_t arg2)
{
 ...
 */
bool LogAccuracyShot(WDLEvents event, player_t *activator, int32_t mod, angle_t angle)
{
 ...
 */
bool LogAccuracyHit(WDLEvents event, player_t *activator, player_t *target, int32_t mod, int32_t hits)
{
    // See if we have an existing accuracy event for this tic.
    WDLEventLog::reverse_iterator it = ::wdlevents.rbegin();
    for (; it != ::wdlevents.rend(); ++it)
...
void M_LogWDLItemRespawnEvent(AActor *activator)
{
    ...
    WDLEvent evt = {WDL_EVENT_SPAWNITEM, NULL, NULL, ::gametic, {ax, ay, az}, {0, 0, 0}, itemtype, itemspawnid, 0, 0};
 ...
 */
void M_LogWDLEvent(WDLEvents event, player_t *activator, player_t *target, int32_t arg0, int32_t arg1, int32_t arg2,
                   int32_t arg3)
{
    if (!::wdlstate.recording)
        return;
    ...
    if (activator && target && (event == WDL_EVENT_DAMAGE || event == WDL_EVENT_CARRIERDAMAGE))
    {
    ...
    if (activator && !target &&
        (event == WDL_EVENT_SSACCURACY || event == WDL_EVENT_SPREADACCURACY || event == WDL_EVENT_PROJACCURACY ||
         event == WDL_EVENT_TRACERACCURACY) &&
        arg2 <= 0)
    {
    ...
    // Add the event to the log.
    WDLEvent evt = {event, aid, tid, ::gametic, {ax, ay, az}, {tx, ty, tz}, arg0, arg1, arg2, arg3};
    ::wdlevents.push_back(evt);
}
 ...
 */
void M_LogActorWDLEvent(WDLEvents event, AActor *activator, AActor *target, int32_t arg0, int32_t arg1, int32_t arg2,
                        int32_t arg3)
{
    if (!::wdlstate.recording)
        return;
...
void M_HandleWDLNameChange(team_t team, std::string oldname, std::string newname, int32_t pid)
{
    if (!::wdlstate.recording)
        return;
...
void M_CommitWDLLog()
{
    ...
    for (; eit != ::wdlevents.end(); ++eit)
    {
        ...
        str.append(StrFormat("%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\n", eit->ev, eit->activator, eit->target,
                             eit->gametic, eit->apos[0], eit->apos[1], eit->apos[2], eit->tpos[0], eit->tpos[1],
                             eit->tpos[2], eit->arg0, eit->arg1, eit->arg2, eit->arg3));
    }
...
static void PrintWDLEvent(const WDLEvent &evt)
{
    ...
    Printf(PRINT_HIGH, "%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\n", evt.ev, evt.activator, evt.target, evt.gametic,
           evt.apos[0], evt.apos[1], evt.apos[2], evt.tpos[0], evt.tpos[1], evt.tpos[2], evt.arg0, evt.arg1, evt.arg2,
           evt.arg3);
}

static void WDLInfoHelp()
{
    Printf(PRINT_HIGH, "wdlinfo - Looks up internal information about logged WDL events\n\n"
                       "Usage:\n"
                       "  ] wdlinfo event <ID>\n"
