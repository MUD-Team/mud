...
struct SkillInfo
{
    ...
    SkillInfo()
        : ammo_factor(1.f), double_ammo_factor(2.f), drop_ammo_factor(-1.f), damage_factor(1.f), armor_factor(1.f),
          health_factor(1.f), kickback_factor(1.f)

          ,
          fast_monsters(false), slow_monsters(false), disable_cheats(false), auto_use_health(false)

          ,
          easy_boss_brain(false), easy_key(false), respawn_counter(0), respawn_limit(0), aggressiveness(1.f),
          spawn_filter(0), spawn_multi(false), instant_reaction(false), ACS_return(0), menu_name(), pic_name()
          //, menu_names_for_player_class(???)
          ,
          must_confirm(false), must_confirm_text("$NIGHTMARE"), shortcut(0), text_color()
          //, replace(???)
          //, replaced(???)
          ,
          monster_health(1.), friendly_health(1.), no_pain(false), infighting(0), player_respawn(false)
    {
    }
};
