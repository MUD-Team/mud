...
//-----------------------------------------------------------------------------
//
// $Id: a53e1b31578c2bef915c45d13ee1818817d5cd41 $
//
...
template <typename KT> struct hashfunc
{
};
...
template <typename KT, typename VT, typename HF = hashfunc<KT>> class OHashTable
{
  private:
  ...
  public:
    ...
    class generic_iterator
    {
      ...
      public:
        ...
        generic_iterator(IndexType bucketnum, IHTT *hashtable) : mBucketNum(bucketnum), mHashTable(hashtable)
        {
            while (mBucketNum < mHashTable->mSize && mHashTable->emptyBucket(mBucketNum))
                mBucketNum++;
    ...
    OHashTable(const HashTableType &other) : mSize(0), mSizeMask(0), mUsed(0), mElements(NULL), mNextOrder(1)
    {
        copyFromOther(other);
    }
    ...
    template <typename Inputiterator> void insert(Inputiterator it1, Inputiterator it2)
    {
        while (it1 != it2)
        {
