...
//-----------------------------------------------------------------------------
//
// $Id: 02b9703b5fbe2acea6bcfb75ad88652cd7c4b4cd $
//
...
static void ParseDefine(OScanner &os)
{
        ...
        else if (os.compareToken("weapons"))
        {
            ...
            for (;;)
            {
                ...
                if (weapon == wp_none)
                {
                    ...
                    {
                        std::string buffer;
                        StrFormat(buffer, "Unknown weapon \"%s\".", os.getToken().c_str());
                        os.error(buffer.c_str());
                    }
                }
    ...
    {
        StrFormat(buf, "Minimum group health for define \"%s\" was not set.", define.name.c_str());
        os.error(buf.c_str());
    }
    if (define.maxGroupHealth <= 0)
    {
        StrFormat(buf, "Maximum group health for define \"%s\" was not set.", define.name.c_str());
        os.error(buf.c_str());
    }
    if (define.minGroupHealth > define.maxGroupHealth)
    {
        StrFormat(buf, "Maximum group health for define \"%s\" is less than minimum.", define.name.c_str());
        os.error(buf.c_str());
    }
    if (define.minBossHealth < 0)
    {
        StrFormat(buf, "Minimum boss health for define \"%s\" was not set.", define.name.c_str());
        os.error(buf.c_str());
    }
    if (define.maxBossHealth <= 0)
    {
        StrFormat(buf, "Maximum boss health for define \"%s\" was not set.", define.name.c_str());
        os.error(buf.c_str());
    }
    if (define.minBossHealth > define.maxBossHealth)
    {
        StrFormat(buf, "Maximum boss health for define \"%s\" is less than minimum.", define.name.c_str());
        os.error(buf.c_str());
    }
...
struct FindHordeDef
{
    ...
    bool operator()(const hordeDefine_t &def)
    {
...
static void ParseHordeDefs()
{
    ...
    for (std::vector<hordeDefine_t>::iterator it = ::WAVE_DEFINES.begin(); it != ::WAVE_DEFINES.end(); ++it)
    {
        ...
        std::vector<hordeDefine_t>::iterator found = std::find_if(after, ::WAVE_DEFINES.end(), FindHordeDef(it->name));
