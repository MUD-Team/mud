...
bool P_CrossCompatibleSpecialLine(line_t *line, int32_t side, AActor *thing, bool bossaction)
{
    ...
    */
    ...
    case 52:
        ...
        if (bossaction || ((!(thing->player && thing->player->health <= 0)) && CheckIfExitIsGood(thing)))
        {
            G_ExitLevel(0, 1);
            return true;
        }
    ...
    case 125:
        ...
        if (!thing->player && thing->type != MT_AVATAR && (EV_LineTeleport(line, side, thing)))
        {
            return true;
            // line->special = 0;
        }
    ...
    default:
        ...
        case 227:
            ...
            if (EV_DoElevator(line, DElevator::elevateUp, SPEED(ELEVATORSPEED), 0, line->id))
            {
                return true;
                // line->special = 0;
            }
        ...
        case 231:
            ...
            if (EV_DoElevator(line, DElevator::elevateDown, SPEED(ELEVATORSPEED), 0, line->id))
            {
                return true;
                // line->special = 0;
            }
        ...
        case 235:
            ...
            if (EV_DoElevator(line, DElevator::elevateCurrent, SPEED(ELEVATORSPEED), 0, line->id))
            {
                return true;
                // line->special = 0;
            }
        ...
        case 264: // jff 4/14/98 add monster-only silent line-line reversed
            if (!thing->player && thing->type != MT_AVATAR && EV_SilentLineTeleport(line, side, thing, line->id, true))
            {
                return true;
                // line->special = 0;
            }
        ...
        case 266: // jff 4/14/98 add monster-only silent line-line
            if (!thing->player && thing->type != MT_AVATAR && EV_SilentLineTeleport(line, side, thing, line->id, false))
            {
                return true;
                // line->special = 0;
            }
        ...
        case 232:
            ...
            EV_DoElevator(line, DElevator::elevateDown, SPEED(ELEVATORSPEED), 0, line->id);
            return true;

        case 236:
            // Elevator to current floor
            // 236 WR Elevator to current floor
            EV_DoElevator(line, DElevator::elevateCurrent, SPEED(ELEVATORSPEED), 0, line->id);
            return true;
...
bool P_ActorInCompatibleSector(AActor *actor)
{
    ...
    if (sector && sector->special & KILL_MONSTERS_MASK && actor->z == actor->floorz && !actor->player &&
        actor->flags & MF_SHOOTABLE && !(actor->flags & MF_FLOAT))
    {
...
void P_SpawnCompatibleExtra(int32_t i)
{
    ...
    case OdamexStaticInits: // Gravity
        grav = ((float)P_AproxDistance(lines[i].dx, lines[i].dy)) / (FRACUNIT * 100.0f);
...
bool P_UseCompatibleSpecialLine(AActor *thing, line_t *line, int32_t side, bool bossaction)
{
    ...
    else if ((uint32_t)line->special >= GenFloorBase)
    {
        if (!thing->player && thing->type != MT_AVATAR && !bossaction)
            if ((line->special & FloorChange) || !(line->special & FloorModel))
                return false;                        // FloorModel is "Allow Monsters" if FloorChange is 0
        if (!line->id && ((line->special & 6) != 6)) // e6y //jff 2/27/98 all non-manual
    ...
    else if ((uint32_t)line->special >= GenDoorBase)
    {
        ...
        if (!line->id && ((line->special & 6) != 6)) // e6y //jff 3/2/98 all non-manual
            return false;                            // generalized types require tag
        linefunc = EV_DoGenDoor;
    }
    else if ((uint32_t)line->special >= GenLockedBase)
