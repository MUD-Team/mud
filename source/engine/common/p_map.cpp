...
//-----------------------------------------------------------------------------
//
// $Id: a3b3ddf44281735418adea860688a07154a77bf5 $
//
...
BOOL P_TeleportMove(AActor *thing, fixed_t x, fixed_t y, fixed_t z, BOOL telefrag)
{
    ...
    StompAlwaysFrags =
        tmthing->player || tmthing->type == MT_AVATAR || (level.flags & LEVEL_MONSTERSTELEFRAG) || telefrag;
        ...
        for (m = mo->touching_sectorlist; m; m = m->m_tnext)
            if ((sec = m->m_sector)->flags & SECF_FRICTION && (sec->friction < friction || friction == ORIG_FRICTION) &&
                (mo->z <= P_FloorHeight(mo) ||
                 (sec->heightsec && !(sec->heightsec->MoreFlags & SECF_IGNOREHEIGHTSEC) && mo->z <= P_FloorHeight(mo))))
    ...
    PIT_CheckLine(line_t *ld)
{
    ...
    if (!(tmthing->flags & MF_DROPOFF) && !(tmthing->flags & (MF_NOGRAVITY | MF_NOCLIP)))
    {
        if (ld->frontsector->floorplane.c < STEEPSLOPE || ld->backsector->floorplane.c < STEEPSLOPE)
        {
    ...
    if (P_IsPlaneLevel(&ld->frontsector->floorplane) && P_IsPlaneLevel(&ld->backsector->floorplane) &&
        P_IsPlaneLevel(&ld->frontsector->ceilingplane) && P_IsPlaneLevel(&ld->backsector->ceilingplane))
    {
    ...
    else
    {
        ...
        double r  = (FIXED2DOUBLE(tmx - ld->v1->x) * dx + FIXED2DOUBLE(tmy - ld->v1->y) * dy) / (dx * dx + dy * dy);
    ...
    else if (projectile->target && projectile->target->player && player->player)
    {
        if (sv_friendlyfire)
        {
            return true; // Always clip if friendly fire is on.
                 ...
                 (projectile->target->player->userinfo.team == player->player->userinfo.team && G_IsTeamGame()))
        {
...
static bool P_ProjectileImmune(AActor *target, AActor *source)
{
    return (  // PG_GROUPLESS means no immunity, even to own species
               mobjinfo[target->type].projectile_group != PG_GROUPLESS || target == source) &&
           (( // target type has default behaviour, and things are the same type
                mobjinfo[target->type].projectile_group == PG_DEFAULT && source->type == target->type) ||
...
static BOOL PIT_CheckThing(AActor *thing)
{
    ...
    if ((thing->player && thing->player->spectator) || (tmthing->player && tmthing->player->spectator))
        return true;
            ...
            for (int by = yl; by <= yh; by++)
            {
                ...
                do
                {
                    ...
                    {     // [RH] If a thing can be stepped up on, we need to continue checking
                        ...
                        else if (thing->player && thing->z + thing->height - BlockingMobj->z <= 24 * FRACUNIT)
                        {
                            if (thingblocker)
               ...
               bool onfloor)                     // [RH] Let P_TryMove keep the thing on the floor
{
    ...
    if (!P_CheckPosition(thing, x, y))
    {
        ...
        if ((!BlockingMobj || BlockingMobj->player || !thing->player) ||
            (BlockingMobj->z + BlockingMobj->height - testz > (24 * FRACUNIT) ||
             (P_CeilingHeight(x, y, BlockingMobj->subsector->sector) - (BlockingMobj->z + BlockingMobj->height) <
              thing->height) ||
    ...
    if (!(thing->flags & MF_NOCLIP) && !(thing->player && thing->player->spectator))
    {
        ...
        if (!(thing->flags & MF_TELEPORT) && tmceilingz - testz < thing->height && !(thing->flags2 & MF2_FLY))
        {
        ...
        if (thing->flags & MF_BOUNCES && // killough 8/13/98
            !(thing->flags & (MF_MISSILE | MF_NOGRAVITY)) && !sentient && tmfloorz - thing->z > 16 * FRACUNIT)
...
BOOL P_ThingHeightClip(AActor *thing)
{
    ...
    fixed_t newz = (onthing) ? underthing->z + underthing->height : thing->floorz;
...
bool P_CheckSlopeWalk(AActor *actor, fixed_t &xmove, fixed_t &ymove)
{
    ...
    fixed_t t     = FixedMul(plane->a, destx) + FixedMul(plane->b, desty) + FixedMul(plane->c, actor->z) + plane->d;
...
void P_HitSlideLine(line_t *ld)
{
    ...
    icyfloor =
        slidemo->player &&
        !slidemo->player
             ->spectator && // Ch0wW: disables bouncing as a flying spectator. (prevents grunt1 sound from playing)
...
BOOL PTR_AimTraverse(intercept_t *in)
{
    ...
    if (in->isaline)
    {
        ...
        if (P_FloorHeight(crossx, crossy, li->frontsector) != P_FloorHeight(crossx, crossy, li->backsector))
        {
        ...
        if (P_CeilingHeight(crossx, crossy, li->frontsector) != P_CeilingHeight(crossx, crossy, li->backsector))
        {
    ...
    if ((sv_gametype == GM_CTF || sv_gametype == GM_TEAMDM) && shootthing->player && th->player &&
        shootthing->player->userinfo.team == th->player->userinfo.team && !sv_friendlyfire)
        return true;
...
BOOL PTR_UseTraverse(intercept_t *in)
{
    ...
    {
        P_LineOpening(in->d.line, trace.x + FixedMul(trace.dx, in->frac), trace.y + FixedMul(trace.dy, in->frac));

        if (openrange <= 0)
        {
            // [RH] Give sector a chance to intercept the use
            sector_t *sec = in->d.line->frontsector;
            if ((!sec->SecActTarget || !A_TriggerAction(sec->SecActTarget, usething, SECSPAC_Use | SECSPAC_UseWall)) &&
                usething->player)
            {
    ...
    if (map_format.getZDoom())
    {
        donteatuse = ((in->d.line->flags & ML_SPAC_USE) ||
                      (!(in->d.line->flags & ML_SPAC_CROSSTHROUGH) && (!(in->d.line->flags & ML_SPAC_USETHROUGH))))
                         ? false
                         : true;
    }
    else
...
static BOOL PIT_DoomRadiusAttack(AActor *thing)
{
    ...
    if ((thing->type == MT_CYBORG && bombsource->type == MT_CYBORG) ||
        (thing->flags3 & MF3_NORADIUSDMG || thing->flags2 & MF2_BOSS) && !(bombspot->flags3 & MF3_FORCERADIUSDMG))
        return true;
    ...
    if (dist >= bombdistance)
    {
        ...
        {
            M_LogWDLEvent(WDL_EVENT_PROJACCURACY, bombsource->player, NULL, bombsource->player->mo->angle / 4, bombmod,
                          0, GetMaxShotsForMod(bombmod));
        }
...
static BOOL PIT_ZDoomRadiusAttack(AActor *thing)
{
    ...
    if (bombspot->type == MT_BARREL || thing->type == MT_BARREL || thing->type == MT_BOSSBRAIN)
    {
...
BOOL PIT_ChangeSector(AActor *thing)
{
    ...
    if (crushchange > 0 && !(level.time & 3))
    {
        ...
        if (!(thing->flags & MF_NOBLOOD))
        {
            AActor *mo = new AActor(thing->x, thing->y, thing->z + thing->height / 2, MT_BLOOD);
...
v3fixed_t P_LinePlaneIntersection(const plane_t *plane, const v3fixed_t &lineorg, const v3fixed_t &linedir)
{
    ...
    fixed_t numer =
        -plane->d - FixedMul(plane->a, lineorg.x) - FixedMul(plane->b, lineorg.y) - FixedMul(plane->c, lineorg.z);

    fixed_t denom = FixedMul(plane->a, linedir.x) + FixedMul(plane->b, linedir.y) + FixedMul(plane->c, linedir.z);

    if (!denom) // line is parallel to the surface of plane
        return pt;

    fixed_t t = FixedDiv(numer, denom);
    M_SetVec3Fixed(&pt, lineorg.x + FixedMul(t, linedir.x), lineorg.y + FixedMul(t, linedir.y),
...
bool P_PointOnPlane(const plane_t *plane, fixed_t x, fixed_t y, fixed_t z)
{
    ...
    fixed_t result = (FixedMul(plane->a, x) + FixedMul(plane->b, y) + FixedMul(plane->c, z) + plane->d);
...
bool P_IdenticalPlanes(const plane_t *pl1, const plane_t *pl2)
{
    ...
    return (pl1 == pl2) || (pl1->a == pl2->a && pl1->b == pl2->b && pl1->c == pl2->c && pl1->d == pl2->d);
}
...
VERSION_CONTROL(p_map_cpp, "$Id: a3b3ddf44281735418adea860688a07154a77bf5 $")
