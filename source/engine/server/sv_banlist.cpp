...
//-----------------------------------------------------------------------------
//
// $Id: c271fe68ce528aebba98e9056a6594ffdb4b8a7b $
//
...
bool Banlist::add(const std::string &address, const time_t expire, const std::string &name, const std::string &reason)
{
    Ban ban;
...
bool Banlist::add(player_t &player, const time_t expire, const std::string &reason)
{
...
bool Banlist::check(const netadr_t &address, Ban &baninfo)
{
    ...
    for (std::vector<Exception>::iterator it = this->exceptionlist.begin(); it != this->exceptionlist.end(); ++it)
    {
    ...
    for (std::vector<Ban>::iterator it = this->banlist.begin(); it != this->banlist.end(); ++it)
    {
        if (it->range.check(address) && (it->expire == 0 || it->expire > time(NULL)))
        {
            baninfo = *it;
            return true;
        }
...
bool Banlist::query(const std::string &query, banlist_results_t &result)
{
    ...
    for (size_t i = 0; i < this->banlist.size(); i++)
    {
        ...
        bool f_name = CheckWildcards(pattern.c_str(), this->banlist[i].name.c_str());
        if (f_ip || f_name)
        {
            result.push_back(banlist_result_t(i, &(this->banlist[i])));
        }
...
bool Banlist::query_exception(const std::string &query, exceptionlist_results_t &result)
{
    ...
    for (size_t i = 0; i < this->exceptionlist.size(); i++)
    {
        ...
        bool f_name = CheckWildcards(pattern.c_str(), this->exceptionlist[i].name.c_str());
        if (f_ip || f_name)
        {
            result.push_back(exceptionlist_result_t(i, &(this->exceptionlist[i])));
        }
...
BEGIN_COMMAND(banlist)
{
    ...
    for (banlist_results_t::iterator it = result.begin(); it != result.end(); ++it)
    {
        std::ostringstream buffer;
...
BEGIN_COMMAND(loadbanlist)
{
    ...
    if (bansize == jsonsize)
        ...
        Printf(PRINT_HIGH, "loadbanlist: loaded %d bans and skipped %d invalid entries from %s.", bansize,
               jsonsize - bansize, banfile.c_str());
}
...
BEGIN_COMMAND(exceptionlist)
{
    ...
    for (exceptionlist_results_t::iterator it = result.begin(); it != result.end(); ++it)
    {
        std::ostringstream buffer;
        buffer << it->first + 1 << ". " << it->second->range.string();
...
void SV_InitBanlist()
{
    ...
    if (bansize == jsonsize)
        ...
        Printf(PRINT_HIGH, "SV_InitBanlist: Loaded %d bans and skipped %d invalid entries from %s.", bansize,
               jsonsize - bansize, banfile);
}
