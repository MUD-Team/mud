...
//-----------------------------------------------------------------------------
//
// $Id: 201e3f5b17959c25d460f1bb876f0615a8973001 $
//
...
class MapVote : public Vote
{
  ...
  public:
    ...
    bool setup(const std::vector<std::string> &args, const player_t &player)
    {
        ...
        for (std::vector<std::string>::const_iterator it = maplist_entry.wads.begin(); it != maplist_entry.wads.end();
             ++it)
...
bool Vote::init(const std::vector<std::string> &args, const player_t &player)
{
    ...
    if (player.timeout_callvote > 0)
    {
        ...
        {
            std::ostringstream buffer;
            buffer << "Please wait another " << timeout_waitsec << " second" << ((timeout_waitsec != 1) ? "s" : "")
                   << " to call a vote.";
            this->error = buffer.str();
            return false;
        }
    ...
    if (this->tally.find(player.id) == this->tally.end())
    {
        ...
        else
        {
            SV_PlayerPrintf(PRINT_HIGH, player.id,
                            "You can't vote on something that was called before you joined the server.\n");
        }
        return false;
    }
...
void SV_VoteCmd(player_t &player, const std::vector<std::string> &args)
{
    if (args.size() < 2)
    {
        SV_PlayerPrintf(PRINT_HIGH, player.id, "Invalid vote, must be \"yes\" or \"no\".\n");
        return;
    }
