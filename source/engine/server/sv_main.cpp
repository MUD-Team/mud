...
//-----------------------------------------------------------------------------
//
// $Id: b6987a9fb0ea75f38a8a58e2b5b490c8f10db334 $
//
    ...
    while (it != players.end())
    {
        if (count <= 0)
        {
            MSG_WriteSVC(&it->client.reliablebuf,
                         SVC_Print(PRINT_CHAT, "Client limit reduced. Please try connecting again later.\n"));
    ...
    for (Players::iterator it = players.begin(); it != players.end(); ++it)
    {
        ...
        if (!spectator)
        {
            ...
            if (normalcount > var)
            {
                ...
                {
                    MSG_WriteSVC(&pit->client.reliablebuf, SVC_PlayerMembers(*it, SVC_PM_SPECTATOR));
                }
                ...
                MSG_WriteSVC(&it->client.reliablebuf,
                             SVC_Print(PRINT_CHAT, "Active player limit reduced. You are now a spectator!\n"));
            }
        }
    }
...
CVAR_FUNC_IMPL(sv_maxrate)
{
    ...
    for (Players::iterator it = players.begin(); it != players.end(); ++it)
        it->client.rate = int32_t(sv_maxrate);
}
...
void SV_InvalidateClient(player_t &player, const std::string &reason)
{
    ...
    SV_PlayerPrintf(PRINT_ERROR, player.id, "The server closed your connection for the following reason: %s.\n",
                    reason.c_str());
    SV_DropClient(player);
}
...
void SV_MidPrint(const char *msg, player_t *p, int32_t msgtime)
{
...
void SV_Sound(AActor *mo, uint8_t channel, const char *name, uint8_t attenuation)
{
    ...
    for (Players::iterator it = players.begin(); it != players.end(); ++it)
    {
        ...
        MSG_WriteSVC(&cl->reliablebuf, SVC_PlaySound(PlaySoundType(mo), channel, sfx_id, 1.0f, attenuation));
    }
}
...
//
void SV_SoundTeam(uint8_t channel, const char *name, uint8_t attenuation, int32_t team)
{
    int32_t sfx_id;
    ...
    for (Players::iterator it = players.begin(); it != players.end(); ++it)
    {
        if (it->ingame() && it->userinfo.team == team)
        {
            ...
            MSG_WriteSVC(&cl->reliablebuf, SVC_PlaySound(PlaySoundType(), channel, sfx_id, 1.0f, attenuation));
        }
    }
}
...
void SV_Sound(fixed_t x, fixed_t y, uint8_t channel, const char *name, uint8_t attenuation)
{
    ...
    for (Players::iterator it = players.begin(); it != players.end(); ++it)
    {
        ...
        MSG_WriteSVC(&cl->reliablebuf, SVC_PlaySound(PlaySoundType(x, y), channel, sfx_id, 1.0f, attenuation));
    }
}
...
bool SV_SetupUserInfo(player_t &player)
{
    ...
    if (validplayer(other))
    {
        ...
        if (player.id != other.id)
        {
            ...
            for (int32_t num = 2; num < MAXPLAYERS + 2; num++)
            {
    ...
    if (!old_netname.empty() && !iequals(new_netname, old_netname))
    {
        ...
        {
            M_HandleWDLNameChange(team, old_netname.c_str(), player.userinfo.netname.c_str(), player.id);
        }
    }
    ...
    if (G_IsTeamGame())
    {
        ...
        if (player.mo && player.userinfo.team != old_team && player.ingame() && !player.spectator &&
            !G_IsLevelState(LevelState::WARMUP))
        {
            // kill player if team is changed
            P_DamageMobj(player.mo, 0, 0, 1000, 0);
            M_LogWDLEvent(WDL_EVENT_DISCONNECT, &player, NULL, old_team, M_GetPlayerId(&player, old_team), 0, 0);
...
void SV_ForceSetTeam(player_t &who, team_t team)
{
    ...
    Printf(PRINT_HIGH, "Forcing %s to %s team\n", who.userinfo.netname.c_str(),
           team == TEAM_NONE ? "NONE" : V_GetTeamColor(team).c_str());
...
team_t SV_GoodTeam(void)
{
    int32_t teamcount = sv_teamsinplay;
...
bool SV_AwarenessUpdate(player_t &player, AActor *mo)
{
    ...
    else if (previously_ok && ok && player.mo && !mo->player && mo->type != MT_AVATAR && mo->type != MT_PLAYER &&
             mo->on_conveyor && (gametic + mo->netid) % 120) // Update items on conveyers every 120 tics
    {
        SV_SendMobjToClient(mo, cl);
        return true;
    }
...
void SV_UpdateSector(client_t *cl, int32_t sectornum)
{
...
{
    for (int32_t sectornum = 0; sectornum < numsectors; sectornum++)
    {
...
void SV_SendMovingSectorUpdate(player_t &player, sector_t *sector)
{
    ...
    int32_t sectornum = sector - sectors;
...
{
    for (int32_t lineNum = 0; lineNum < numlines; lineNum++)
    {
        ...
        for (int32_t sideNum = 0; sideNum < 2; sideNum++)
        {
...
bool SV_CheckClientVersion(client_t *cl, Players::iterator it)
{
    ...
    if (!AllowConnect)
    {
        ...
        Printf("%s disconnected (version mismatch %s).\n", NET_AdrToString(::net_from), VersionStr.c_str());
    }
...
static void SV_DisconnectOldClient()
{
    ...
    Printf("%s disconnected (version mismatch %d.%d.%d).\n", NET_AdrToString(::net_from), cl_maj, cl_min, cl_pat);
}
...
{
    int32_t challenge = MSG_ReadLong();
    ...
    if (strlen(join_password.cstring()) && MD5SUM(join_password.cstring()) != passhash)
    {
        ...
        MSG_WriteSVC(&cl->reliablebuf,
                     SVC_Print(PRINT_HIGH, "Server is passworded, no password specified or bad password.\n"));
...
void SV_ConnectClient2(player_t &player)
{
    ...
    if (!step_mode)
    {
        ...
        for (Players::iterator pit = players.begin(); pit != players.end(); ++pit)
        {
            MSG_WriteSVC(&pit->client.reliablebuf, SVC_PlayerMembers(player, SVC_PM_SPECTATOR));
        }
    }
...
void SV_DrawScores()
{
    ...
    if (sv_gametype == GM_TEAMDM)
    {
        ...
        for (int32_t team_num = 0; team_num < sv_teamsinplay; team_num++)
        {
            ...
            for (PlayerPtrList::const_iterator it = sortedplayers.begin(); it != sortedplayers.end(); ++it)
            {
                ...
                {
                    Printf_Bold("%-3d %-16s %-15s %-5d %-6d %2.1f %-3d", itplayer->id,
                                NET_AdrToString(itplayer->client.address), itplayer->userinfo.netname.c_str(),
                                P_GetFragCount(itplayer), P_GetDeathCount(itplayer),
                                SV_CalculateFragDeathRatio(itplayer), itplayer->GameTime / 60);
                }
            }
        }
    }
    ...
    else if (sv_gametype == GM_DM)
    {
        ...
        for (PlayerPtrList::const_iterator it = sortedplayers.begin(); it != sortedplayers.end(); ++it)
        {
            ...
            Printf_Bold("%-3d %-16s %-15s %-5d %-6d %2.1f %-3d", itplayer->id,
                        NET_AdrToString(itplayer->client.address), itplayer->userinfo.netname.c_str(),
                        P_GetFragCount(itplayer), P_GetDeathCount(itplayer), SV_CalculateFragDeathRatio(itplayer),
    ...
    else if (G_IsCoopGame())
    {
        ...
        for (PlayerPtrList::const_iterator it = sortedplayers.begin(); it != sortedplayers.end(); ++it)
        {
            ...
            Printf_Bold("%-3d %-16s %-15s %-5d %-6d %2.1f %-3d", itplayer->id,
                        NET_AdrToString(itplayer->client.address), itplayer->userinfo.netname.c_str(),
                        itplayer->killcount, itplayer->deathcount, SV_CalculateKillDeathRatio(itplayer),
                        itplayer->GameTime / 60);
        }
    }
    ...
    if (!sortedspectators.empty())
    {
        ...
        {
            const player_t *itplayer = *it;
            Printf_Bold("%-3d %-16s %-15s\n", itplayer->id, NET_AdrToString(itplayer->client.address),
...
void STACK_ARGS SV_BroadcastPrintf(int32_t printlevel, const char *format, ...)
{
    ...
    for (Players::iterator it = players.begin(); it != players.end(); ++it)
    {
        ...
        MSG_WriteSVC(&cl->reliablebuf, SVC_Print(static_cast<printlevel_t>(printlevel), string));
    }
}
...
void STACK_ARGS SV_PlayerPrintf(int32_t level, int32_t player_id, const char *fmt, ...)
{
    va_list argptr;
    char    string[2048];
...
void STACK_ARGS SV_TeamPrintf(int32_t level, int32_t who, const char *fmt, ...)
{
    va_list argptr;
    char    string[2048];
...
void SV_UpdateMissiles(player_t &pl)
{
    ...
    while ((mo = iterator.Next()))
    {
        ...
        if (((gametic + mo->netid) % 30) && (mo->type != MT_TRACER) && (mo->type != MT_FATSHOT) &&
            !(mo->flags2 & MF2_SEEKERMISSILE))
            continue;
        // Revenant tracers and Mancubus fireballs need to be updated more often (and custom tracers)
        else if (((gametic + mo->netid) % 5) &&
                 (mo->type == MT_TRACER || mo->type == MT_FATSHOT || mo->flags2 & MF2_SEEKERMISSILE))
            continue;
...
//
int32_t SV_CalculateNumTiccmds(player_t &player)
{
...
{
    const int32_t max_forward_move = 50 << 8;
#if 0
	const int32_t max_sr40_side_move = 40 << 8;
#endif
    const int32_t max_sr50_side_move = 50 << 8;
    ...
    int32_t num_cmds = SV_CalculateNumTiccmds(player);

    for (int32_t i = 0; i < num_cmds && !player.cmdqueue.empty(); i++)
    {
...
void SV_ChangeTeam(player_t &player) // [Toke - Teams]
{
    ...
    if (G_IsTeamGame() && player.mo && player.userinfo.team != old_team && !G_IsLevelState(LevelState::WARMUP))
    {
        P_DamageMobj(player.mo, 0, 0, 1000, 0);

        M_LogWDLEvent(WDL_EVENT_DISCONNECT, &player, NULL, old_team, M_GetPlayerId(&player, old_team), 0, 0);
        M_LogWDLEvent(WDL_EVENT_JOINGAME, &player, NULL, team, M_GetPlayerId(&player, team), 0, 0);
    }
...
void SV_JoinPlayer(player_t &player, bool silent)
{
    ...
    if (G_IsTeamGame())
    {
        ...
        bool toomanyplayers = sv_maxplayersperteam && P_NumPlayersOnTeam(player.userinfo.team) >= sv_maxplayersperteam;
...
void SV_SpecPlayer(player_t &player, bool silent)
{
    ...
    for (Players::iterator it = ::players.begin(); it != ::players.end(); ++it)
    {
        MSG_WriteSVC(&it->client.reliablebuf, SVC_PlayerMembers(player, SVC_PM_SPECTATOR));
    }
...
void SV_SetReady(player_t &player, bool setting, bool silent)
{
    ...
    if (changed)
    {
        ...
        for (Players::iterator it = players.begin(); it != players.end(); ++it)
        {
            MSG_WriteSVC(&it->client.reliablebuf, SVC_PlayerMembers(player, SVC_PM_READY));
        }
    }
...
static void ReadyCmd(player_t &player)
{
    ...
    {
        int32_t timeout = level.time - player.timeout_ready;
...
void SV_RConPassword(player_t &player)
{
    ...
    {
        Printf(PRINT_HIGH, "RCON login failure from %s - %s", player.userinfo.netname.c_str(),
...
void SV_Cheat(player_t &player)
{
    ...
    if (cheatType == 0)
    {
        ...
        int32_t oldCheats = player.cheats;
...
void SV_WantWad(player_t &player)
{
    ...
    MSG_WriteSVC(&cl->reliablebuf, SVC_Print(PRINT_HIGH, "Server: Downloading is disabled\n"));
...
void SV_ParseCommands(player_t &player)
{
    while (validplayer(player))
    {
        ...
        case clc_rcon: {
            ...
            {
                Printf(PRINT_HIGH, "RCON command from %s - %s -> %s", player.userinfo.netname.c_str(),
                       NET_AdrToString(net_from), str.c_str());
        ...
        case clc_kill:
            if (player.mo && player.suicidedelay == 0 && gamestate == GS_LEVEL && (sv_allowcheats || G_IsCoopGame()))
            {
                SV_Suicide(player);
            }
            break;
...
void SV_RunTics()
{
    ...
    if (gamestate == GS_LEVEL && sv_emptyreset && players.empty() && last_player_count > 0)
    {
...
BEGIN_COMMAND(playerinfo)
{
    ...
    char ip[16];
    sprintf(ip, "%d.%d.%d.%d", player->client.address.ip[0], player->client.address.ip[1], player->client.address.ip[2],
            player->client.address.ip[3]);
    ...
    {
        Printf(" frags - %d  deaths - %d  points - %d\n", player->fragcount, player->deathcount, player->points);
    }
    if (g_lives)
    {
...
BEGIN_COMMAND(playerlist)
{
    ...
    for (Players::reverse_iterator it = players.rbegin(); it != players.rend(); ++it)
    {
        ...
        StrFormat(strMain, "(%02d): %s %s - %s - time:%d - ping:%d", it->id, it->userinfo.netname.c_str(),
                  it->spectator ? "(SPEC)" : "", NET_AdrToString(it->client.address), it->GameTime, it->ping);
        ...
        if (G_IsCoopGame())
        {
            ...
            {
                // Kills and Deaths
                StrFormat(strScore, " - kills:%d - deaths:%d", it->killcount, it->deathcount);
            }
        }
        else if (sv_gametype == GM_DM)
        {
            if (G_IsLivesGame())
            {
                ...
                StrFormat(strScore, " - wins:%d - lives:%d - frags:%d", it->roundwins, it->lives, frags);
            }
            else
            {
                // Frags, Deaths
...
void SV_SendDamagePlayer(player_t *player, AActor *inflictor, int32_t healthDamage, int32_t armorDamage)
{
    for (Players::iterator it = players.begin(); it != players.end(); ++it)
    {
        ...
        MSG_WriteSVC(&cl->reliablebuf, SVC_DamagePlayer(*player, inflictor, healthDamage, armorDamage));
    }
}
...
void SV_SendKillMobj(AActor *source, AActor *target, AActor *inflictor, bool joinkill)
{
    ...
    for (Players::iterator it = players.begin(); it != players.end(); ++it)
    {
        ...
        MSG_WriteSVC(&cl->reliablebuf, SVC_KillMobj(source, target, inflictor, ::MeansOfDeath, joinkill));
    }
}
...
void SV_UpdatePlayerQueueLevelChange(const WinInfo &win)
{
    if (::g_winnerstays)
    {
        ...
        for (PlayersView::iterator it = loserPlayers.begin(); it != loserPlayers.end(); ++it)
        {
        ...
        {
            SV_BroadcastPrintf("%s and %s lost the last game and were forced to spectate.\n", names.at(0).c_str(),
                               names.at(1).c_str());
        }
...
void SV_UpdatePlayerQueuePositions(JoinTest joinTest, player_t *disconnectPlayer)
{
    ...
    {
        for (PlayersView::iterator it = queueUpdates.begin(); it != queueUpdates.end(); ++it)
        {
                               ...
                               int32_t arg4)
{
    ...
    for (Players::iterator it = players.begin(); it != players.end(); ++it)
    {
        ...
        int32_t args[5] = {arg0, arg1, arg2, arg3, arg4};
        MSG_WriteSVC(&cl->reliablebuf, SVC_ExecuteLineSpecial(special, line, activator, args));
    }
}
                          ...
                          const std::vector<int32_t> &args)
{
    ...
    for (Players::iterator it = players.begin(); it != players.end(); ++it)
    {
        ...
        MSG_WriteSVC(&cl->reliablebuf, SVC_ExecuteACSSpecial(special, activator, print, args));
    }
}
...
void SV_ShareKeys(card_t card, player_t &player)
{
    ...
    if (item = FindCardItem(card))
    {
        ...
        SV_BroadcastPrintf("%s found the %s%s%s!\n", player.userinfo.netname.c_str(), coloritem, item->pickup_name,
                           TEXTCOLOR_NORMAL);
    }
    else
    {
...
VERSION_CONTROL(sv_main_cpp, "$Id: b6987a9fb0ea75f38a8a58e2b5b490c8f10db334 $")
