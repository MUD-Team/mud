...
void Maplist::shuffle()
{
    ...
    std::shuffle(this->s_maplist.begin(), this->s_maplist.end(), random_shuffler);
...
bool Maplist::insert(const size_t &position, maplist_entry_t &maplist_entry)
{
    ...
    if (maplist_entry.wads.empty())
    {
        ...
        else
        {
            ...
            std::copy(this->maplist.at(position - 1).wads.begin(), this->maplist.at(position - 1).wads.end(),
...
bool Maplist::query(const std::vector<std::string> &query, std::vector<std::pair<size_t, maplist_entry_t *>> &result)
{
    // There's no maplist to query.
    if (this->maplist.empty())
    {
...
BEGIN_COMMAND(maplist)
{
    ...
    for (std::vector<std::pair<size_t, maplist_entry_t *>>::iterator it = result.begin(); it != result.end(); ++it)
    {
        char flag = ' ';
        ...
        Printf(PRINT_HIGH, "%c%d. %s %s\n", flag, it->first + 1, JoinStrings(it->second->wads, " ").c_str(),
               it->second->map.c_str());
    }
}
END_COMMAND(maplist)
...
BEGIN_COMMAND(addmap)
{
    ...
    Printf(PRINT_HIGH, "Adding %s to maplist (WAD%s : %s)", arguments[0].c_str(), (arguments.size() > 2) ? "s" : "",
...
BEGIN_COMMAND(insertmap)
{
    ...
    // Successfully warn the server a map has been added.
    Printf(PRINT_HIGH, "Successfully inserting %s to position #%s (WAD%s : %s)", arguments[1].c_str(),
           arguments[0].c_str(), (arguments.size() > 3) ? "s" : "", JoinStrings(maplist_entry.wads, " ").c_str());
}
...
BEGIN_COMMAND(setlobbymap)
{
    if (argc < 2)
    {
        ...
        Printf(PRINT_HIGH, "There can only be one map with the lobby.\n");
        return;
    }
    ...
    Printf(PRINT_HIGH, "Setting %s as the Lobby map (WAD%s : %s)\n", arguments[0].c_str(),
           (arguments.size() > 2) ? "s" : "", JoinStrings(maplist_entry.wads, " ").c_str());
}
