...
#define QRYNEWINFO(INTRODUCED) if (EqProtocolVersion >= INTRODUCED || EqProtocolVersion >= PROTOCOL_VERSION)
...
#define QRYRANGEINFO(INTRODUCED, REMOVED) if (EqProtocolVersion >= INTRODUCED && EqProtocolVersion < REMOVED)
...
static void IntQryBuildInformation(const DWORD &EqProtocolVersion, const DWORD &EqTime)
{
    ...
    else MSG_WriteShort(&ml_message, timeleft);
    ...
    {
        MSG_WriteString(&ml_message, D_CleanseFileName(::wadfiles[i].getBasename(), "wad").c_str());
    ...
    for (Players::iterator it = players.begin(); it != players.end(); ++it)
    {
        ...
        spectator = (it->spectator || ((it->playerstate != PST_LIVE) && (it->playerstate != PST_DEAD) &&
                                ...
                                const WORD &TagPacketType)
{
    ...
    // Encode our tag
    ReTag = (((ReId << 20) & 0xFFF00000) | ((ReApplication << 16) & 0x000F0000) | ((ReQRId << 12) & 0x0000F000) |
             (RePacketType & 0x00000FFF));
...
VERSION_CONTROL(sv_sqp_cpp, "$Id: 8f283a99cc775396b0e74a5452865e9b97cd91ea $")
