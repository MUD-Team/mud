...
//-----------------------------------------------------------------------------
//
// $Id: da7f5940e1ab8c16540123f8f4ceedb057e600f9 $
//
...
void SV_FlagScore(player_t &player, team_t f)
{
    ...
    SV_BroadcastPrintf("%s has captured the %s flag (held for %s)\n", player.userinfo.netname.c_str(),
                       teamInfo->ColorizedTeamName().c_str(), CTF_TimeMSG(time_held));
...
void SV_SocketTouch(player_t &player, team_t f)
{
    ...
    {
        if (player.userinfo.team == f && player.userinfo.team != (team_t)i && player.flags[i] &&
            (!ctf_flagathometoscore || teamInfo->FlagData.state == flag_home))
        {
...
void SV_FlagDrop(player_t &player, team_t f)
{
    ...
    SV_BroadcastPrintf("%s has dropped the %s flag. (held for %s)\n", player.userinfo.netname.c_str(),
                       V_GetTeamColor(f).c_str(), CTF_TimeMSG(time_held));
...
void CTF_SpawnFlag(team_t f)
{
    ...
    AActor *flag =
        new AActor(teamInfo->FlagData.x, teamInfo->FlagData.y, teamInfo->FlagData.z, flag_table[f][flag_home]);
...
VERSION_CONTROL(sv_ctf_cpp, "$Id: da7f5940e1ab8c16540123f8f4ceedb057e600f9 $")
