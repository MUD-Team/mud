...
//-----------------------------------------------------------------------------
//
// $Id: e3d57347b6c2a8c394cbd1a729846834e5df0f7c $
//
...
BEGIN_COMMAND(callvote)
{
    ...
    case VOTE_MAP:
        ...
        MaplistCache::instance().defer_query(arguments, &CMD_MapVoteCallback, &CMD_MapVoteErrback);
        return;
    case VOTE_RANDMAP:
        // No arguments are necessary.
        arguments.resize(0);
        ...
        MaplistCache::instance().defer_query(arguments, &CMD_RandmapVoteCallback, &CMD_RandmapVoteErrback);
        return;
    default:
