 ...
 */
static void SetPNGComments(PNGStrings &out, png_struct *png_ptr, png_info *info_ptr, time_t *now)
{
    ...
    pngtext[text_line].text =
        (I_GetPrimarySurface()->getBitsPerPixel() == 8) ? (png_charp) "8bpp" : (png_charp) "32bpp";
    text_line++;
    ...
    pngtext[text_line].text = (vid_gammatype == 0) ? (png_charp) "Classic Doom" : (png_charp) "ZDoom";
    text_line++;
...
// see include/pnglibconf.h for libpng feature support macros
    ...
    png_set_IHDR(png_ptr, info_ptr, width, height,
                 ...
                 png_colortype, PNG_INTERLACE_NONE, PNG_COMPRESSION_TYPE_DEFAULT, PNG_FILTER_TYPE_DEFAULT);
