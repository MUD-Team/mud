...
//-----------------------------------------------------------------------------
//
// $Id: 2f197a5e300a7e949d4266bda10aba13dd0266c1 $
//
...
sector_t *R_FakeFlat(sector_t *sec, sector_t *tempsec, int32_t *floorlightlevel, int32_t *ceilinglightlevel, bool back)
{
    ...
    {
        *floorlightlevel = sec->floorlightsec == NULL ? sec->lightlevel : sec->floorlightsec->lightlevel;
    }
    ...
    if (ceilinglightlevel != NULL)
    {
        *ceilinglightlevel = sec->ceilinglightsec == NULL ? // killough 4/11/98
                                 sec->lightlevel
                                                          : sec->ceilinglightsec->lightlevel;
    }
    ...
    *tempsec = *sec;
        ...
        else if (s->MoreFlags & SECF_FAKEFLOORONLY)
        {
            if (underwater)
            {
                ...
                if (!(s->MoreFlags & SECF_NOFAKELIGHT))
                {
                    ...
                    if (floorlightlevel != NULL)
                    {
                        *floorlightlevel = s->floorlightsec == NULL ? s->lightlevel : s->floorlightsec->lightlevel;
                    }

                    if (ceilinglightlevel != NULL)
                    {
                        *ceilinglightlevel =
                            s->ceilinglightsec == NULL ? s->lightlevel : s->ceilinglightsec->lightlevel;
                    }
                }
    ...
    { // head-below-floor hack
        ...
        if (!(s->MoreFlags & SECF_NOFAKELIGHT))
        {
            ...
            if (ceilinglightlevel != NULL)
            {
                *ceilinglightlevel = s->ceilinglightsec == NULL ? s->lightlevel : s->ceilinglightsec->lightlevel;
            }
        }
        FakeSide = FAKED_BelowFloor;
    }
    else if (heightsec && viewz >= P_CeilingHeight(viewx, viewy, heightsec) && orgceilz > refceilz &&
             !(s->MoreFlags & SECF_FAKEFLOORONLY))
    { // Above-ceiling hack
        tempsec->ceilingplane = s->ceilingplane;
...
void R_AddLine(seg_t *line)
{
    ...
    if (!backsector || !(line->linedef->flags & ML_TWOSIDED) ||
        ...
        ((rw_backcz1 <= rw_backfz1 && rw_backcz2 <= rw_backfz2) &&
         ...
         ((rw_backcz1 >= rw_frontcz1 && rw_backcz2 >= rw_frontcz2) || line->sidedef->toptexture) &&

         ((rw_backfz1 <= rw_frontfz1 && rw_backfz2 <= rw_frontfz2) || line->sidedef->bottomtexture) &&
    ...
    if (P_IdenticalPlanes(&frontsector->ceilingplane, &backsector->ceilingplane) &&
        ...
        backsector->lightlevel == frontsector->lightlevel && backsector->floorpic == frontsector->floorpic &&
...
void R_Subsector(int32_t num)
{
    ...
    frontsector = R_FakeFlat(frontsector, &tempsec, &floorlightlevel, &ceilinglightlevel, false); // killough 4/11/98

    basecolormap = frontsector->colormap->maps;

    ceilingplane =
        P_CeilingHeight(camera) > viewz || frontsector->ceilingpic == skyflatnum ||
            ...
            ? R_FindPlane(frontsector->ceilingplane,               // killough 3/8/98
                          ...
                          frontsector->ceiling_yoffs + frontsector->base_ceiling_yoffs, frontsector->ceiling_xscale,
    ...
    floorplane = P_FloorHeight(camera) < viewz || // killough 3/7/98
                     ...
                     ? R_FindPlane(frontsector->floorplane,
                                   ...
                                   frontsector->floor_yoffs + frontsector->base_floor_yoffs, frontsector->floor_xscale,
                                   frontsector->floor_yscale, frontsector->floor_angle + frontsector->base_floor_angle)
                     : NULL;

    // [RH] set foggy flag
    foggy = level.fadeto_color[0] || level.fadeto_color[1] || level.fadeto_color[2] || level.fadeto_color[3] ||
            frontsector->colormap->fade.getr() || frontsector->colormap->fade.getg() ||
            frontsector->colormap->fade.getb();
...
VERSION_CONTROL(r_bsp_cpp, "$Id: 2f197a5e300a7e949d4266bda10aba13dd0266c1 $")
