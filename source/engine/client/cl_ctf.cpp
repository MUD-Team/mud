...
//-----------------------------------------------------------------------------
//
// $Id: fea4e1072fe603db55b89703edb9a195a2b143ad $
//
...
static mobjtype_t flag_table[NUMTEAMS][NUMFLAGSTATES] = {
    {MT_BFLG, MT_BDWN, MT_BCAR}, {MT_RFLG, MT_RDWN, MT_RCAR}, {MT_GFLG, MT_GDWN, MT_GCAR}};
...
void CTF_RunTics(void)
{
    ...
    for (size_t flag = 0; flag < NUMTEAMS; flag++)
    {
        ...
        if (teamInfo->FlagData.flagger == displayplayer().id && teamInfo->FlagData.state == flag_carried)
        {
...
FArchive &operator<<(FArchive &arc, flagdata &flag)
{
    ...
    arc << flag.flaglocated << netid << flag.flagger << flag.pickup_time << flag.x << flag.y << flag.z << flag.timeout
        << static_cast<byte>(flag.state) << flag.sb_tick;
...
FArchive &operator>>(FArchive &arc, flagdata &flag)
{
    ...
    arc >> flag.flaglocated >> netid >> flag.flagger >> flag.pickup_time >> flag.x >> flag.y >> flag.z >>
        flag.timeout >> state >> flag.sb_tick;
...
static const char *flag_sound[NUM_CTF_SCORE][7] = {
    ...
    {"", "", "", "", "", ""},                                                 // KILL
    {"", "", "", "", "", ""},                                                 // BETRAYAL
    {"ctf/your/flag/take", "ctf/enemy/flag/take", "vox/your/flag/take", "vox/enemy/flag/take", "vox/blue/flag/take",
     "vox/red/flag/take", "vox/green/flag/take"},                             // GRAB
    {"ctf/your/flag/take", "ctf/enemy/flag/take", "vox/your/flag/take", "vox/enemy/flag/take", "vox/blue/flag/take",
     "vox/red/flag/take", "vox/green/flag/take"},                             // FIRSTGRAB
    {"", "", "", "", "", ""},                                                 // CARRIERKILL
    {"ctf/your/flag/return", "ctf/enemy/flag/return", "vox/your/flag/return", "vox/enemy/flag/return",
     "vox/blue/flag/return", "vox/red/flag/return", "vox/green/flag/return"}, // RETURN
    {"ctf/your/score", "ctf/enemy/score", "vox/your/score", "vox/enemy/score", "vox/blue/score", "vox/red/score",
     "vox/green/score"},                                                      // CAPTURE
    {"ctf/your/flag/drop", "ctf/enemy/flag/drop", "vox/your/flag/drop", "vox/enemy/flag/drop", "vox/blue/flag/drop",
     "vox/red/flag/drop", "vox/green/flag/drop"},                             // DROP
    {"ctf/your/flag/manualreturn", "ctf/enemy/flag/manualreturn", "vox/your/flag/manualreturn",
     "vox/enemy/flag/manualreturn", "vox/blue/flag/manualreturn", "vox/red/flag/manualreturn",
     "vox/green/flag/manualreturn"},                                          // MANUALRETURN
};
...
void CTF_Sound(team_t flag, team_t team, flag_score_t ev)
{
    ...
    if (snd_gamesfx)
    {
        ...
        if ((ev == SCORE_RETURN || !(nonScoreEvent && !yourTeam && !yourFlag)) &&
            S_FindSound(flag_sound[ev][sound]) != -1)
    ...
    case 2:
        ...
        if (!consoleplayer().spectator)
        {
            ...
            if (ev == SCORE_CAPTURE && playerTeam == team)
                sound = Yours;
            else if ((ev == SCORE_RETURN || ev == SCORE_MANUALRETURN || ev == SCORE_DROP || ev == SCORE_GRAB ||
                      ev == SCORE_FIRSTGRAB) &&
                     playerTeam == flag)
                sound = Yours;
...
static const char *flag_message[NUM_CTF_SCORE][5] = {
    ...
    {"", "", "", "", ""},                                                                             // CARRIERKILL
    {"Your Flag Returned", "Enemy Flag Returned", "Blue Flag Returned", "Red Flag Returned",
     "Green Flag Returned"},                                                                          // RETURN
    ...
    {"Your Flag Returning", "Enemy Flag Returning", "Blue Flag Returning", "Red Flag Returning",
     "Green Flag Returning"} // MANUALRETURN*/
};
...
VERSION_CONTROL(cl_ctf_cpp, "$Id: fea4e1072fe603db55b89703edb9a195a2b143ad $")
