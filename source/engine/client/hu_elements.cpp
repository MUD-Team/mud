...
//-----------------------------------------------------------------------------
//
// $Id: 5d3c5b5e29699581fbae90b3f9dc09ca7bf40f5d $
//
...
std::string HelpText()
{
    ...
    if (join != JOIN_OK)
    {
        if (queuePos > 0)
        {
            StrFormat(str, "%s - " TEXTCOLOR_GREEN "%d%s" TEXTCOLOR_NORMAL " in line", joinmsg.c_str(), queuePos,
                      ordinal(queuePos));
            return str;
        }

        StrFormat(str, "%s - Press " TEXTCOLOR_GOLD "%s" TEXTCOLOR_NORMAL " to queue", joinmsg.c_str(),
    ...
    {
        StrFormat(str,
                  "Press " TEXTCOLOR_GOLD "%s" TEXTCOLOR_NORMAL " to join - " TEXTCOLOR_GREEN "%d" TEXTCOLOR_NORMAL
                  " secs left",
                  ::Bindings.GetKeynameFromCommand("+use").c_str(), ::levelstate.getJoinTimeLeft());
        return str;
    }
...
std::string Timer()
{
    ...
    OTimespan tspan;
    if (::levelstate.getState() == LevelState::WARMUP || ::levelstate.getState() == LevelState::WARMUP_COUNTDOWN ||
    ...
    if (tspan.hours)
    {
        StrFormat(str, "%s%02d:%02d:%02d", color, tspan.hours, tspan.minutes, tspan.seconds);
    }
    else
    {
...
std::string PersonalSpread()
{
    ...
    if (G_IsFFAGame())
    {
        ...
        int top_number = g_rounds && !G_IsMatchDuelGame() ? max_players.players.at(0)->roundwins
                                                          : max_players.players.at(0)->fragcount;

        int plyr_number = g_rounds && !G_IsMatchDuelGame() ? plyr.roundwins : plyr.fragcount;
        ...
        if (max_players.players.at(0)->id == plyr.id)
        {
            ...
            // The interesting thing changes based on rounds.
            int next_number = g_rounds && !G_IsMatchDuelGame() ? other_players.players.at(0)->roundwins
    ...
    else if (G_IsTeamGame())
    {
        ...
        if (max_teams.at(0)->Team == plyr_team.Team)
        {
            ...
            // The interesting thing changes based on rounds.
            int next_number = g_rounds ? other_teams.at(0)->RoundWins : other_teams.at(0)->Points;
...
std::string PersonalScore()
{
    ...
    if (G_IsTeamGame())
    {
        ...
        if (G_IsRoundsGame())
        {
            if (g_winlimit)
            {
                StrFormat(str, "%s%d/%d", plyr_team.TextColor.c_str(), plyr_team.RoundWins, g_winlimit.asInt());
            }
            else
            {
        ...
        else
        {
            ...
            {
                StrFormat(str, "%s%d/%d", plyr_team.TextColor.c_str(), plyr_team.Points, sv_fraglimit.asInt());
            }
            else if (!G_UsesFraglimit() && sv_scorelimit > 0)
            {
                StrFormat(str, "%s%d/%d", plyr_team.TextColor.c_str(), plyr_team.Points, sv_scorelimit.asInt());
            }
            else
            {
    ...
    else if (G_IsFFAGame())
    {
        if (G_IsRoundsGame() && !G_IsMatchDuelGame())
        {
            if (g_winlimit)
            {
                StrFormat(str, TEXTCOLOR_GREY "%d/%d", plyr.roundwins, g_winlimit.asInt());
            }
            else
            {
        ...
        else
        {
            if (sv_fraglimit)
            {
                StrFormat(str, TEXTCOLOR_GREY "%d/%d", plyr.fragcount, sv_fraglimit.asInt());
            }
            else
            {
...
std::string PersonalMatchDuelPlacement()
{
    ...
    if (g_winlimit)
    {
        StrFormat(str, TEXTCOLOR_GREY "%d/%d W", plyr.roundwins, g_winlimit.asInt());
    }
    else
    {
            ...
            const x_align_t x_origin, const y_align_t y_origin, const float pct, const EColorRange color)
{
    ...
    for (size_t i = 0; i < lineHandles.size(); i++)
    {
        ...
        hud::DrawTranslatedPatch(drawX, y, scale, x_align, y_align, x_origin, y_origin, patch, ::Ranges + color * 256);
                   ...
                   const bool force_opaque)
{
    ...
    for (size_t i = 0; i < sortedPlayers().size(); i++)
    {
        ...
        if (ingamePlayer(player))
        {
            ...
            hud::DrawText(x, y, scale, x_align, y_align, x_origin, y_origin, player->userinfo.netname.c_str(), color,
                          force_opaque);
...
void EASpectatorNames(int x, int y, const float scale, const x_align_t x_align, const y_align_t y_align,
                      ...
                      const short limit, const bool force_opaque)
{
    byte                    drawn       = 0;
    std::vector<player_t *> sortPlayers = sortedPlayers();
    for (size_t i = 0; i < sortPlayers.size(); i++)
        ...
        if (spectatingPlayer(player))
        {
            if (skip <= 0)
            {
                ...
                if (player->QueuePosition)
                {
                    ...
                    hud::DrawText(x, y, scale, x_align, y_align, x_origin, y_origin, ss.str().c_str(), color,
                                  force_opaque);
                }
                else
                {
                    hud::DrawText(x, y, scale, x_align, y_align, x_origin, y_origin, player->userinfo.netname.c_str(),
                                  color, force_opaque);
                }
                y += 7 + padding;
                drawn += 1;
                       ...
                       const bool force_opaque)
{
    ...
    for (size_t i = 0; i < sortedPlayers().size(); i++)
    {
        ...
        if (ingamePlayer(player))
        {
            ...
            hud::DrawText(x, y, scale, x_align, y_align, x_origin, y_origin, buffer.c_str(), CR_GREY, force_opaque);
            y += 7 + padding;
            drawn += 1;
        }
    }
                   ...
                   const bool force_opaque)
{
    ...
    for (size_t i = 0; i < sortedPlayers().size(); i++)
    {
        ...
        if (ingamePlayer(player))
        {
            ...
            hud::DrawText(x, y, scale, x_align, y_align, x_origin, y_origin, buffer.str().c_str(), CR_GREY,
                          force_opaque);
            y += 7 + padding;
            drawn += 1;
        }
                   ...
                   const bool force_opaque)
{
    ...
    for (size_t i = 0; i < sortedPlayers().size(); i++)
    {
        ...
        if (ingamePlayer(player))
        {
            ...
            hud::DrawText(x, y, scale, x_align, y_align, x_origin, y_origin, buffer.str().c_str(),
                          pingTextColor(player->ping), force_opaque);
            y += 7 + padding;
            drawn += 1;
        }
    }
               ...
               const bool force_opaque)
{
    ...
    for (Players::iterator it = players.begin(); it != players.end(); ++it)
    {
        ...
        TargetInfo_t temp = {&*it, P_AproxDistance2(displayplayer().mo, it->mo) >> FRACBITS, color};
    ...
    for (size_t i = 0; i < Targets.size(); i++)
    {
        ...
        if (Targets[i].PlayPtr == &(consoleplayer()))
        {
            ...
            hud::DrawText(x, y, scale, x_align, y_align, x_origin, y_origin, "You", Targets[i].Color);
        }
        else
        {
            // Figure out if we should be showing this player's health.
            std::string nameplate;
            if (netdemoplaying || (::hud_targethealth_debug && P_AreTeammates(*Targets[i].PlayPtr, consoleplayer())))
            {
                ...
                StrFormat(nameplate, "%s %s%d", Targets[i].PlayPtr->userinfo.netname.c_str(), color, health);
            }
            else
            {
                nameplate = Targets[i].PlayPtr->userinfo.netname;
            }
            hud::DrawText(x, y, scale, x_align, y_align, x_origin, y_origin, nameplate.c_str(), Targets[i].Color);
        }
