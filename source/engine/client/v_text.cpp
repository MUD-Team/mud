...
//-----------------------------------------------------------------------------
//
// $Id: 804170f0d8e9a690edd6f1d96b130de66e4b5cd1 $
//
...
void V_TextInit()
{
    ...
    // Font heights.
    ::hu_bigfont_height   = W_ResolvePatchHandle(::hu_bigfont['M' - HU_FONTSTART])->height();
    ::hu_smallfont_height = W_ResolvePatchHandle(::hu_smallfont['M' - HU_FONTSTART])->height();
    ::hu_digfont_height   = W_ResolvePatchHandle(::hu_digfont['M' - HU_FONTSTART])->height();
    ...
    while (*str && x <= (surface_width - 8))
    {
        ...
        else
        {
            ...
            for (int z = 0; z < 8; z++)
            {
                ...
                {
                    const argb_t mask =
                        (source[a + 8] << 24) | (source[a + 8] << 16) | (source[a + 8] << 8) | source[a + 8];
...
void DCanvas::TextSWrapper(EWrapperCode drawer, int normalcolor, int x, int y, const byte *string, int scalex,
                           int scaley) const
{
    if (::hu_font[0].empty())
        return;
...
VERSION_CONTROL(v_text_cpp, "$Id: 804170f0d8e9a690edd6f1d96b130de66e4b5cd1 $")
