...
//-----------------------------------------------------------------------------
//
// $Id: 6268e1819b0a1d23a558848f65dd2f21e0f347bc $
//
...
oldmenuitem_t DoomMainMenu[] = {{1, "M_NGAME", M_NewGame, 'N'},   {1, "M_OPTION", M_Options, 'O'}, // [RH] Moved
...
oldmenuitem_t Doom2MainMenu[] = {{1, "M_NGAME", M_NewGame, 'N'},
...
oldmenu_t MainDef = {d1_main_end, DoomMainMenu, M_DrawMainMenu, 97, 64, 0};

//
// EPISODE SELECT
//

oldmenuitem_t EpisodeMenu[MAX_EPISODES] = {{1, "\0", M_Episode, 0}, {1, "\0", M_Episode, 0}, {1, "\0", M_Episode, 0},
                                           {1, "\0", M_Episode, 0}, {1, "\0", M_Episode, 0}, {1, "\0", M_Episode, 0},
                                           {1, "\0", M_Episode, 0}, {1, "\0", M_Episode, 0}};
...
oldmenuitem_t NewGameMenu[MAX_SKILLS + 1] = {
    {1, "\0", M_ChooseSkill, 0}, {1, "\0", M_ChooseSkill, 0}, {1, "\0", M_ChooseSkill, 0}, {1, "\0", M_ChooseSkill, 0},
    {1, "\0", M_ChooseSkill, 0}, {1, "\0", M_ChooseSkill, 0}, {1, "\0", M_ChooseSkill, 0}, {1, "\0", M_ChooseSkill, 0},
    //{1,"\0", M_ChooseSkill,0}
};
...
oldmenuitem_t PlayerSetupMenu[] = {{1, "", M_EditPlayerName, 'N'},    {2, "", M_ChangeTeam, 'T'},
                                   {2, "", M_ChangeGender, 'E'},      {2, "", M_ChangeAutoAim, 'A'},
                                   {2, "", M_ChangeColorPreset, 'C'}, {2, "", M_SlidePlayerRed, 'R'},
                                   {2, "", M_SlidePlayerGreen, 'G'},  {2, "", M_SlidePlayerBlue, 'B'}};

oldmenu_t PSetupDef = {psetup_end, PlayerSetupMenu, M_PlayerSetupDrawer, 48, 47, playername};
...
oldmenu_t OptionsDef = {0, NULL, NULL, 0, 0, 0};
...
oldmenuitem_t ReadMenu1[] = {{1, "", M_ReadThis2, 0}};

oldmenu_t ReadDef1 = {read1_end, ReadMenu1, M_DrawReadThis1, 280, 185, 0};
...
oldmenuitem_t ReadMenu2[] = {{1, "", M_ReadThis3, 0}};

oldmenu_t ReadDef2 = {read2_end, ReadMenu2, M_DrawReadThis2, 330, 175, 0};
...
oldmenuitem_t ReadMenu3[] = {{1, "", M_FinishReadThis, 0}};

oldmenu_t ReadDef3 = {read3_end, ReadMenu3, M_DrawReadThis3, 330, 175, 0};
...
static oldmenuitem_t LoadSavegameMenu[] = {
    ...
    {1, "", M_LoadSelect, '5'}, {1, "", M_LoadSelect, '6'}, {1, "", M_LoadSelect, '7'}, {1, "", M_LoadSelect, '8'},
};

oldmenu_t LoadDef = {load_end, LoadSavegameMenu, M_DrawLoad, 80, 54, 0};

//
// SAVE GAME MENU
//
oldmenuitem_t SaveMenu[] = {{1, "", M_SaveSelect, '1'}, {1, "", M_SaveSelect, '2'}, {1, "", M_SaveSelect, '3'},
                            {1, "", M_SaveSelect, '4'}, {1, "", M_SaveSelect, '5'}, {1, "", M_SaveSelect, '6'},
                            {1, "", M_SaveSelect, '7'}, {1, "", M_SaveSelect, '8'}};

oldmenu_t SaveDef = {load_end, SaveMenu, M_DrawSave, 80, 54, 0};
...
void M_DrawNewGame()
{
    ...
    screen->DrawTextCleanMove(CR_GREY, NewDef.x + V_StringWidth(pslabel), psy, g_resetinvonexit ? "ON" : "OFF");
}
...
void M_ChooseSkill(int choice)
{
    ...
    else if (SkillInfos[choice].must_confirm)
    {
        ...
        if (must_confirm_text[0] == '$')
            M_StartMessage(GStrings(StdStringToUpper(must_confirm_text + 1)), M_VerifyNightmare, true);
        else
...
void M_QuitDOOM(int choice)
{
    ...
    StrFormat(endstring, "%s\n\n%s", GStrings.getIndex(GStrings.toIndex(QUITMSG) + (gametic % NUM_QUITMESSAGES)),
              GStrings(DOSY));
...
static const char *colorpresets[11] = {"custom", "blue",         "indigo", "green", "brown", "red",
                                       "gold",   "jungle green", "purple", "white", "black"};
...
template <typename PIXEL_T> static forceinline PIXEL_T R_FirePixel(const byte c);

template <> forceinline byte R_FirePixel<byte>(const byte c)
{
    return FireRemap[c];
}

template <> forceinline argb_t R_FirePixel<argb_t>(const byte c)
{
    return V_GammaCorrect(argb_t(c, 0, 0));
}

template <int xscale, typename PIXEL_T> static forceinline void R_RenderFire(int x, int y)
{
...
static void M_PlayerSetupDrawer()
{
    ...
    // Draw player character
    {
        ...
        else
        {
            ...
            if (I_GetPrimarySurface()->getBitsPerPixel() == 8)
            {
                // 8bpp rendering:
                if (CleanXfac == 1)
                    R_RenderFire<1, palindex_t>(x, y);
                else if (CleanXfac == 2)
                    R_RenderFire<2, palindex_t>(x, y);
                else if (CleanXfac == 3)
                    R_RenderFire<3, palindex_t>(x, y);
                else if (CleanXfac == 4)
                    R_RenderFire<4, palindex_t>(x, y);
                else if (CleanXfac == 5)
                    R_RenderFire<5, palindex_t>(x, y);
            }
            else
            {
                // 32bpp rendering:
                if (CleanXfac == 1)
                    R_RenderFire<1, argb_t>(x, y);
                else if (CleanXfac == 2)
                    R_RenderFire<2, argb_t>(x, y);
                else if (CleanXfac == 3)
                    R_RenderFire<3, argb_t>(x, y);
                else if (CleanXfac == 4)
                    R_RenderFire<4, argb_t>(x, y);
                else if (CleanXfac == 5)
                    R_RenderFire<5, argb_t>(x, y);
            }
    ...
    screen->DrawPatchClean(W_CachePatch("M_PBOX"), 320 - 88 - 32 + 36, PSetupDef.y + LINEHEIGHT * 3 + 22);
    ...
    // Draw team setting
    {
        ...
        screen->DrawTextCleanMove(CR_GREY, x, PSetupDef.y + LINEHEIGHT,
...
void M_DrawEmptyCell(oldmenu_t *menu, int item)
{
    screen->DrawPatchClean(W_CachePatch("M_CELL1"), menu->x - 10, menu->y + item * LINEHEIGHT - 1);
}

void M_DrawSelCell(oldmenu_t *menu, int item)
{
    screen->DrawPatchClean(W_CachePatch("M_CELL2"), menu->x - 10, menu->y + item * LINEHEIGHT - 1);
}
...
bool M_Responder(event_t *ev)
{
    ...
    if (genStringEnter)
    {
        ...
        else
        {
            ...
            if (ch >= 32 && ch <= 127 && saveCharIndex < genStringLen &&
                V_StringWidth(savegamestrings[saveSlot]) < (genStringLen - 1) * 8)
            {
    ...
    if (messageToPrint)
    {
        if (messageNeedsInput && (!(ch2 == ' ' || Key_IsMenuKey(ch) || Key_IsYesKey(ch) || Key_IsNoKey(ch) ||
...
void M_Drawer()
{
    ...
    else if (menuactive)
    {
        ...
        else
        {
            ...
            if (drawSkull)
            {
                screen->DrawPatchClean(W_CachePatch(skullName[whichSkull]), x + SKULLXOFF,
...
void M_Init(void)
{
    ...
    // (ie from doom to doom2 back to doom), you will have less menu items
    {
        ...
        MainDef.routine = M_DrawMainMenu, MainDef.lastOn = 0;
        MainDef.x = 97;
        MainDef.y = 64;
    }
