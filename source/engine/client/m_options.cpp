...
//-----------------------------------------------------------------------------
//
// $Id: 30c720ef235a4c9faeb2c93b1585e3f003e90c49 $
//
...
value_t YesNo[2] = {{0.0, "No"}, {1.0, "Yes"}};

value_t NoYes[2] = {{0.0, "Yes"}, {1.0, "No"}};

value_t OnOff[2] = {{0.0, "Off"}, {1.0, "On"}};

value_t HideShow[2] = {{0.0, "Hide"}, {1.0, "Show"}};

value_t OffOn[2] = {{0.0, "On"}, {1.0, "Off"}};

value_t OnOffAuto[3] = {{0.0, "Off"}, {1.0, "On"}, {2.0, "Auto"}};

value_t DemoRestrictions[2] = {{0.0, "Restrict"}, {1.0, "Allow"}};

static value_t DoomOrOdamex[2] = {{0.0, "Odamex"}, {1.0, "Doom"}};
...
menu_t OptionMenu = {"M_OPTTTL", 0, ARRAY_LENGTH(OptionItems), 177, OptionItems, 0, 0, NULL};
...
menu_t ControlsMenu = {"M_CONTRO", 3, ARRAY_LENGTH(ControlsItems), 0, ControlsItems, 2, 0, NULL};
...
menu_t MouseMenu = {"M_MOUSET", 0, ARRAY_LENGTH(MouseItems), 177, MouseItems, 0, 0, NULL};
...
menu_t JoystickMenu = {"M_JOYSTK", 0, ARRAY_LENGTH(JoystickItems), 177, JoystickItems, 0, 0, NULL};
...
static value_t MusSys[] = {{MS_FLUIDLITE, "On"}, {MS_NONE, "Off"}};

static value_t MidiReset[] = {{0.0, "None"}, {1.0, "GM"}, {2.0, "GS"}, {3.0, "XG"}};

static value_t VoxType[] = {{0.0, "Off"}, {1.0, "Team Colors"}, {2.0, "Possessive"}};

static value_t ChatSndType[] = {{0.0, "Disabled"}, {1.0, "Enabled"}, {2.0, "Teamchat only"}};
...
menu_t SoundMenu = {"M_SOUND", 2, ARRAY_LENGTH(SoundItems), 177, SoundItems, 0, 0, NULL};
...
menu_t CompatMenu = {
    "M_COMPAT", 1, ARRAY_LENGTH(CompatItems), 240, CompatItems, 0, 0, NULL,
};
...
static value_t PredictSectors[] = {{0.0, "None"}, {1.0, "All"}, {2.0, "Only Mine"}};
...
menu_t NetworkMenu = {"M_NETWRK", 2, ARRAY_LENGTH(NetworkItems), 177, NetworkItems, 1, 0, NULL};
...
static value_t WeapSwitch[] = {{0.0, "Never"}, {1.0, "Always"}, {2.0, "By Preference"}, {3.0, "Attack Cancels PWO"}};
...
menu_t WeaponMenu = {"M_WEAPON", 1, ARRAY_LENGTH(WeaponItems), 177, WeaponItems, 0, 0, NULL};
...
static value_t Wipes[] = {{0.0, "None"}, {1.0, "Melt"}, {2.0, "Burn"}, {3.0, "Crossfade"}};

static value_t Overlays[] = {{0.0, "Off"}, {1.0, "Standard"}, {2.0, "Full"}, {3.0, "Full Only"}};
...
menu_t VideoMenu = {"M_VIDEO", 0, ARRAY_LENGTH(VideoItems), 0, VideoItems, 4, 0, &M_UpdateDisplayOptions};
...
static value_t TimerStyles[] = {{0.0, "No Timer"}, {1.0, "Count Down"}, {2.0, "Count Up"}};

static value_t FlagHelds[] = {{0.0, "Off"}, {1.0, "Complete"}, {2.0, "Simple"}};

static value_t Crosshairs[] = {{0.0, "None"}, {1.0, "Cross 1"}, {2.0, "Cross 2"}, {3.0, "X"},        {4.0, "Diamond"},
                               {5.0, "Dot"},  {6.0, "Box"},     {7.0, "Angle"},   {8.0, "Big Thing"}};
...
static value_t TextColors[] = {{CR_BRICK, "brick"},
                               {CR_TAN, "tan"},
                               {CR_GRAY, "gray"},
                               {CR_GREEN, "green"},
                               {CR_BROWN, "brown"},
...
static value_t Languages[] = {{0.0, "Auto"}, {1.0, "English"}, {2.0, "French"}, {3.0, "Italian"}};
...
menu_t MessagesMenu = {"M_MESS", 0, ARRAY_LENGTH(MessagesItems), 0, MessagesItems, 0, 0, NULL};
...
static value_t ClassicMapStringTypes[] = {{0.0, "Odamex"}, {1.0, "Classic"}};
...
menu_t AutomapMenu = {"M_AUTOMP", 0, ARRAY_LENGTH(AutomapItems), 0, AutomapItems, 0, 0, NULL};
...
static value_t VidFPSCaps[] = {{35.0, "35fps"},   {60.0, "60fps"},   {70.0, "70fps"},
                               {105.0, "105fps"}, {120.0, "120fps"}, {140.0, "140fps"},
                               {144.0, "144fps"}, {240.0, "240fps"}, {0.0, "Unlimited"}};

static value_t FullScreenOptions[] = {{WINDOW_Windowed, "Window"},
                                      {WINDOW_Fullscreen, "Full Screen Exclusive"},
                                      {WINDOW_DesktopFullscreen, "Full Screen Window"}};

static value_t WidescreenMode[] = {{0.0, "Off"},  {1.0, "Auto"}, {2.0, "16:10"},
...
menu_t ModesMenu = {"M_VIDMOD", 0, ARRAY_LENGTH(ModesItems), 130, ModesItems, 0, 0, NULL};
...
void M_OptResponder(event_t *ev)
{
    ...
    if (WaitingForAxis)
    {
        ...
        else if (ev->type == ev_joystick)
        {
            if (ev->data1 == 0) // Analog Motion
            {
                ...
                {
                    if ((ev->data2 == (int)joy_forwardaxis) && strcmp(joy_forwardaxis.name(), item->a.cvar->name()))
                        joy_forwardaxis.Set(item->a.cvar->value());
                    else if ((ev->data2 == (int)joy_strafeaxis) && strcmp(joy_strafeaxis.name(), item->a.cvar->name()))
                        joy_strafeaxis.Set(item->a.cvar->value());
                    else if ((ev->data2 == (int)joy_turnaxis) && strcmp(joy_turnaxis.name(), item->a.cvar->name()))
                        joy_turnaxis.Set(item->a.cvar->value());
                    else if ((ev->data2 == (int)joy_lookaxis) && strcmp(joy_lookaxis.name(), item->a.cvar->name()))
    ...
    if (item->type == bitflag && flagsvar &&
        (Key_IsLeftKey(ch, numlock) || Key_IsRightKey(ch, numlock) || Key_IsAcceptKey(ch)) && !demoplayback)
    {
    ...
    // Handle Keys
    {
            ...
            do
            {
                ...
                if (CurrentItem == CurrentMenu->numitems)
                {
                    ...
                    CurrentItem            = 0;
                     ...
                     (CurrentMenu->items[CurrentItem].type == screenres && !CurrentMenu->items[CurrentItem].b.res1));
        ...
        else if (Key_IsUpKey(ch, numlock))
        {
            ...
            {
                CurrentItem--;
                if (CanScrollUp && CurrentItem == CurrentMenu->scrolltop + CurrentMenu->scrollpos)
                {
                    CurrentMenu->scrollpos--;
        ...
        else if (Key_IsLeftKey(ch, numlock))
        {
            ...
            case svdiscrete: {
                ...
                if (item->type == svdiscrete && (multiplayer || demoplayback || netdemo.isPlaying()))
                    break;
...
VERSION_CONTROL(m_options_cpp, "$Id: 30c720ef235a4c9faeb2c93b1585e3f003e90c49 $")
