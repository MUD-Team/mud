...
//-----------------------------------------------------------------------------
//
// $Id: acc1f17603bd6dbf6c4f1792c122c8ba17b607f0 $
//
...
DCanvas::vdrawfunc DCanvas::Pfuncs[6] = {
    DCanvas::DrawPatchP,           DCanvas::DrawLucentPatchP,
    DCanvas::DrawTranslatedPatchP, DCanvas::DrawTlatedLucentPatchP,
    DCanvas::DrawColoredPatchP,    DCanvas::DrawColorLucentPatchP,
};
DCanvas::vdrawsfunc DCanvas::Psfuncs[6] = {DCanvas::DrawPatchSP,
...
DCanvas::vdrawfunc DCanvas::Dfuncs[6] = {
    DCanvas::DrawPatchD,           DCanvas::DrawLucentPatchD,
    DCanvas::DrawTranslatedPatchD, DCanvas::DrawTlatedLucentPatchD,
    DCanvas::DrawColoredPatchD,    DCanvas::DrawColorLucentPatchD,
};
DCanvas::vdrawsfunc DCanvas::Dsfuncs[6] = {DCanvas::DrawPatchSD,
 ...
 * @param destheight
 */
void DCanvas::DrawSWrapper(EWrapperCode drawer, const patch_t *patch, int x0, int y0, const int destwidth,
                           const int destheight) const
{
    if (patch == NULL)
        return;

    if (patch->width() <= 0 || patch->height() <= 0 || destwidth <= 0 || destheight <= 0)
        return;
    ...
    {
        tallpost_t *post = (tallpost_t *)((byte *)patch + LELONG(patch->columnofs[col >> FRACBITS]));

        // step through the posts in a column
        while (!post->end())
        {
            drawfunc(post->data(), desttop + (((post->topdelta * ymul)) >> FRACBITS) * surface_pitch,
                     (post->length * ymul) >> FRACBITS, surface_pitch, yinc);
...
void DCanvas::DrawIWrapper(EWrapperCode drawer, const patch_t *patch, int x0, int y0) const
{
    ...
    if (surface_width == 320 && surface_height == 200)
        ...
        DrawSWrapper(drawer, patch, (surface_width * x0) / 320, (surface_height * y0) / 200,
...
void DCanvas::DrawCWrapper(EWrapperCode drawer, const patch_t *patch, int x0, int y0) const
{
    ...
    if (CleanXfac == 1 && CleanYfac == 1)
        ...
        DrawSWrapper(drawer, patch, (x0 - 160) * CleanXfac + (surface_width / 2),
                     (y0 - 100) * CleanYfac + (surface_height / 2), patch->width() * CleanXfac,
...
void DCanvas::DrawCNMWrapper(EWrapperCode drawer, const patch_t *patch, int x0, int y0) const
{
    ...
    if (CleanXfac == 1 && CleanYfac == 1)
        ...
        DrawSWrapper(drawer, patch, x0, y0, patch->width() * CleanXfac, patch->height() * CleanYfac);
}
...
void DCanvas::DrawPatchFlipped(const patch_t *patch, int x0, int y0) const
{
    ...
    {
        tallpost_t *post = (tallpost_t *)((byte *)patch + LELONG(patch->columnofs[col >> 16]));
...
static inline void V_GetTransposedBlockGeneric(byte *destbuffer, const byte *sourcebuffer, int x, int y, int width,
                                               int height, int sourcepitchpixels)
{
...
void DCanvas::GetTransposedBlock(int x, int y, int width, int height, byte *destbuffer) const
{
    ...
    if (mSurface->getBitsPerPixel() == 8)
        V_GetTransposedBlockGeneric<palindex_t>(destbuffer, mSurface->getBuffer(), x, y, width, height,
                                                mSurface->getPitchInPixels());
    else
        V_GetTransposedBlockGeneric<argb_t>(destbuffer, mSurface->getBuffer(), x, y, width, height,
                                            mSurface->getPitchInPixels());
}

VERSION_CONTROL(v_draw_cpp, "$Id: acc1f17603bd6dbf6c4f1792c122c8ba17b607f0 $")
