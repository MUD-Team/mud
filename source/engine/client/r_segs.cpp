...
//-----------------------------------------------------------------------------
//
// $Id: 8ba19986de7fdf33085e43eb4ea5804aa01c8ddc $
//
...
static int32_t walltopf[MAXWIDTH];
static int32_t walltopb[MAXWIDTH];
static int32_t wallbottomf[MAXWIDTH];
static int32_t wallbottomb[MAXWIDTH];
...
//
static inline fixed_t R_TexScaleX(fixed_t x, texhandle_t texnum)
{
    return FixedMul(x, texturemanager.getTexture(texnum)->getScaleX());
}
...
//
static inline fixed_t R_TexScaleY(fixed_t y, texhandle_t texnum)
{
    return FixedMul(y, texturemanager.getTexture(texnum)->getScaleY());
}
...
//
static inline fixed_t R_TexInvScaleX(fixed_t x, texhandle_t texnum)
{
    return FixedDiv(x, texturemanager.getTexture(texnum)->getScaleX());
}
...
//
static inline fixed_t R_TexInvScaleY(fixed_t y, texhandle_t texnum)
{
    return FixedDiv(y, texturemanager.getTexture(texnum)->getScaleY());
}

//
// R_OrthogonalLightnumAdjustment
//
int32_t R_OrthogonalLightnumAdjustment()
{
...
//
static void R_FillWallHeightArray(int32_t *array, int32_t start, int32_t stop, fixed_t val1, fixed_t val2, float scale1,
                                  float scale2)
{
    if (start > stop)
        return;
    ...
    {
        array[i] = clamp((int32_t)frac, ceilingclipinitial[0], floorclipinitial[0]);
        frac -= step;
    }
}
...
inline void R_ColumnSetup(int32_t x, int32_t *top, int32_t *bottom, tallpost_t **posts, bool calc_light)
{
    if (calc_light)
    {
...
static inline int32_t R_ColumnRangeMinimumHeight(int32_t start, int32_t stop, int32_t *top)
{
    int32_t minheight = viewheight - 1;
...
static inline int32_t R_ColumnRangeMaximumHeight(int32_t start, int32_t stop, int32_t *bottom)
{
    int32_t maxheight = 0;
    for (int32_t x = start; x <= stop; x++)
...
#define BLOCKMASK (BLOCKSIZE - 1)
        ...
        static int32_t light_lookup[MAXWIDTH];
        if (calc_light)
        {
            for (int32_t x = start; x <= stop; x++)
            {
        ...
        for (int32_t bx = start; bx <= stop; bx = (bx & ~BLOCKMASK) + BLOCKSIZE)
        {
            int32_t blockstartx = bx;
            int32_t blockstopx  = MIN((bx & ~BLOCKMASK) + BLOCKSIZE - 1, stop);

            int32_t miny = R_ColumnRangeMinimumHeight(blockstartx, blockstopx, top);
            int32_t maxy = R_ColumnRangeMaximumHeight(blockstartx, blockstopx, bottom);

            for (int32_t by = miny; by <= maxy; by = (by & ~BLOCKMASK) + BLOCKSIZE)
            {
                int32_t blockstarty = by;
                int32_t blockstopy  = (by & ~BLOCKMASK) + BLOCKSIZE - 1;

                for (int32_t x = blockstartx; x <= blockstopx; x++)
                {
                    if (calc_light)
...
void R_RenderSolidSegRange(int32_t start, int32_t stop)
{
    ...
    int32_t        initial_light = rw_light;
    ...
    if (markceiling)
    {
        for (int32_t x = start; x <= stop; x++)
        {
            int32_t top    = MAX(ceilingclip[x], 0);
            int32_t bottom = MIN(MIN(walltopf[x], floorclip[x]) - 1, viewheight - 1);
    ...
    if (markfloor)
    {
        for (int32_t x = start; x <= stop; x++)
        {
            int32_t top    = MAX(MAX(wallbottomf[x], ceilingclip[x]), 0);
            int32_t bottom = MIN(floorclip[x] - 1, viewheight - 1);
    ...
    if (midtexture) // 1-sided line
    {
        ...
        dcol.textureheight = texturemanager.getTexture(midtexture)->getFracHeight();
    ...
    else // 2-sided line
    {
        if (toptexture)
        {
            ...
            dcol.textureheight = texturemanager.getTexture(toptexture)->getFracHeight();
        ...
        if (bottomtexture)
        {
            ...
            dcol.textureheight = texturemanager.getTexture(bottomtexture)->getFracHeight();
...
void R_RenderMaskedSegRange(drawseg_t *ds, int32_t x1, int32_t x2)
{
    ...
    lightnum =
        (R_FakeFlat(frontsector, &tempsec, NULL, NULL, false)->lightlevel >> LIGHTSEGSHIFT) + (foggy ? 0 : extralight);

    lightnum += R_OrthogonalLightnumAdjustment();

    walllights = lightnum >= LIGHTLEVELS ? scalelight[LIGHTLEVELS - 1]
                 : lightnum < 0          ? scalelight[0]
                                         : scalelight[lightnum];
                ...
                int32_t stop)
{
    ...
    int32_t width = stop - start + 1;
    if (width <= 0)
        return;

    texhandle_t toptexture    = curline->sidedef->toptexture;
    texhandle_t midtexture    = curline->sidedef->midtexture;
    texhandle_t bottomtexture = curline->sidedef->bottomtexture;
...
void R_StoreWallRange(int32_t start, int32_t stop)
{
    ...
    midtexture = toptexture = bottomtexture = maskedtexture = TextureManager::NO_TEXTURE_HANDLE;
    ds_p->midposts                                          = NULL;
    ...
    if (!backsector)
    {
        ...
        if (linedef->flags & ML_DONTPEGBOTTOM)
        {
            ...
            fixed_t texheight = R_TexScaleY(texturemanager.getTexture(midtexture)->getFracHeight(), midtexture);
    ...
    else
    {
        ...
        else
        {
            ...
            if (rw_frontfz1 > rw_backfz1 || rw_frontfz2 > rw_backfz2 || rw_backfz1 > viewz || rw_backfz2 > viewz ||
                !P_IsPlaneLevel(&backsector->floorplane)) // backside sloping?
                ds_p->silhouette |= SIL_BOTTOM;

            if (rw_frontcz1 < rw_backcz1 || rw_frontcz2 < rw_backcz2 || rw_backcz1 < viewz || rw_backcz2 < viewz ||
        ...
        {
            markfloor = !P_IdenticalPlanes(&backsector->floorplane, &frontsector->floorplane) ||
            ...
            markceiling = !P_IdenticalPlanes(&backsector->ceilingplane, &frontsector->ceilingplane) ||
                          backsector->lightlevel != frontsector->lightlevel ||
            ...
            // Sky hack
            markceiling =
                markceiling && (frontsector->ceilingpic != skyflatnum || backsector->ceilingpic != skyflatnum);
        }
        ...
        if (rw_hashigh)
        {
            ...
            {
                // bottom of texture
                fixed_t texheight = R_TexScaleY(texturemanager.getTexture(toptexture)->getFracHeight(), toptexture);
    ...
    if (segtextured)
    {
        ...
        {
            int32_t lightnum = (frontsector->lightlevel >> LIGHTSEGSHIFT) + (foggy ? 0 : extralight);
    ...
    if (frontsector->heightsec == NULL || (frontsector->heightsec->MoreFlags & SECF_IGNOREHEIGHTSEC))
    {
        // above view plane?
        if (P_FloorHeight(viewx, viewy, frontsector) >= viewz)
            markfloor = false;
        // below view plane?
        if (P_CeilingHeight(viewx, viewy, frontsector) <= viewz && frontsector->ceilingpic != skyflatnum)
            markceiling = false;
    }
...
VERSION_CONTROL(r_segs_cpp, "$Id: 8ba19986de7fdf33085e43eb4ea5804aa01c8ddc $")
