...
//-----------------------------------------------------------------------------
//
// $Id: a79bf1c286266d9016fd44f4c40296eeb18543b7 $
//
 ...
 */
void ST_DrawBar(int normalcolor, unsigned int value, unsigned int total, int x, int y, int width, bool reverse = false,
...
void ST_voteDraw(int y)
{
    ...
    ST_DrawBar(CR_RED, vote_state.no, vote_state.no_needed, (I_GetSurfaceWidth() >> 1) - xscale * 40, y, xscale * 40,
               true, false, true);
    ST_DrawBar(CR_GREEN, vote_state.yes, vote_state.yes_needed, (I_GetSurfaceWidth() >> 1), y, xscale * 40, false,
               true);
}
...
static void drawTeamGametype()
{
    ...
    for (int i = 0; i < sv_teamsinplay; i++)
    {
        ...
        if (shouldShowScores)
        {
            ...
            if (drawPatch != NULL)
            {
                hud::DrawPatch(SCREEN_BORDER, patchPosY, hud_scale, hud::X_RIGHT, hud::Y_BOTTOM, hud::X_RIGHT,
                               hud::Y_BOTTOM, drawPatch);
            }
        ...
        if (shouldShowLives)
        {
            ...
            hud::DrawPatch(SCREEN_BORDER, patchPosY, hud_scale, hud::X_RIGHT, hud::Y_BOTTOM, hud::X_RIGHT,
                           hud::Y_BOTTOM, W_ResolvePatchHandle(::LivesIcon[i]));

            StrFormat(buffer, "%d", teamInfo->LivesPool());
            int color = (i % 2) ? CR_GOLD : CR_GREY;
            hud::DrawText(SCREEN_BORDER + 12, patchPosY + 3, hud_scale, hud::X_RIGHT, hud::Y_BOTTOM, hud::X_RIGHT,
                          hud::Y_BOTTOM, buffer.c_str(), color);
        }
    }
}
...
static void drawHordeGametype()
{
    ...
    hud::DrawText(SCREEN_BORDER, y, ::hud_scale, hud::X_RIGHT, hud::Y_BOTTOM, hud::X_RIGHT, hud::Y_BOTTOM,
                  waverow.c_str(), CR_GREY);
    hud::EleBar(SCREEN_BORDER, y + LINE_SPACING, V_StringWidth("WAVE:0/0"), ::hud_scale, hud::X_RIGHT, hud::Y_BOTTOM,
                hud::X_RIGHT, hud::Y_BOTTOM, killPct, killColor);
    hud::DrawText(SCREEN_BORDER, y + LINE_SPACING + BAR_BORDER, ::hud_scale, hud::X_RIGHT, hud::Y_BOTTOM, hud::X_RIGHT,
                  hud::Y_BOTTOM, killrow.c_str(), CR_GREY);
    ...
    if (hud_hordeinfo_debug)
    {
        ...
        StrFormat(buf, "Min HP: %d\nAlive HP: %d\nMax HP: %d\nSpawn: %s", define.minTotalHealth(), info.alive(),
                  define.maxTotalHealth(), buf2.c_str());
        hud::DrawText(SCREEN_BORDER, 64, ::hud_scale, hud::X_LEFT, hud::Y_BOTTOM, hud::X_LEFT, hud::Y_BOTTOM,
                      buf.c_str(), CR_GREY);
...
void drawProtos()
{
    ...
    for (Protos::const_iterator it = protos.begin(); it != protos.end(); ++it)
    {
        ...
        {
            // Draw arrow
            hud::DrawText(0, y, scale, hud::X_LEFT, hud::Y_TOP, hud::X_LEFT, hud::Y_TOP, " >", CR_GOLD, true);
        }
        ...
        // Draw name
        hud::DrawText(indent, y, scale, hud::X_LEFT, hud::Y_TOP, hud::X_LEFT, hud::Y_TOP, it->name.c_str(), rowColor,
                      true);
        ...
        {
            // Draw data
            hud::DrawText(indent, y, 0.75, hud::X_LEFT, hud::Y_TOP, hud::X_LEFT, hud::Y_TOP, it->data.c_str(), CR_WHITE,
                          true);
...
void drawNetdemo()
{
    ...
    // Dim the background.
    hud::Dim(1, 41, 74, 13, ::hud_scale, hud::X_LEFT, hud::Y_BOTTOM, hud::X_LEFT, hud::Y_BOTTOM);

    // Draw demo elapsed time
    hud::DrawText(2, 47, hud_scale, hud::X_LEFT, hud::Y_BOTTOM, hud::X_LEFT, hud::Y_BOTTOM,
                  hud::NetdemoElapsed().c_str(), color);

    // Draw map number/total
    hud::DrawText(74, 47, hud_scale, hud::X_LEFT, hud::Y_BOTTOM, hud::X_RIGHT, hud::Y_BOTTOM,
    ...
    ST_DrawBar(color, netdemo.calculateTimeElapsed(), netdemo.calculateTotalTime(), 2 * xscale,
               I_GetSurfaceHeight() - 46 * yscale, 72 * xscale);
...
void OdamexHUD()
{
    ...
    {
        num_ax = (((float)I_GetSurfaceWidth() - (float)I_GetSurfaceHeight() * 4.0f / 3.0f) / 2.0f) *
                 (1.0f - hud_anchoring.value());
        num_ax   = MAX(0, num_ax);
    ...
    if (plyr->armortype && plyr->armorpoints)
    {
        ...
        if (current_armor)
        {
            ...
            hud::DrawPatchScaled(patch_ax + 48 + 2 + 10, 32, 20, 20, hud_scale, hud::X_LEFT, hud::Y_BOTTOM,
                                 hud::X_CENTER, hud::Y_MIDDLE, current_armor);
        }
    ...
    hud::DrawPatchScaled(patch_ax + 48 + 2 + 10, 2, 20, 20, hud_scale, hud::X_LEFT, hud::Y_BOTTOM, hud::X_CENTER,
                         hud::Y_BOTTOM, W_ResolvePatchHandle(faces[st_faceindex]));
    ...
    if (g_lives)
    {
        ...
        hud::DrawText(text_ax + 48 + 2 + 20 + 2, 10 + 2, hud_scale, hud::X_LEFT, hud::Y_BOTTOM, hud::X_LEFT,
                      hud::Y_MIDDLE, buf.c_str(), lives_color, false);
    }
    ...
    if (ammotype < NUMAMMO)
    {
        ...
        {
            hud::DrawPatchScaled(patch_ax + 12, 12, 16, 16, hud_scale, hud::X_RIGHT, hud::Y_BOTTOM, hud::X_CENTER,
                                 hud::Y_MIDDLE, ammopatch);
        }
        else
        {
            hud::DrawPatch(patch_ax + 12, 12, hud_scale, hud::X_RIGHT, hud::Y_BOTTOM, hud::X_CENTER, hud::Y_MIDDLE,
                           ammopatch);
        }
    ...
    if (::hud_timer)
    {
        ...
        hud::DrawText(0, iy, hud_scale, hud::X_CENTER, hud::Y_BOTTOM, hud::X_CENTER, hud::Y_BOTTOM,
                      hud::Timer().c_str(), CR_GREY);
        iy += V_LineHeight() + 1;
    ...
    if (::hud_speedometer && ::consoleplayer_id == ::displayplayer_id)
    {
        ...
        hud::DrawText(0, iy, hud_scale, hud::X_CENTER, hud::Y_BOTTOM, hud::X_CENTER, hud::Y_BOTTOM, buf.c_str(),
                      CR_GREY);
        iy += V_LineHeight() + 1;
    ...
    hud::DrawText(0, iy, hud_scale, hud::X_CENTER, hud::Y_BOTTOM, hud::X_CENTER, hud::Y_BOTTOM,
                  hud::SpyPlayerName().c_str(), CR_GREY);
    iy += V_LineHeight() + 1;

    // Draw targeted player names.
    hud::EATargets(0, iy, hud_scale, hud::X_CENTER, hud::Y_BOTTOM, hud::X_CENTER, hud::Y_BOTTOM, 1, hud_targetcount);
    iy += V_LineHeight() + 1;
    ...
    hud::DrawText(text_ax + 4, spreadheight, ::hud_scale, hud::X_RIGHT, hud::Y_BOTTOM, hud::X_RIGHT, hud::Y_BOTTOM,
                  hud::PersonalSpread().c_str(), CR_GREY);
    hud::DrawText(text_ax + 4, scoreheight, ::hud_scale, hud::X_RIGHT, hud::Y_BOTTOM, hud::X_RIGHT, hud::Y_BOTTOM,
                  hud::PersonalScore().c_str(), CR_GREY);

    if (G_IsMatchDuelGame())
    {
        hud::DrawText(text_ax + 4, placeheight, ::hud_scale, hud::X_RIGHT, hud::Y_BOTTOM, hud::X_RIGHT, hud::Y_BOTTOM,
    ...
    if (G_IsCoopGame())
    {
        for (byte i = 0; i < NUMCARDS; i++)
        {
            if (plyr->cards[i])
            {
                hud::DrawPatch(patch_ax + 4 + (i * 10), 24, hud_scale, hud::X_RIGHT, hud::Y_BOTTOM, hud::X_RIGHT,
                               hud::Y_BOTTOM, W_ResolvePatchHandle(keys[i]));
            }
        }
    }
...
void DrawToasts()
{
    ...
    for (drawToasts_t::const_iterator it = g_Toasts.begin(); it != g_Toasts.end(); ++it)
    {
        ...
        // Right-hand side.
        hud::DrawText(x, y + 1, hud_scale, hud::X_RIGHT, hud::Y_TOP, hud::X_RIGHT, hud::Y_TOP, it->right.c_str(),
                      CR_GREY);
        x += V_StringWidth(it->right.c_str()) + 1;

        // Icon
        patch_t     *icon = W_ResolvePatchHandle(it->icon);
        const double yoff = (static_cast<double>(TOAST_HEIGHT) - static_cast<double>(icon->height())) / 2.0;

        hud::DrawPatch(x, y + ceil(yoff), hud_scale, hud::X_RIGHT, hud::Y_TOP, hud::X_RIGHT, hud::Y_TOP, icon, false,
                       true);
        x += icon->width() + 1;

        // Left-hand side.
        hud::DrawText(x, y + 1, hud_scale, hud::X_RIGHT, hud::Y_TOP, hud::X_RIGHT, hud::Y_TOP, it->left.c_str(),
                      CR_GREY);
...
static std::string WinToColorString(const WinInfo &win)
{
    ...
    if (win.type == WinInfo::WIN_PLAYER)
    {
        ...
        {
            StrFormat(buf, TEXTCOLOR_GREEN "%s" TEXTCOLOR_NORMAL, pl.userinfo.netname.c_str());
        }
        return buf;
    }
    else if (win.type == WinInfo::WIN_TEAM)
    {
        ...
        {
            StrFormat(buf, "%s%s" TEXTCOLOR_NORMAL, tm.TextColor.c_str(), tm.ColorString.c_str());
        }
        return buf;
    }
...
static void LevelStateHorde(levelStateLines_t &lines)
{
    ...
    if (info.hasBoss())
    {
        ...
        lines.subtitle[0] = "Defeat the " TEXTCOLOR_YELLOW "boss " TEXTCOLOR_GREY "to win the wave";
        tics              = info.bossTic();
    }
    else
    {
        if (::g_horde_waves.asInt() != 0)
        {
            StrFormat(lines.title, "Wave " TEXTCOLOR_YELLOW "%d " TEXTCOLOR_GREY "of " TEXTCOLOR_YELLOW "%d", info.wave,
                      ::g_horde_waves.asInt());
        }
        else
        {
        ...
        StrFormat(lines.subtitle[0], "\"" TEXTCOLOR_YELLOW "%s" TEXTCOLOR_GREY "\"", define.name.c_str());
...
void LevelStateHUD()
{
    ...
    case LevelState::WARMUP: {
        ...
        if (::sv_warmup)
        {
            ...
            {
                StrFormat(lines.subtitle[0], "Press " TEXTCOLOR_GOLD "%s" TEXTCOLOR_NORMAL " when ready to play",
    ...
    case LevelState::WARMUP_FORCED_COUNTDOWN: {
        ...
        StrFormat(lines.subtitle[0], "Match begins in " TEXTCOLOR_GREEN "%d", ::levelstate.getCountdown());
        break;
    }
    case LevelState::PREROUND_COUNTDOWN: {
        ...
        if (g_preroundreset)
        {
            StrFormat(lines.subtitle[0], "Round begins in " TEXTCOLOR_GREEN "%d", ::levelstate.getCountdown());
        }
        else
        {
            StrFormat(lines.subtitle[0], "Weapons unlocked in " TEXTCOLOR_GREEN "%d", ::levelstate.getCountdown());
        }
        break;
    }
    case LevelState::INGAME: {
        ...
        else if (G_CanShowFightMessage())
        {
            if (G_IsSidesGame())
            {
                ...
                lines.lucentFade(::level.time - ::levelstate.getIngameStartTime(), TICRATE * 2, TICRATE * 3);
            }
            else if (G_IsCoopGame())
            {
                ...
                {
                    StrFormat(lines.subtitle[0], TEXTCOLOR_GREEN "%d" TEXTCOLOR_GREY " attempts left",
    ...
    case LevelState::ENDROUND_COUNTDOWN: {
        if (G_IsCoopGame() || G_IsHordeMode())
            StrFormat(lines.title, "Attempt " TEXTCOLOR_YELLOW "%d " TEXTCOLOR_GREY "complete\n",
                      ::levelstate.getRound());
        else
            StrFormat(lines.title, "Round " TEXTCOLOR_YELLOW "%d " TEXTCOLOR_GREY "complete\n",
                      ::levelstate.getRound());
        ...
        if (win.type == WinInfo::WIN_DRAW)
            ...
            StrFormat(lines.subtitle[0], "%s wins the round", WinToColorString(win).c_str());
        else if (win.type == WinInfo::WIN_TEAM)
            StrFormat(lines.subtitle[0], "%s team wins the round", WinToColorString(win).c_str());
        else if (G_IsCoopGame() || G_IsHordeMode())
            StrFormat(lines.subtitle[0], "Next attempt in " TEXTCOLOR_GREEN "%d", ::levelstate.getCountdown());
        else
            StrFormat(lines.subtitle[0], "Next round in " TEXTCOLOR_GREEN "%d", ::levelstate.getCountdown());
        break;
    }
    case LevelState::ENDGAME_COUNTDOWN: {
        WinInfo win = ::levelstate.getWinInfo();
        // Upper Text
        if (win.type == WinInfo::WIN_EVERYBODY)
            StrFormat(lines.title, TEXTCOLOR_YELLOW "Mission Success!");
        else if (win.type == WinInfo::WIN_NOBODY)
        ...
        if (win.type == WinInfo::WIN_DRAW)
            ...
            StrFormat(lines.subtitle[0], "Intermission in " TEXTCOLOR_GREEN "%d", ::levelstate.getCountdown());
        break;
    }
    default:
        break;
    ...
    {
        ::screen->DrawTextStretchedLuc(CR_GREY, surface_width / 2 - w / 2, surface_height / 4 - h / 2,
...
void SpectatorHUD()
{
    ...
    hud::DrawText(0, iy, hud_scale, hud::X_CENTER, hud::Y_BOTTOM, hud::X_CENTER, hud::Y_BOTTOM, hud::HelpText().c_str(),
                  CR_GREY);
    iy += V_LineHeight() + 1;

    // Draw targeted player names.
    hud::EATargets(0, iy, hud_scale, hud::X_CENTER, hud::Y_BOTTOM, hud::X_CENTER, hud::Y_BOTTOM, 1, 0);
...
void DoomHUD()
{
    ...
    if (hud_timer)
    {
        hud::DrawText(0, st_y, hud_scale, hud::X_CENTER, hud::Y_BOTTOM, hud::X_CENTER, hud::Y_BOTTOM,
                      hud::Timer().c_str(), CR_UNTRANSLATED);
        st_y += V_LineHeight() + 1;
    }
    ...
    if (::hud_speedometer && ::consoleplayer_id == ::displayplayer_id)
    {
        std::string buf;
        StrFormat(buf, "%d" TEXTCOLOR_DARKGREY "ups", static_cast<int>(HU_GetPlayerSpeed()));
        hud::DrawText(0, st_y, hud_scale, hud::X_CENTER, hud::Y_BOTTOM, hud::X_CENTER, hud::Y_BOTTOM, buf.c_str(),
                      CR_GREY);
        st_y += V_LineHeight() + 1;
    }
    ...
    // Draw other player name, if spying
    hud::DrawText(0, st_y, hud_scale, hud::X_CENTER, hud::Y_BOTTOM, hud::X_CENTER, hud::Y_BOTTOM,
                  hud::SpyPlayerName().c_str(), CR_UNTRANSLATED);
    st_y += V_LineHeight() + 1;

    // Draw targeted player names.
    hud::EATargets(0, st_y, hud_scale, hud::X_CENTER, hud::Y_BOTTOM, hud::X_CENTER, hud::Y_BOTTOM, 1, hud_targetcount);
    st_y += V_LineHeight() + 1;
...
VERSION_CONTROL(st_new_cpp, "$Id: a79bf1c286266d9016fd44f4c40296eeb18543b7 $")
