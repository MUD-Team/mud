...
//-----------------------------------------------------------------------------
//
// $Id: 42bf0dfdfee3dbe93bf6db3be28c8d71d0bf3196 $
//
...
void R_MapLevelPlane(int32_t y, int32_t x1, int32_t x2)
{
    ...
    dspan.xfrac = pl_viewxtrans + FixedMul(FixedMul(pl_viewcos, distance), pl_xscale) + (x1 - centerx) * dspan.xstep;
    dspan.yfrac = pl_viewytrans - FixedMul(FixedMul(pl_viewsin, distance), pl_yscale) + (x1 - centerx) * dspan.ystep;
                        ...
                        fixed_t yscale, angle_t angle)
{
    ...
    for (check = visplanes[hash]; check; check = check->next)           // killough
        if (P_IdenticalPlanes(&secplane, &check->secplane) && picnum == check->picnum &&
            lightlevel == check->lightlevel && xoffs == check->xoffs && // killough 2/28/98: Add offset checks
            yoffs == check->yoffs && basecolormap == check->colormap && // [RH] Add colormap check
            xscale == check->xscale && yscale == check->yscale && angle == check->angle)
            return check;
...
VERSION_CONTROL(r_plane_cpp, "$Id: 42bf0dfdfee3dbe93bf6db3be28c8d71d0bf3196 $")
