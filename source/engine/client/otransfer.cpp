...
//-----------------------------------------------------------------------------
//
// $Id: b68ba44db2d73055ba5ee8cad907687d62c4d139 $
//
...
const char *ODAMEX_USERAGENT = "MUD/" DOTVERSIONSTR;
...
bool OTransfer::tick()
{
    ...
    if (ok != 0)
    {
        ...
        if (ok != 0)
        {
            ...
            std::string buf;
            StrFormat(buf, "File %s could not be renamed to %s - %s", m_filePart.c_str(), m_filename.c_str(),
                      strerror(errno));
