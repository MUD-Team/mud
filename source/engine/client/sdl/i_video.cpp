...
template <typename SOURCE_PIXEL_T, typename DEST_PIXEL_T>
static void BlitLoop(DEST_PIXEL_T *dest, const SOURCE_PIXEL_T *source, int destpitchpixels, int srcpitchpixels,
                     int destw, int desth, fixed_t xstep, fixed_t ystep, const argb_t *palette)
{
    fixed_t yfrac = 0;
...
//
void IWindowSurface::blit(const IWindowSurface *source_surface, int srcx, int srcy, int srcw, int srch, int destx,
...
void I_SetVideoMode(const IVideoMode &requested_mode)
{
    ...
    if (surface_width != primary_surface->getWidth() || surface_height != primary_surface->getHeight())
    {
...
void I_BlitEmulatedSurface()
{
    if (emulated_surface)
    {
        primary_surface->blit(emulated_surface, 0, 0, emulated_surface->getWidth(), emulated_surface->getHeight(), 0, 0,
...
void I_FinishUpdate()
{
    if (I_VideoInitialized())
    {
        ...
        if (converted_surface)
        {
            ...
            real_primary_surface->blit(converted_surface, 0, 0, converted_surface->getWidth(),
                                       converted_surface->getHeight(), 0, 0, real_primary_surface->getWidth(),
                                       real_primary_surface->getHeight());
        }
