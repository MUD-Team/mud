...
//-----------------------------------------------------------------------------
//
// $Id: 8baf3d9d1af3769b0e022fddf3d3922386c43b7a $
//
...
cvar_t *chat_macros[10] = {&chatmacro0, &chatmacro1, &chatmacro2, &chatmacro3, &chatmacro4,
...
BOOL HU_Responder(event_t *ev)
{
    ...
    if (altdown)
    {
        ...
        else if (ev->data1 >= OKEYP_1 && ev->data1 <= OKEYP_0 &&
                 ev->mod & OMOD_NUM) // Use numpad keys for chat macros if numlock is on
        {
...
static void HU_DrawCrosshair()
{
    ...
    if (hud_crosshair && crosshair_lump)
    {
        ...
        if (hud_crosshairhealth)
        {
            if (camera->health > 75)
            {
                crosshair_trans[crosshair_color] = V_BestColor(V_GetDefaultPalette()->basecolors, 0x00, 0xFF, 0x00);
            }
            else if (camera->health > 50)
            {
                crosshair_trans[crosshair_color] = V_BestColor(V_GetDefaultPalette()->basecolors, 0xFF, 0xFF, 0x00);
            }
            else if (camera->health > 25)
            {
                crosshair_trans[crosshair_color] = V_BestColor(V_GetDefaultPalette()->basecolors, 0xFF, 0x7F, 0x00);
            }
            else
            {
                crosshair_trans[crosshair_color] = V_BestColor(V_GetDefaultPalette()->basecolors, 0xFF, 0x00, 0x00);
            }
        }
        else
        {
...
static void HU_DrawChatPrompt()
{
    ...
    show_text += '_';
    screen->DrawTextStretched(CR_RED, 0, y, prompt, scaledxfac, scaledyfac);
    screen->DrawTextStretched(CR_GREY, promptwidth, y, show_text.c_str() + i, scaledxfac, scaledyfac);
}
...
void HU_Drawer()
{
    ...
    if (idmypos && gamestate == GS_LEVEL)
        Printf(PRINT_HIGH, "ang=%d;x,y,z=(%d,%d,%d)\n", displayplayer().camera->angle / FRACUNIT,
               displayplayer().camera->x / FRACUNIT, displayplayer().camera->y / FRACUNIT,
    ...
    if (consoleplayer().camera && !(demoplayback))
    {
        if ((gamestate != GS_INTERMISSION && Actions[ACTION_SHOWSCORES]) ||
            (::multiplayer && hud_show_scoreboard_ondeath && displayplayer().health <= 0 && !displayplayer().spectator))
        {
...
void drawHeader(player_t *player, int y)
{
    ...
    hud::DrawText(0, y, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_CENTER, hud::Y_TOP, str.c_str(),
                  CR_GOLD, true);
    ...
    {
        hud::DrawText(0, y + 8 * (i + 1), hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_CENTER, hud::Y_TOP,
                      hostname[i].string, CR_GREY, true);
    }
    V_FreeBrokenLines(hostname);

    // Left
    hud::DrawText(-236, y, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_LEFT, hud::Y_TOP,
                  "CLIENTS: ", CR_GREY, true);
    hud::DrawText(-236 + V_StringWidth("CLIENTS: "), y, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_LEFT,
                  hud::Y_TOP, hud::ClientsSplit().c_str(), CR_GREEN, true);
    ...
    if (G_IsTeamGame())
    {
        ...
        for (int i = 0; i < sv_teamsinplay; i++)
        {
            ...
            yOffset += 8;
            hud::DrawText(-236, y + yOffset, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_LEFT, hud::Y_TOP,
            ...
            hud::DrawText(-236 + xOffset, y + yOffset, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_LEFT,
                          hud::Y_TOP, points.c_str(), CR_GREEN, true);
        }
    }
    else
    {
        hud::DrawText(-236, y + 8, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_LEFT, hud::Y_TOP,
                      "PLAYERS: ", CR_GREY, true);
        hud::DrawText(-236 + V_StringWidth("PLAYERS: "), y + 8, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE,
                      hud::X_LEFT, hud::Y_TOP, hud::PlayersSplit().c_str(), CR_GREEN, true);
        yOffset += 16;
    }
    ...
    {
        int yoff = i * 8;
        hud::DrawText(236 - rw, y + yoff, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_RIGHT, hud::Y_TOP,
                      names.at(i).c_str(), CR_GREY, true);
        hud::DrawText(236 - rw, y + yoff, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_LEFT, hud::Y_TOP,
                      values.at(i).c_str(), CR_GREEN, true);
    }

    // Line
    for (short xi = -236 + 1; xi < 236; xi += 2)
    {
        hud::DrawTranslatedPatch(xi, y + yOffset + 8, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_CENTER,
                                 hud::Y_TOP, W_ResolvePatchHandle(sbline), Ranges + CR_GREY * 256, true);
    }
}
...
void drawScores(player_t *player, int y, byte extra_rows)
{
    ...
    for (size_t i = 0; i < ARRAY_LENGTH(columns); i++)
    {
        ...
        case SCOL_NONE:
            break;
        case SCOL_DAMAGE:
            hud::DrawText(x, y, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_RIGHT, hud::Y_TOP, "DAMAGE",
                          CR_GREY, true);
            hud::EAPlayerDamage(x, y + 11, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_RIGHT, hud::Y_TOP,
                                1, limit, true);
            break;
        case SCOL_LIVES:
            hud::DrawText(x, y, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_RIGHT, hud::Y_TOP, "LIVES",
                          CR_GREY, true);
            hud::EAPlayerLives(x, y + 11, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_RIGHT, hud::Y_TOP,
                               1, limit, true);
            break;
        case SCOL_DEATHS:
            hud::DrawText(x, y, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_RIGHT, hud::Y_TOP, "DETHS",
                          CR_GREY, true);
            hud::EAPlayerDeaths(x, y + 11, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_RIGHT, hud::Y_TOP,
                                1, limit, true);
            break;
        case SCOL_WINS:
            hud::DrawText(x, y, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_RIGHT, hud::Y_TOP, "WINS",
                          CR_GREY, true);
            hud::EAPlayerRoundWins(x, y + 11, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_RIGHT,
                                   hud::Y_TOP, 1, limit, true);
            break;
        case SCOL_FRAGS:
            hud::DrawText(x, y, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_RIGHT, hud::Y_TOP, "FRAGS",
                          CR_GREY, true);
            hud::EAPlayerFrags(x, y + 11, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_RIGHT, hud::Y_TOP,
                               1, limit, true);
            break;
        case SCOL_KD:
            hud::DrawText(x, y, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_RIGHT, hud::Y_TOP, "K/D",
                          CR_GREY, true);
            hud::EAPlayerKD(x, y + 11, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_RIGHT, hud::Y_TOP, 1,
                            limit, true);
            break;
        }
    }
    ...
    hud::DrawText(-227, y, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_LEFT, hud::Y_TOP, "Name", CR_GREY,
                  true);
    hud::DrawText(188, y, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_RIGHT, hud::Y_TOP, "MINS", CR_GREY,
                  true);
    hud::DrawText(236, y, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_RIGHT, hud::Y_TOP, "PING", CR_GREY,
                  true);
    ...
    for (short xi = -236 + 1; xi < 236; xi += 2)
    {
        hud::DrawTranslatedPatch(xi, y + 8, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_CENTER,
                                 hud::Y_TOP, pSBLine, Ranges + CR_GREY * 256, true);
    }

    // Ingame Players
    hud::EAPlayerColors(-236, y + 11, 7, 7, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_LEFT, hud::Y_TOP,
                        1, limit);
    hud::EAPlayerNames(-227, y + 11, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_LEFT, hud::Y_TOP, 1,
                       limit, true);

    // Other column data
    hud::EAPlayerTimes(188, y + 11, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_RIGHT, hud::Y_TOP, 1,
                       limit, true);
    hud::EAPlayerPings(236, y + 11, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_RIGHT, hud::Y_TOP, 1,
                       limit, true);
}
...
void drawTeamScores(player_t *player, int &y, byte extra_rows)
{
    ...
    for (int i = 0; i < teams + 1; i++)
    {
        ...
        // Column headers
        hud::DrawText(xOffset + 9, yOffset + y, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_LEFT,
                      hud::Y_TOP, "Name", CR_GREY, true);
        if (sv_gametype == GM_CTF)
        {
            if (g_lives)
            {
                hud::DrawText(xOffset + 168, yOffset + y, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE,
                              hud::X_RIGHT, hud::Y_TOP, "PTS", CR_GREY, true);
                hud::DrawText(xOffset + 200, yOffset + y, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE,
        ...
        else
        {
            if (g_lives)
            {
                hud::DrawText(xOffset + 164, yOffset + y, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE,
                              hud::X_RIGHT, hud::Y_TOP, "FRG", CR_GREY, true);
                hud::DrawText(xOffset + 200, y, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_RIGHT,
                              hud::Y_TOP, "LIV", CR_GREY, true);
            }
            else
            {
                hud::DrawText(xOffset + 164, yOffset + y, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE,
                              hud::X_RIGHT, hud::Y_TOP, "FRG", CR_GREY, true);
                hud::DrawText(xOffset + 200, y, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_RIGHT,
                              hud::Y_TOP, "K/D", CR_GREY, true);
            }
        }
        hud::DrawText(xOffset + 232, yOffset + y, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_RIGHT,
        ...
        {
            hud::DrawTranslatedPatch(xi, yOffset + y + 8, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE,
                                     hud::X_LEFT, hud::Y_TOP, pSBLine, Ranges + color * 256, true);
            hud::DrawTranslatedPatch(xi, yOffset + y + 19, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE,
                                     hud::X_LEFT, hud::Y_TOP, pSBLine, Ranges + color * 256, true);
        }

        // Team Info
        str = hud::TeamName(color, i);
        hud::DrawText(xOffset + 9, yOffset + y + 11, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_LEFT,
                      hud::Y_TOP, str.c_str(), color, true);
        if (sv_gametype == GM_CTF)
        {
            if (g_lives)
            {
                str = hud::TeamPoints(color, i);
                hud::DrawText(xOffset + 168, yOffset + y + 11, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE,
                              hud::X_RIGHT, hud::Y_TOP, str.c_str(), color, true);
                hud::TeamLives(str, color, i);
                hud::DrawText(xOffset + 200, yOffset + y + 11, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE,
                              hud::X_RIGHT, hud::Y_TOP, str.c_str(), color, true);
            }
            else
            {
        ...
        else
        {
            ...
            {
                str = hud::TeamPoints(color, i);
                hud::DrawText(xOffset + 164, yOffset + y + 11, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE,
                              hud::X_RIGHT, hud::Y_TOP, str.c_str(), color, true);
                hud::TeamLives(str, color, i);
                hud::DrawText(xOffset + 200, yOffset + y + 11, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE,
                              hud::X_RIGHT, hud::Y_TOP, str.c_str(), color, true);
        ...
        hud::DrawText(xOffset + 232, yOffset + y + 11, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_RIGHT,
                      hud::Y_TOP, str.c_str(), color, true);

        // Ingame Players
        byte limit = extra_rows + 4;
        hud::EATeamPlayerColors(xOffset, yOffset + y + 22, 7, 7, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE,
                                hud::X_LEFT, hud::Y_TOP, 1, limit, i);
        hud::EATeamPlayerNames(xOffset + 9, yOffset + y + 22, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE,
                               hud::X_LEFT, hud::Y_TOP, 1, limit, i, true);
        if (sv_gametype == GM_CTF)
        {
            if (g_lives)
            {
                hud::EATeamPlayerPoints(xOffset + 168, yOffset + y + 22, hud_scalescoreboard, hud::X_CENTER,
                                        hud::Y_MIDDLE, hud::X_RIGHT, hud::Y_TOP, 1, limit, i, true);
                hud::EATeamPlayerLives(xOffset + 200, yOffset + y + 22, hud_scalescoreboard, hud::X_CENTER,
                                       hud::Y_MIDDLE, hud::X_RIGHT, hud::Y_TOP, 1, limit, i, true);
            }
            else
            {
                hud::EATeamPlayerPoints(xOffset + 168, yOffset + y + 22, hud_scalescoreboard, hud::X_CENTER,
                                        hud::Y_MIDDLE, hud::X_RIGHT, hud::Y_TOP, 1, limit, i, true);
                hud::EATeamPlayerFrags(xOffset + 200, yOffset + y + 22, hud_scalescoreboard, hud::X_CENTER,
                                       hud::Y_MIDDLE, hud::X_RIGHT, hud::Y_TOP, 1, limit, i, true);
            }
        }
        else
        {
            if (g_lives)
            {
                hud::EATeamPlayerFrags(xOffset + 164, yOffset + y + 22, hud_scalescoreboard, hud::X_CENTER,
                                       hud::Y_MIDDLE, hud::X_RIGHT, hud::Y_TOP, 1, limit, i, true);
                hud::EATeamPlayerLives(xOffset + 200, yOffset + y + 22, hud_scalescoreboard, hud::X_CENTER,
                                       hud::Y_MIDDLE, hud::X_RIGHT, hud::Y_TOP, 1, limit, i, true);
            }
            else
            {
                hud::EATeamPlayerFrags(xOffset + 164, yOffset + y + 22, hud_scalescoreboard, hud::X_CENTER,
                                       hud::Y_MIDDLE, hud::X_RIGHT, hud::Y_TOP, 1, limit, i, true);
                hud::EATeamPlayerKD(xOffset + 200, yOffset + y + 22, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE,
                                    hud::X_RIGHT, hud::Y_TOP, 1, limit, i, true);
            }
        }
        hud::EATeamPlayerPings(xOffset + 232, yOffset + y + 22, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE,
                               hud::X_RIGHT, hud::Y_TOP, 1, limit, i, true);
    }
}
...
void drawSpectators(player_t *player, int y, byte extra_rows)
{
    ...
    for (short xi = -236 + 1; xi < 236; xi += 2)
    {
        hud::DrawTranslatedPatch(xi, y, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_CENTER, hud::Y_TOP,
                                 pSBLine, Ranges + CR_GREY * 256, true);
    }
    ...
    for (byte i = 0; i < 3; i++)
    {
        ...
        hud::EASpectatorNames(tx[i], y + 3, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_LEFT, hud::Y_TOP,
                              1, skip, limit, true);
        hud::EASpectatorPings(tx[i] + 124, y + 3, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_LEFT,
                              hud::Y_TOP, 1, skip, limit, true);
        skip += limit;
    }
}
...
void Scoreboard(player_t *player)
{
    ...
    hud::Dim(0, y, HiResolutionWidth, height, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_CENTER,
             hud::Y_TOP);
...
void drawLowScores(player_t *player, int y, byte extra_rows)
{
    ...
    for (size_t i = 0; i < ARRAY_LENGTH(columns); i++)
    {
        ...
        case SCOL_NONE:
            break;
        case SCOL_DAMAGE:
            hud::DrawText(x, y, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_RIGHT, hud::Y_TOP, "DMG",
                          CR_GREY, true);
            hud::EAPlayerDamage(x, y + 11, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_RIGHT, hud::Y_TOP,
                                1, limit, true);
            break;
        case SCOL_LIVES:
            hud::DrawText(x, y, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_RIGHT, hud::Y_TOP, "LIV",
                          CR_GREY, true);
            hud::EAPlayerLives(x, y + 11, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_RIGHT, hud::Y_TOP,
                               1, limit, true);
            break;
        case SCOL_DEATHS:
            hud::DrawText(x, y, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_RIGHT, hud::Y_TOP, "DTH",
                          CR_GREY, true);
            hud::EAPlayerDeaths(x, y + 11, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_RIGHT, hud::Y_TOP,
                                1, limit, true);
            break;
        case SCOL_WINS:
            hud::DrawText(x, y, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_RIGHT, hud::Y_TOP, "WIN",
                          CR_GREY, true);
            hud::EAPlayerRoundWins(x, y + 11, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_RIGHT,
                                   hud::Y_TOP, 1, limit, true);
            break;
        case SCOL_FRAGS:
            hud::DrawText(x, y, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_RIGHT, hud::Y_TOP, "FRG",
                          CR_GREY, true);
            hud::EAPlayerFrags(x, y + 11, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_RIGHT, hud::Y_TOP,
                               1, limit, true);
            break;
    ...
    // Other column headers
    hud::DrawText(-137, y, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_LEFT, hud::Y_TOP, "Name", CR_GREY,
                  true);
    hud::DrawText(118, y, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_RIGHT, hud::Y_TOP, "MIN", CR_GREY,
                  true);
    hud::DrawText(146, y, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_RIGHT, hud::Y_TOP, "PNG", CR_GREY,
                  true);
    ...
    // Ingame Players
    hud::EAPlayerColors(-146, y + 11, 7, 7, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_LEFT, hud::Y_TOP,
                        1, limit);
    hud::EAPlayerNames(-137, y + 11, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_LEFT, hud::Y_TOP, 1,
                       limit, true);

    // Other column data
    hud::EAPlayerTimes(118, y + 11, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_RIGHT, hud::Y_TOP, 1,
                       limit, true);
    hud::EAPlayerPings(146, y + 11, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_RIGHT, hud::Y_TOP, 1,
                       limit, true);
}
...
void drawLowTeamScores(player_t *player, int y, byte extra_rows)
{
    ...
    if (sv_gametype == GM_CTF)
    {
        hud::DrawText(34, y, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_RIGHT, hud::Y_TOP, "PPL",
                      CR_GREY, true);
        if (g_lives)
        {
            hud::DrawText(62, y, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_RIGHT, hud::Y_TOP, "PTS",
                          CR_GREY, true);
            hud::DrawText(90, y, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_RIGHT, hud::Y_TOP, "LIV",
                          CR_GREY, true);
        }
        else
        {
            hud::DrawText(62, y, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_RIGHT, hud::Y_TOP, "PTS",
                          CR_GREY, true);
            hud::DrawText(90, y, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_RIGHT, hud::Y_TOP, "FRG",
                          CR_GREY, true);
        }
    }
    else
    {
        hud::DrawText(22, y, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_RIGHT, hud::Y_TOP, "PPL",
                      CR_GREY, true);
        if (g_lives)
        {
            hud::DrawText(50, y, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_RIGHT, hud::Y_TOP, "FRG",
                          CR_GREY, true);
            hud::DrawText(90, y, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_RIGHT, hud::Y_TOP, "LIV",
                          CR_GREY, true);
        }
        else
        {
            hud::DrawText(50, y, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_RIGHT, hud::Y_TOP, "FRG",
                          CR_GREY, true);
            hud::DrawText(90, y, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_RIGHT, hud::Y_TOP, "K/D",
                          CR_GREY, true);
        }
    }
    hud::DrawText(118, y, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_RIGHT, hud::Y_TOP, "MIN", CR_GREY,
    ...
    for (int i = 0; i < sv_teamsinplay; i++)
    {
        ...
        for (short xi = -146 + 1; xi < 146; xi += 2)
        {
            hud::DrawTranslatedPatch(xi, y + yOffset, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_CENTER,
                                     hud::Y_TOP, pSBLine, Ranges + color * 256, true);
            hud::DrawTranslatedPatch(xi, y + yOffset + 11, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE,
                                     hud::X_CENTER, hud::Y_TOP, pSBLine, Ranges + color * 256, true);
        }

        // Team Info
        str = hud::TeamName(color, i);
        hud::DrawText(-137, y + yOffset + 3, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_LEFT, hud::Y_TOP,
                      str.c_str(), color, true);

        if (sv_gametype == GM_CTF)
        {
            str = hud::TeamPlayers(color, i);
            hud::DrawText(34, y + yOffset + 3, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_RIGHT,
                          hud::Y_TOP, str.c_str(), color, true);

            if (g_lives)
            {
                str = hud::TeamPoints(color, i);
                hud::DrawText(62, y + yOffset + 3, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_RIGHT,
                              hud::Y_TOP, str.c_str(), color, true);
                hud::TeamLives(str, color, i);
                hud::DrawText(90, y + yOffset + 3, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_RIGHT,
                              hud::Y_TOP, str.c_str(), color, true);
            }
            else
            {
        ...
        {
            str = hud::TeamPlayers(color, i);
            hud::DrawText(22, y + yOffset + 3, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_RIGHT,
                          hud::Y_TOP, str.c_str(), color, true);

            if (g_lives)
            {
                str = hud::TeamPoints(color, i);
                hud::DrawText(50, y + yOffset + 3, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_RIGHT,
                              hud::Y_TOP, str.c_str(), color, true);
                hud::TeamLives(str, color, i);
                hud::DrawText(90, y + yOffset + 3, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_RIGHT,
                              hud::Y_TOP, str.c_str(), color, true);
        ...
        str = hud::TeamPing(color, i);
        hud::DrawText(146, y + yOffset + 3, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_RIGHT, hud::Y_TOP,
                      str.c_str(), color, true);

        // Ingame Players
        byte limit = extra_rows + 4;
        hud::EATeamPlayerColors(-146, y + yOffset + 14, 7, 7, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE,
                                hud::X_LEFT, hud::Y_TOP, 1, limit, i);
        hud::EATeamPlayerNames(-137, y + yOffset + 14, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_LEFT,
        ...
        if (sv_gametype == GM_CTF)
        {
            if (g_lives)
            {
                hud::EATeamPlayerPoints(62, y + yOffset + 14, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE,
                                        hud::X_RIGHT, hud::Y_TOP, 1, limit, i, true);
                hud::EATeamPlayerLives(90, y + yOffset + 14, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE,
                                       hud::X_RIGHT, hud::Y_TOP, 1, limit, i, true);
            }
            else
            {
                hud::EATeamPlayerPoints(62, y + yOffset + 14, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE,
                                        hud::X_RIGHT, hud::Y_TOP, 1, limit, i, true);
                hud::EATeamPlayerFrags(90, y + yOffset + 14, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE,
                                       hud::X_RIGHT, hud::Y_TOP, 1, limit, i, true);
            }
        }
        else
        {
            if (g_lives)
            {
                hud::EATeamPlayerFrags(50, y + yOffset + 14, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE,
                                       hud::X_RIGHT, hud::Y_TOP, 1, limit, i, true);
                hud::EATeamPlayerLives(90, y + yOffset + 14, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE,
                                       hud::X_RIGHT, hud::Y_TOP, 1, limit, i, true);
            }
            else
            {
                hud::EATeamPlayerFrags(50, y + yOffset + 14, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE,
                                       hud::X_RIGHT, hud::Y_TOP, 1, limit, i, true);
                hud::EATeamPlayerKD(90, y + yOffset + 14, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE,
                                    hud::X_RIGHT, hud::Y_TOP, 1, limit, i, true);
            }
        }

        hud::EATeamPlayerTimes(118, y + yOffset + 14, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_RIGHT,
                               hud::Y_TOP, 1, limit, i, true);
        hud::EATeamPlayerPings(146, y + yOffset + 14, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_RIGHT,
                               hud::Y_TOP, 1, limit, i, true);
...
void drawLowSpectators(player_t *player, int y, byte extra_rows)
{
    ...
    for (byte i = 0; i < 2; i++)
    {
        ...
        hud::EASpectatorNames(tx[i], y + 3, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_LEFT, hud::Y_TOP,
                              1, skip, limit, true);
        hud::EASpectatorPings(tx[i] + 121, y + 3, hud_scalescoreboard, hud::X_CENTER, hud::Y_MIDDLE, hud::X_LEFT,
                              hud::Y_TOP, 1, skip, limit, true);
        skip += limit;
    }
}
...
void LowScoreboard(player_t *player)
{
    ...
    hud::drawLowSpectators(player, y + (height - 14 - (extra_spec_rows * 8)), extra_spec_rows);
}
...
void HU_ConsoleScores(player_t *player)
{
        ...
        for (int team_num = 0; team_num < sv_teamsinplay; team_num++)
        {
            ...
            for (PlayerPtrList::const_iterator it = sortedplayers.begin(); it != sortedplayers.end(); ++it)
            {
                ...
                if (itplayer->userinfo.team == team_num)
                {
                    ...
                    sprintf(str, "%-15s %-6d N/A  %-5d %4d\n", itplayer->userinfo.netname.c_str(), itplayer->points,
                            // itplayer->captures,
                            itplayer->fragcount, itplayer->GameTime / 60);
    ...
    else if (sv_gametype == GM_TEAMDM)
    {
        ...
        for (int team_num = 0; team_num < sv_teamsinplay; team_num++)
        {
            ...
            for (PlayerPtrList::const_iterator it = sortedplayers.begin(); it != sortedplayers.end(); ++it)
            {
                ...
                if (itplayer->userinfo.team == team_num)
                {
                    sprintf(str, "%-15s %-5d %-6d %2.1f %4d\n", itplayer->userinfo.netname.c_str(), itplayer->fragcount,
                            itplayer->deathcount, HU_CalculateFragDeathRatio(itplayer), itplayer->GameTime / 60);
    ...
    else if (G_IsLivesGame())
    {
        ...
        {
            const player_t *itplayer = *it;
            sprintf(str, "%-15s %-5d %-6d %2.1f %4d\n", itplayer->userinfo.netname.c_str(), itplayer->killcount,
                    itplayer->deathcount, HU_CalculateKillDeathRatio(itplayer), itplayer->GameTime / 60);
...
VERSION_CONTROL(hu_stuff_cpp, "$Id: 8baf3d9d1af3769b0e022fddf3d3922386c43b7a $")
