...
void MaplistCache::check_complete()
{
    ...
    if (this->maplist.size() > 0 && (this->size == this->maplist.size()) && (this->valid_indexes > 0))
    {
...
bool MaplistCache::query(const std::vector<std::string> &query, maplist_qrows_t &result)
{
    // Check to see if the query should fail and pass an error.
    if (this->status != MAPLIST_OK)
    {
...
void MaplistCache::defer_query(query_callback_t query_callback, query_errback_t query_errback)
{
...
// A deferred query.
void MaplistCache::defer_query(const std::vector<std::string> &query, query_callback_t query_callback,
...
BEGIN_COMMAND(maplist)
{
    ...
    MaplistCache::instance().defer_query(arguments, &CMD_MaplistCallback, &CMD_MaplistErrback);
}
END_COMMAND(maplist)
