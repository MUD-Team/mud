...
//-----------------------------------------------------------------------------
//
// $Id: d31def67e3ee7d19274d77079d5abe329d8273e1 $
//
...
static byte CheatPowerup[7][10] = {
    {'i', 'd', 'b', 'e', 'h', 'o', 'l', 'd', 'v', 255}, {'i', 'd', 'b', 'e', 'h', 'o', 'l', 'd', 's', 255},
    {'i', 'd', 'b', 'e', 'h', 'o', 'l', 'd', 'i', 255}, {'i', 'd', 'b', 'e', 'h', 'o', 'l', 'd', 'r', 255},
    {'i', 'd', 'b', 'e', 'h', 'o', 'l', 'd', 'a', 255}, {'i', 'd', 'b', 'e', 'h', 'o', 'l', 'd', 'l', 255},
    {'i', 'd', 'b', 'e', 'h', 'o', 'l', 'd', 255}};

cheatseq_t DoomCheats[] = {{CheatMus, 0, 1, 0, {0, 0}, CHEAT_ChangeMusic},
...
static void ST_refreshBackground()
{
    ...
    if (multiplayer)
    {
        if (!demoplayback)
        {
            ...
            stbar_canvas->DrawTranslatedPatch(W_ResolvePatchHandle(faceback), ST_FX, ST_FY);
        }
        else
        {
            stbar_canvas->DrawPatch(W_ResolvePatchHandle(faceclassic[displayplayer_id - 1]), ST_FX, ST_FY);
        }
    }
...
void ST_Drawer()
{
    ...
    if (st_statusbaron)
    {
        ...
        if (st_needrefresh)
        {
            ...
            if (st_scale)
                stnum_surface->blit(stbar_surface, 0, 0, stbar_surface->getWidth(), stbar_surface->getHeight(), 0, 0,
                                    stnum_surface->getWidth(), stnum_surface->getHeight());
            else
                surface->blit(stbar_surface, 0, 0, stbar_surface->getWidth(), stbar_surface->getHeight(), ST_X, ST_Y,
                              ST_WIDTH, ST_HEIGHT);
        }
        ...
        if (st_scale)
            surface->blit(stnum_surface, 0, 0, stnum_surface->getWidth(), stnum_surface->getHeight(), ST_X, ST_Y,
                          ST_WIDTH, ST_HEIGHT);
...
void ST_createWidgets()
{
    ...
    for (int i = 0; i < 6; i++)
    {
        w_arms[i].init(ST_ARMSX + (i % 3) * ST_ARMSXSPACE, ST_ARMSY + (i / 3) * ST_ARMSYSPACE, arms[i],
                       &st_weaponowned[i]);
    }

    // frags sum
    w_frags.init(ST_FRAGSX, ST_FRAGSY, tallnum, &st_fragscount, ST_FRAGSWIDTH);
    ...
    w_maxammo[0].init(ST_MAXAMMO0X, ST_MAXAMMO0Y, shortnum, &st_maxammo[0], ST_MAXAMMO0WIDTH);
    w_maxammo[1].init(ST_MAXAMMO1X, ST_MAXAMMO1Y, shortnum, &st_maxammo[1], ST_MAXAMMO1WIDTH);
    w_maxammo[2].init(ST_MAXAMMO2X, ST_MAXAMMO2Y, shortnum, &st_maxammo[2], ST_MAXAMMO2WIDTH);
    w_maxammo[3].init(ST_MAXAMMO3X, ST_MAXAMMO3Y, shortnum, &st_maxammo[3], ST_MAXAMMO3WIDTH);
...
VERSION_CONTROL(st_stuff_cpp, "$Id: d31def67e3ee7d19274d77079d5abe329d8273e1 $")
