...
void AM_restoreScaleAndLoc()
{
    ...
    {
        M_SetVec2Fixed(&m_ll, displayplayer().camera->x - m_wh.x / 2, displayplayer().camera->y - m_wh.y / 2);
    }
    M_AddVec2Fixed(&m_ur, &m_ll, &m_wh);
...
void AM_initColors(const bool overlayed)
{
    ...
    if (overlayed && !am_ovshare)
    {
        ...
        gameinfo.currentAutomapColors.ThingColor_Item =
            AM_GetColorFromString(palette_colors, am_ovthingcolor_item.cstring());
        gameinfo.currentAutomapColors.ThingColor_CountItem =
            AM_GetColorFromString(palette_colors, am_ovthingcolor_countitem.cstring());
        gameinfo.currentAutomapColors.ThingColor_Monster =
            AM_GetColorFromString(palette_colors, am_ovthingcolor_monster.cstring());
        gameinfo.currentAutomapColors.ThingColor_NoCountMonster =
            AM_GetColorFromString(palette_colors, am_ovthingcolor_nocountmonster.cstring());
        gameinfo.currentAutomapColors.ThingColor_Friend =
            AM_GetColorFromString(palette_colors, am_ovthingcolor_friend.cstring());
        gameinfo.currentAutomapColors.ThingColor_Projectile =
            AM_GetColorFromString(palette_colors, am_ovthingcolor_projectile.cstring());
    ...
    else if (am_usecustomcolors || (overlayed && am_ovshare))
    {
        ...
        gameinfo.currentAutomapColors.ThingColor_Item =
            AM_GetColorFromString(palette_colors, am_thingcolor_item.cstring());
        gameinfo.currentAutomapColors.ThingColor_CountItem =
            AM_GetColorFromString(palette_colors, am_thingcolor_countitem.cstring());
        gameinfo.currentAutomapColors.ThingColor_Monster =
            AM_GetColorFromString(palette_colors, am_thingcolor_monster.cstring());
        gameinfo.currentAutomapColors.ThingColor_NoCountMonster =
            AM_GetColorFromString(palette_colors, am_thingcolor_nocountmonster.cstring());
        gameinfo.currentAutomapColors.ThingColor_Friend =
            AM_GetColorFromString(palette_colors, am_thingcolor_friend.cstring());
        gameinfo.currentAutomapColors.ThingColor_Projectile =
            AM_GetColorFromString(palette_colors, am_thingcolor_projectile.cstring());
        ...
        gameinfo.currentAutomapColors.TeleportColor = AM_GetColorFromString(palette_colors, am_teleportcolor.cstring());
        {
            ...
            gameinfo.currentAutomapColors.AlmostBackground.index =
                V_BestColor(palette_colors, gameinfo.currentAutomapColors.AlmostBackground.rgb);
        }
    }
    else
    {
...
void AM_doFollowPlayer()
{
    ...
    {
        M_SetVec2Fixed(&m_ll, FTOM(MTOF(p.camera->x)) - m_wh.x / 2, FTOM(MTOF(p.camera->y)) - m_wh.y / 2);
        M_AddVec2Fixed(&m_ur, &m_ll, &m_wh);
    ...
    for (int i = 0; i < numlines; i++)
    {
        ...
        if (am_cheating || (lines[i].flags & ML_MAPPED))
        {
            if ((lines[i].flags & ML_DONTDRAW) && !am_cheating)
                continue;
            if (!lines[i].backsector && ((am_usecustomcolors || viewactive) || (!am_usecustomcolors && !viewactive)))
            {
                AM_drawMline(&l, gameinfo.currentAutomapColors.WallColor);
            }
            else
            {
                if ((P_IsTeleportLine(lines[i].special)) && (am_usecustomcolors || viewactive))
                { // teleporters
                    AM_drawMline(&l, gameinfo.currentAutomapColors.TeleportColor);
                }
                else if ((P_IsExitLine(lines[i].special)) && (am_usecustomcolors || viewactive))
                { // exit
...
{
    const fixed_t tmpx =
        FixedMul(pt.x, finecosine[a >> ANGLETOFINESHIFT]) - FixedMul(pt.y, finesine[a >> ANGLETOFINESHIFT]);

    pt.y = FixedMul(pt.x, finesine[a >> ANGLETOFINESHIFT]) + FixedMul(pt.y, finecosine[a >> ANGLETOFINESHIFT]);
...
void AM_drawLineCharacter(const std::vector<mline_t> &lineguy, fixed_t scale, angle_t angle, am_color_t color,
                          fixed_t x, fixed_t y)
{
...
void AM_drawPlayers()
{
    ...
    if (!multiplayer)
    {
        ...
        if (am_cheating && !gameinfo.mapArrowCheat.empty())
            AM_drawLineCharacter(gameinfo.mapArrowCheat, INT2FIXED(16), angle, gameinfo.currentAutomapColors.YourColor,
                                 conplayer.camera->x, conplayer.camera->y);
        else
            AM_drawLineCharacter(gameinfo.mapArrow, INT2FIXED(16), angle, gameinfo.currentAutomapColors.YourColor,
                                 conplayer.camera->x, conplayer.camera->y);
        return;
    }
...
{
    if (t->sprite == SPR_BKEY || t->sprite == SPR_YKEY || t->sprite == SPR_RKEY || t->sprite == SPR_BSKU ||
        t->sprite == SPR_YSKU || t->sprite == SPR_RSKU)
    {
        return true;
    }
        ...
        while (t)
        {
            ...
            if (AM_actorIsKey(t))
            {
                if (!G_GetCurrentSkill().easy_key)
                {
                    ...
                    AM_drawLineCharacter(gameinfo.cheatKey, t->radius, 0, key_color, p.x, p.y);
                }
            }
            else
            {
